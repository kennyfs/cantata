...
#define DBUG \
    if (debugIsEnabled) qWarning() << "Avahi" << __FUNCTION__

void AvahiDiscovery::enableDebug() { debugIsEnabled = true; }
...
static void resolverCallback(
    ...
    AVAHI_GCC_UNUSED const char* domain, AVAHI_GCC_UNUSED const char* hostName,
    ...
    switch (event) {
        ...
        case AVAHI_BROWSER_NEW:
            if (!(avahi_service_resolver_new(
                    avahi_service_browser_get_client(b), interface, protocol,
                    name, type, domain, AVAHI_PROTO_UNSPEC,
                    AVAHI_LOOKUP_USE_MULTICAST, resolverCallback,
                    new int(AVAHI_BROWSER_NEW)))) {
...
static void clientCallback(AvahiClient* c, AvahiClientState state,
    ...
    if (AVAHI_CLIENT_FAILURE == state) {
        DBUG << "Server connection failure: "
...
AvahiDiscovery::AvahiDiscovery() {
    ...
    /* create avahi client */
    int error;
    m_client = avahi_client_new(getAvahiPoll(), AVAHI_CLIENT_IGNORE_USER_CONFIG,
                                clientCallback, nullptr, &error);
    if (!m_client) {
    ...
    m_browser = avahi_service_browser_new(
        m_client, AVAHI_IF_UNSPEC, AVAHI_PROTO_INET, "_mpd._tcp", nullptr,
        AVAHI_LOOKUP_USE_MULTICAST, browseCallback, m_client);
    if (!m_browser) {
        DBUG << "Failed to create service browser: "
