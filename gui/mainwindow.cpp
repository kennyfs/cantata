...
#endif
{
    ...
    if (check->inherits("Kvantum::Style")) {
        ...
        toolbar->setFixedHeight(qMax(54, (int)(fontMetrics().height() * 3.25) +
                                             (toolbarLayout->spacing() * 3) +
                                             (vSpace * 2)));
    }
#endif
    ...
    prefAction = ActionCollection::get()->createAction(
        "configure",
        Utils::KDE == Utils::currentDe() ? tr("Configure Cantata...")
                                         : tr("Preferences..."),
        Icons::self()->configureIcon);
    connect(prefAction, SIGNAL(triggered()), this,
            SLOT(showPreferencesDialog()));
    quitAction = ActionCollection::get()->createAction(
        "quit", tr("Quit"),
        MonoIcon::icon(FontAwesome::poweroff, MonoIcon::constRed,
                       MonoIcon::constRed));
    connect(quitAction, SIGNAL(triggered()), this, SLOT(quit()));
    quitAction->setShortcut(QKeySequence::Quit);
    Action* aboutAction = ActionCollection::get()->createAction(
        "about", tr("About Cantata..."), Icons::self()->appIcon);
    ...
    serverInfoAction = ActionCollection::get()->createAction(
        "mpdinfo", tr("Server information..."),
        MonoIcon::icon(FontAwesome::server, iconCol));
    connect(serverInfoAction, SIGNAL(triggered()), this,
            SLOT(showServerInfo()));
    serverInfoAction->setEnabled(Settings::self()->firstRun());
    refreshDbAction = ActionCollection::get()->createAction(
        "refresh", tr("Refresh Database"), Icons::self()->refreshIcon);
    doDbRefreshAction =
        new Action(refreshDbAction->icon(), tr("Refresh"), this);
    refreshDbAction->setEnabled(false);
    connectAction = new Action(Icons::self()->connectIcon, tr("Connect"), this);
    connectionsAction = new Action(MonoIcon::icon(FontAwesome::server, iconCol),
                                   tr("Collection"), this);
    partitionsAction = new Action(MonoIcon::icon(FontAwesome::columns, iconCol),
                                  tr("Partitions"), this);
    outputsAction = new Action(MonoIcon::icon(FontAwesome::volumeup, iconCol),
                               tr("Outputs"), this);
    stopAfterTrackAction = ActionCollection::get()->createAction(
        "stopaftertrack", tr("Stop After Track"),
        Icons::self()->toolbarStopIcon);

    QList<int> seeks = QList<int>() << 5 << 30 << 60;
    QList<int> seekShortcuts =
        QList<int>() << (int)Qt::ControlModifier << (int)Qt::ShiftModifier
                     << (int)(Qt::ControlModifier | Qt::ShiftModifier);
    for (int i = 0; i < seeks.length(); ++i) {
        int seek = seeks.at(i);
        Action* fwdAction = ActionCollection::get()->createAction(
            "seekfwd" + QString::number(seek),
            tr("Seek forward (%1 seconds)").arg(seek));
        Action* revAction = ActionCollection::get()->createAction(
            "seekrev" + QString::number(seek),
            tr("Seek backward (%1 seconds)").arg(seek));
        fwdAction->setProperty("offset", seek);
        revAction->setProperty("offset", -1 * seek);
        connect(fwdAction, SIGNAL(triggered()), MPDConnection::self(),
                SLOT(seek()));
        connect(revAction, SIGNAL(triggered()), MPDConnection::self(),
                SLOT(seek()));
        addAction(fwdAction);
        addAction(revAction);
        if (i < seekShortcuts.length()) {
            fwdAction->setShortcut((Qt::RightToLeft == layoutDirection()
                                        ? Qt::Key_Left
                                        : Qt::Key_Right) +
                                   seekShortcuts.at(i));
            revAction->setShortcut((Qt::RightToLeft == layoutDirection()
                                        ? Qt::Key_Right
                                        : Qt::Key_Left) +
                                   seekShortcuts.at(i));
        }
    ...
    addPlayQueueToStoredPlaylistAction = new Action(
        Icons::self()->playlistListIcon, tr("Add To Stored Playlist"), this);
#ifdef ENABLE_DEVICES_SUPPORT
    copyToDeviceAction = new Action(
        StdActions::self()->copyToDeviceAction->icon(),
        Utils::strippedText(StdActions::self()->copyToDeviceAction->text()),
        this);
    copyToDeviceAction->setMenu(
        DevicesModel::self()->menu()->duplicate(nullptr));
#endif
    cropPlayQueueAction = ActionCollection::get()->createAction(
        "cropplaylist", tr("Crop Others"));
    addStreamToPlayQueueAction = ActionCollection::get()->createAction(
        "addstreamtoplayqueue", tr("Add Stream URL"));
    addLocalFilesToPlayQueueAction = ActionCollection::get()->createAction(
        "addlocalfiles", tr("Add Local Files"));
    QIcon clearIcon = MonoIcon::icon(FontAwesome::times, MonoIcon::constRed,
                                     MonoIcon::constRed);
    clearPlayQueueAction = ActionCollection::get()->createAction(
        "clearplaylist", tr("Clear"), clearIcon);
    clearPlayQueueAction->setShortcut(Qt::ControlModifier + Qt::Key_K);
    centerPlayQueueAction = ActionCollection::get()->createAction(
        "centerplaylist", tr("Center On Current Track"),
        Icons::self()->centrePlayQueueOnTrackIcon);
    expandInterfaceAction = ActionCollection::get()->createAction(
        "expandinterface", tr("Expanded Interface"),
        MonoIcon::icon(FontAwesome::expand, iconCol));
    expandInterfaceAction->setCheckable(true);
    songInfoAction = ActionCollection::get()->createAction(
        "showsonginfo", tr("Show Current Song Information"),
        Icons::self()->infoIcon);
    songInfoAction->setShortcut(Qt::Key_F12);
    songInfoAction->setCheckable(true);
    fullScreenAction = ActionCollection::get()->createAction(
        "fullScreen", tr("Full Screen"),
        MonoIcon::icon(FontAwesome::arrowsalt, iconCol));
#ifndef Q_OS_MAC
    fullScreenAction->setShortcut(Qt::Key_F11);
#endif
    randomPlayQueueAction = ActionCollection::get()->createAction(
        "randomplaylist", tr("Random"), Icons::self()->shuffleIcon);
    repeatPlayQueueAction = ActionCollection::get()->createAction(
        "repeatplaylist", tr("Repeat"), Icons::self()->repeatIcon);
    singlePlayQueueAction = ActionCollection::get()->createAction(
        "singleplaylist", tr("Single"), Icons::self()->singleIcon,
        tr("When 'Single' is activated, playback is stopped after current "
           "song, or song is repeated if 'Repeat' is enabled."));
    consumePlayQueueAction = ActionCollection::get()->createAction(
        "consumeplaylist", tr("Consume"), Icons::self()->consumeIcon,
        tr("When consume is activated, a song is removed from the play queue "
           "after it has been played."));
    searchPlayQueueAction = ActionCollection::get()->createAction(
        "searchplaylist", tr("Find in Play Queue"), Icons::self()->searchIcon);
    addAction(searchPlayQueueAction);
    searchPlayQueueAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                       Qt::Key_F);
#ifdef ENABLE_HTTP_STREAM_PLAYBACK
    streamPlayAction = ActionCollection::get()->createAction(
        "streamplay", tr("Play HTTP Output Stream"),
        Icons::self()->httpStreamIcon);
    ...
    locateArtistAction =
        ActionCollection::get()->createAction("locateartist", tr("Artist"));
    locateAlbumAction =
        ActionCollection::get()->createAction("locatealbum", tr("Album"));
    locateTrackAction =
        ActionCollection::get()->createAction("locatetrack", tr("Track"));
    ...
    playNextAction =
        ActionCollection::get()->createAction("playnext", tr("Play Next"));
#ifdef TAGLIB_FOUND
    editPlayQueueTagsAction = ActionCollection::get()->createAction(
        "editpqtags",
        Utils::strippedText(StdActions::self()->editTagsAction->text()),
        StdActions::self()->editTagsAction->icon());
    editPlayQueueTagsAction->setSettingsText(
        tr("Edit Track Information (Play Queue)"));
#endif
    addAction(expandAllAction = ActionCollection::get()->createAction(
                  "expandall", tr("Expand All")));
    expandAllAction->setShortcut(Qt::ControlModifier + Qt::Key_Down);
    addAction(collapseAllAction = ActionCollection::get()->createAction(
                  "collapseall", tr("Collapse All")));
    collapseAllAction->setShortcut(Qt::ControlModifier + Qt::Key_Up);
    cancelAction = ActionCollection::get()->createAction(
        "cancel", tr("Cancel"), Icons::self()->cancelIcon);
    cancelAction->setShortcut(Qt::AltModifier + Qt::Key_Escape);
    connect(cancelAction, SIGNAL(triggered()), messageWidget,
            SLOT(animatedHide()));
    ...
    addPlayQueueToStoredPlaylistAction->setMenu(
        PlaylistsModel::self()->menu()->duplicate(nullptr));

    playPauseTrackButton->setDefaultAction(
        StdActions::self()->playPauseTrackAction);
    ...
    savePlayQueueButton->setDefaultAction(
        StdActions::self()->savePlayQueueAction);
    ...
    bool playQueueInSidebar =
        !hiddenPages.contains(playQueuePage->metaObject()->className());
    bool contextInSidebar =
...
#define TAB_ACTION(A) A->icon(), A->text(), A->text()
    ...
    addAction(
        showPlayQueueAction = ActionCollection::get()->createAction(
            "showplayqueue", tr("Play Queue"), Icons::self()->playqueueIcon));
    showPlayQueueAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                     Qt::Key_Q);
    tabWidget->addTab(playQueuePage, TAB_ACTION(showPlayQueueAction),
                      playQueueInSidebar);
    connect(showPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(showPlayQueue()));
    libraryPage = new LibraryPage(this);
    addAction(libraryTabAction = ActionCollection::get()->createAction(
                  "showlibrarytab", tr("Library"), Icons::self()->libraryIcon));
    libraryTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                  nextKey(sidebarPageShortcutKey));
    tabWidget->addTab(
        libraryPage, TAB_ACTION(libraryTabAction),
        !hiddenPages.contains(libraryPage->metaObject()->className()));
    connect(libraryTabAction, SIGNAL(triggered()), this,
            SLOT(showLibraryTab()));
    folderPage = new FolderPage(this);
    addAction(foldersTabAction = ActionCollection::get()->createAction(
                  "showfolderstab", tr("Folders"), Icons::self()->foldersIcon));
    foldersTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                  nextKey(sidebarPageShortcutKey));
    tabWidget->addTab(
        folderPage, TAB_ACTION(foldersTabAction),
        !hiddenPages.contains(folderPage->metaObject()->className()));
    connect(foldersTabAction, SIGNAL(triggered()), this,
            SLOT(showFoldersTab()));
    folderPage->setEnabled(
        !hiddenPages.contains(folderPage->metaObject()->className()));
    playlistsPage = new PlaylistsPage(this);
    addAction(
        playlistsTabAction = ActionCollection::get()->createAction(
            "showplayliststab", tr("Playlists"), Icons::self()->playlistsIcon));
    playlistsTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                    nextKey(sidebarPageShortcutKey));
    tabWidget->addTab(
        playlistsPage, TAB_ACTION(playlistsTabAction),
        !hiddenPages.contains(playlistsPage->metaObject()->className()));
    connect(playlistsTabAction, SIGNAL(triggered()), this,
            SLOT(showPlaylistsTab()));
    connect(playlistsPage, SIGNAL(error(const QString&)),
            SLOT(showError(const QString&)));
    connect(DynamicPlaylists::self(), SIGNAL(error(const QString&)),
            SLOT(showError(const QString&)));
    connect(DynamicPlaylists::self(), SIGNAL(running(bool)), dynamicLabel,
            SLOT(setVisible(bool)));
    connect(DynamicPlaylists::self(), SIGNAL(running(bool)), this,
            SLOT(controlDynamicButton()));
    stopDynamicButton->setDefaultAction(DynamicPlaylists::self()->stopAct());
    onlinePage = new OnlineServicesPage(this);
    addAction(onlineTabAction = ActionCollection::get()->createAction(
                  "showonlinetab", tr("Internet"), Icons::self()->onlineIcon));
    onlineTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                 nextKey(sidebarPageShortcutKey));
    tabWidget->addTab(
        onlinePage, TAB_ACTION(onlineTabAction),
        !hiddenPages.contains(onlinePage->metaObject()->className()));
    onlinePage->setEnabled(
        !hiddenPages.contains(onlinePage->metaObject()->className()));
    connect(onlineTabAction, SIGNAL(triggered()), this, SLOT(showOnlineTab()));
    //    connect(onlinePage, SIGNAL(addToDevice(const QString &, const QString
    //    &, const QList<Song> &)), SLOT(copyToDevice(const QString &, const
    //    QString &, const QList<Song> &)));
    connect(onlinePage, SIGNAL(error(const QString&)), this,
            SLOT(showError(const QString&)));
#ifdef ENABLE_DEVICES_SUPPORT
    devicesPage = new DevicesPage(this);
    addAction(devicesTabAction = ActionCollection::get()->createAction(
                  "showdevicestab", tr("Devices"), Icons::self()->devicesIcon));
    devicesTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                  nextKey(sidebarPageShortcutKey));
    tabWidget->addTab(
        devicesPage, TAB_ACTION(devicesTabAction),
        !hiddenPages.contains(devicesPage->metaObject()->className()));
    DevicesModel::self()->setEnabled(
        !hiddenPages.contains(devicesPage->metaObject()->className()));
    connect(devicesTabAction, SIGNAL(triggered()), this,
            SLOT(showDevicesTab()));
#endif
    searchPage = new SearchPage(this);
    addAction(searchTabAction = ActionCollection::get()->createAction(
                  "showsearchtab", tr("Search"), Icons::self()->searchTabIcon));
    searchTabAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                                 nextKey(sidebarPageShortcutKey));
    connect(searchTabAction, SIGNAL(triggered()), this, SLOT(showSearchTab()));
    connect(searchPage, SIGNAL(locate(QList<Song>)), this,
            SLOT(locateTracks(QList<Song>)));
    tabWidget->addTab(
        searchPage, TAB_ACTION(searchTabAction),
        !hiddenPages.contains(searchPage->metaObject()->className()));
    tabWidget->addTab(
        contextPage, Icons::self()->infoSidebarIcon, tr("Info"),
        songInfoAction->text(),
        !hiddenPages.contains(contextPage->metaObject()->className()));
    ...
    connect(fullScreenLabel, SIGNAL(leftClickedUrl()), fullScreenAction,
            SIGNAL(triggered()));
    connect(playQueueSearchWidget, SIGNAL(active(bool)), playQueue,
            SLOT(searchActive(bool)));
    if (Configuration(playQueuePage->metaObject()->className())
            .get(ItemView::constSearchActiveKey, false)) {
        playQueueSearchWidget->activate();
    } else {
        playQueueSearchWidget->setVisible(false);
    }
    playQueueSearchWidget->setToolTip(tr(
        "<p>Enter a string to search artist, album, title, etc. To filter "
        "based on year, add <i>#year-range</i> to search string - e.g.</p><ul>"
        "<li><b><i>#2000</i></b> return tracks from 2000</li>"
        "<li><b><i>#1980-1989</i></b> return tracks from the 80's</li>"
        "<li><b><i>Blah #2000</i></b> to search for string <i>Blah</i> and "
        "only return tracks from 2000</li>"
        "</ul></p>"));
    QList<QToolButton*> playbackBtns =
        QList<QToolButton*>() << prevTrackButton << stopTrackButton
                              << playPauseTrackButton << nextTrackButton;
    QList<QToolButton*> controlBtns = QList<QToolButton*>()
                                      << menuButton << songInfoButton;
    int playbackIconSizeNonScaled =
        24 == Icons::self()->toolbarPlayIcon.actualSize(QSize(24, 24)).width()
            ? 24
            : 28;
    ...
    int playbackButtonSize = 28 == playbackIconSizeNonScaled
                                 ? Utils::scaleForDpi(34)
                                 : controlButtonSize;
    ...
    playPauseTrackButton->setIconSize(
        QSize(playPauseIconSize, playPauseIconSize));
    playPauseTrackButton->setFixedSize(
        QSize(playPauseIconSize + 6, playPauseIconSize + 6));

    QList<QWidget*> pqWidgets = QList<QWidget*>()
                                << stopDynamicButton << dynamicLabel
                                << playQueueStatsLabel << fullScreenLabel
                                << repeatButton << singleButton << randomButton
                                << consumeButton << midSpacer
                                << centerPlayQueueButton << savePlayQueueButton
                                << clearPlayQueueButton << sizeGrip;
    ...
    QSize defaultSize(playPauseTrackButton->width() * 25,
                      playPauseTrackButton->height() * 18);

    if (Settings::self()->firstRun() ||
        (expandInterfaceAction->isChecked() && expandedSize.isEmpty())) {
        resize(defaultSize);
        splitter->setSizes(QList<int>() << width * 0.4 << width * 0.6);
    } else {
        if (expandInterfaceAction->isChecked()) {
            if (!expandedSize.isEmpty()) {
                if (!Settings::self()->maximized() &&
                    expandedSize.width() > 1) {
                    resize(expandedSize);
                    expandOrCollapse(false);
                } else {
                    resize(expandedSize.width() > 1 ? expandedSize
                                                    : defaultSize);
                    // Issue #1137 Under Windows, Cantata does not restore
                    // maximized correctly if context is in sidebar
...
#else
        showMenubarAction = ActionCollection::get()->createAction(
            "showmenubar", tr("Show Menubar"));
        showMenubarAction->setShortcut(Qt::ControlModifier + Qt::Key_M);
        showMenubarAction->setCheckable(true);
        connect(showMenubarAction, SIGNAL(toggled(bool)), this,
                SLOT(toggleMenubar()));
#endif
    ...
    for (int i = 0; i < ((Song::Rating_Max / Song::Rating_Step) + 1); ++i) {
        ...
        Action* action = ActionCollection::get()->createAction(
            QLatin1String("rating") + QString::number(i), text);
...
#ifdef ENABLE_DEVICES_SUPPORT
    connect(DevicesModel::self(), SIGNAL(addToDevice(const QString&)), this,
            SLOT(addToDevice(const QString&)));
    connect(DevicesModel::self(), SIGNAL(error(const QString&)), this,
            SLOT(showError(const QString&)));
    connect(
        libraryPage,
        SIGNAL(addToDevice(const QString&, const QString&, const QList<Song>&)),
        SLOT(copyToDevice(const QString&, const QString&, const QList<Song>&)));
    connect(
        folderPage->mpd(),
        SIGNAL(addToDevice(const QString&, const QString&, const QList<Song>&)),
        SLOT(copyToDevice(const QString&, const QString&, const QList<Song>&)));
    ...
    connect(
        ...
        SIGNAL(addToDevice(const QString&, const QString&, const QList<Song>&)),
        SLOT(copyToDevice(const QString&, const QString&, const QList<Song>&)));
    connect(StdActions::self()->deleteSongsAction, SIGNAL(triggered()),
            SLOT(deleteSongs()));
    connect(devicesPage,
            SIGNAL(deleteSongs(const QString&, const QList<Song>&)),
            SLOT(deleteSongs(const QString&, const QList<Song>&)));
    connect(libraryPage,
            SIGNAL(deleteSongs(const QString&, const QList<Song>&)),
            SLOT(deleteSongs(const QString&, const QList<Song>&)));
    connect(folderPage->mpd(),
            SIGNAL(deleteSongs(const QString&, const QList<Song>&)),
            SLOT(deleteSongs(const QString&, const QList<Song>&)));
    connect(searchPage, SIGNAL(deleteSongs(const QString&, const QList<Song>&)),
            SLOT(deleteSongs(const QString&, const QList<Song>&)));
#endif
    for (QAction* act :
         StdActions::self()->setPriorityAction->menu()->actions()) {
        connect(act, SIGNAL(triggered()), this, SLOT(addWithPriority()));
    }
    for (QAction* act :
         StdActions::self()->addWithPriorityAction->menu()->actions()) {
        connect(act, SIGNAL(triggered()), this, SLOT(addWithPriority()));
    }
    connect(StdActions::self()->appendToPlayQueueAndPlayAction,
            SIGNAL(triggered()), this, SLOT(appendToPlayQueueAndPlay()));
    connect(StdActions::self()->addToPlayQueueAndPlayAction,
            SIGNAL(triggered()), this, SLOT(addToPlayQueueAndPlay()));
    connect(StdActions::self()->insertAfterCurrentAction, SIGNAL(triggered()),
            this, SLOT(insertIntoPlayQueue()));
    connect(MPDConnection::self(),
            SIGNAL(partitionsUpdated(const QList<Partition>&)), this,
            SLOT(partitionsUpdated(const QList<Partition>&)));
    connect(MPDConnection::self(), SIGNAL(outputsUpdated(const QList<Output>&)),
            this, SLOT(outputsUpdated(const QList<Output>&)));
    connect(this, SIGNAL(changePartition(QString)), MPDConnection::self(),
            SLOT(changePartition(QString)));
    connect(this, SIGNAL(newPartition(QString)), MPDConnection::self(),
            SLOT(newPartition(QString)));
    connect(this, SIGNAL(delPartition(QString)), MPDConnection::self(),
            SLOT(delPartition(QString)));
    connect(this, SIGNAL(enableOutput(quint32, bool)), MPDConnection::self(),
            SLOT(enableOutput(quint32, bool)));
    connect(this, SIGNAL(moveOutput(QString)), MPDConnection::self(),
            SLOT(moveOutput(QString)));
    connect(this, SIGNAL(outputs()), MPDConnection::self(), SLOT(outputs()));
    connect(this, SIGNAL(pause(bool)), MPDConnection::self(),
            SLOT(setPause(bool)));
    connect(this, SIGNAL(play()), MPDConnection::self(), SLOT(play()));
    connect(this, SIGNAL(stop(bool)), MPDConnection::self(),
            SLOT(stopPlaying(bool)));
    connect(this, SIGNAL(terminating()), MPDConnection::self(), SLOT(stop()));
    connect(this, SIGNAL(getStatus()), MPDConnection::self(),
            SLOT(getStatus()));
    connect(this, SIGNAL(playListInfo()), MPDConnection::self(),
            SLOT(playListInfo()));
    connect(this, SIGNAL(currentSong()), MPDConnection::self(),
            SLOT(currentSong()));
    connect(this, SIGNAL(setSeekId(qint32, quint32)), MPDConnection::self(),
            SLOT(setSeekId(qint32, quint32)));
    connect(this, SIGNAL(startPlayingSongId(qint32)), MPDConnection::self(),
            SLOT(startPlayingSongId(qint32)));
    connect(this, SIGNAL(setDetails(const MPDConnectionDetails&)),
            MPDConnection::self(),
            SLOT(setDetails(const MPDConnectionDetails&)));
    connect(this, SIGNAL(setPriority(const QList<qint32>&, quint8, bool)),
            MPDConnection::self(),
            SLOT(setPriority(const QList<qint32>&, quint8, bool)));
    connect(this,
            SIGNAL(addSongsToPlaylist(const QString&, const QStringList&)),
            MPDConnection::self(),
            SLOT(addToPlaylist(const QString&, const QStringList&)));
    connect(PlayQueueModel::self(), SIGNAL(statsUpdated(int, quint32)), this,
            SLOT(updatePlayQueueStats(int, quint32)));
    connect(PlayQueueModel::self(), SIGNAL(fetchingStreams()), playQueue,
            SLOT(showSpinner()));
    connect(PlayQueueModel::self(), SIGNAL(streamsFetched()), playQueue,
            SLOT(hideSpinner()));
    connect(PlayQueueModel::self(), SIGNAL(updateCurrent(Song)),
            SLOT(updateCurrentSong(Song)));
    connect(PlayQueueModel::self(), SIGNAL(streamFetchStatus(QString)),
            playQueue, SLOT(streamFetchStatus(QString)));
    connect(PlayQueueModel::self(), SIGNAL(error(QString)),
            SLOT(showError(QString)));
    connect(playQueue, SIGNAL(cancelStreamFetch()), PlayQueueModel::self(),
            SLOT(cancelStreamFetch()));
    connect(playQueue, SIGNAL(itemsSelected(bool)),
            SLOT(playQueueItemsSelected(bool)));
    connect(MPDStatus::self(), SIGNAL(updated()), this, SLOT(updateStatus()));
    connect(MPDConnection::self(),
            SIGNAL(playlistUpdated(const QList<Song>&, bool)), this,
            SLOT(updatePlayQueue(const QList<Song>&, bool)));
    connect(MPDConnection::self(), SIGNAL(currentSongUpdated(Song)), this,
            SLOT(updateCurrentSong(Song)));
    connect(MPDConnection::self(), SIGNAL(stateChanged(bool)),
            SLOT(mpdConnectionStateChanged(bool)));
    connect(MPDConnection::self(), SIGNAL(error(const QString&, bool)),
            SLOT(showError(const QString&, bool)));
    connect(MPDConnection::self(), SIGNAL(info(const QString&)),
            SLOT(showInformation(const QString&)));
    connect(MPDConnection::self(), SIGNAL(dirChanged()), SLOT(checkMpdDir()));
    connect(MPDConnection::self(), SIGNAL(connectionNotChanged(QString)),
            SLOT(mpdConnectionName(QString)));
    connect(MpdLibraryModel::self(), SIGNAL(error(QString)),
            SLOT(showError(QString)));
    connect(ApiKeys::self(), SIGNAL(error(const QString&)),
            SLOT(showError(const QString&)));
    connect(refreshDbAction, SIGNAL(triggered()), this, SLOT(refreshDbPromp()));
    connect(doDbRefreshAction, SIGNAL(triggered()), MpdLibraryModel::self(),
            SLOT(clearDb()));
    connect(doDbRefreshAction, SIGNAL(triggered()), MPDConnection::self(),
            SLOT(update()));
    connect(doDbRefreshAction, SIGNAL(triggered()), messageWidget,
            SLOT(animatedHide()));
    connect(connectAction, SIGNAL(triggered()), this, SLOT(connectToMpd()));
    connect(StdActions::self()->prevTrackAction, SIGNAL(triggered()),
            MPDConnection::self(), SLOT(goToPrevious()));
    connect(StdActions::self()->nextTrackAction, SIGNAL(triggered()),
            MPDConnection::self(), SLOT(goToNext()));
    connect(StdActions::self()->playPauseTrackAction, SIGNAL(triggered()), this,
            SLOT(playPauseTrack()));
    connect(StdActions::self()->stopPlaybackAction, SIGNAL(triggered()), this,
            SLOT(stopPlayback()));
    connect(StdActions::self()->stopAfterCurrentTrackAction,
            SIGNAL(triggered()), this, SLOT(stopAfterCurrentTrack()));
    connect(stopAfterTrackAction, SIGNAL(triggered()), this,
            SLOT(stopAfterTrack()));
    connect(this, SIGNAL(setVolume(int)), MPDConnection::self(),
            SLOT(setVolume(int)));
    ...
    connect(PlayQueueModel::self(), SIGNAL(currentSongRating(QString, quint8)),
            nowPlaying, SLOT(rating(QString, quint8)));
    connect(randomPlayQueueAction, SIGNAL(triggered(bool)),
            MPDConnection::self(), SLOT(setRandom(bool)));
    connect(repeatPlayQueueAction, SIGNAL(triggered(bool)),
            MPDConnection::self(), SLOT(setRepeat(bool)));
    connect(singlePlayQueueAction, SIGNAL(triggered(bool)),
            MPDConnection::self(), SLOT(setSingle(bool)));
    connect(consumePlayQueueAction, SIGNAL(triggered(bool)),
            MPDConnection::self(), SLOT(setConsume(bool)));
#ifdef ENABLE_HTTP_STREAM_PLAYBACK
    connect(streamPlayAction, SIGNAL(triggered(bool)), HttpStream::self(),
            SLOT(setEnabled(bool)));
    connect(MPDConnection::self(), SIGNAL(streamUrl(QString)),
            SLOT(streamUrl(QString)));
#endif
    connect(playQueueSearchWidget, SIGNAL(returnPressed()), this,
            SLOT(searchPlayQueue()));
    connect(playQueueSearchWidget, SIGNAL(textChanged(const QString)), this,
            SLOT(searchPlayQueue()));
    connect(playQueueSearchWidget, SIGNAL(active(bool)), this,
            SLOT(playQueueSearchActivated(bool)));
    connect(playQueue, SIGNAL(doubleClicked(const QModelIndex&)), this,
            SLOT(playQueueItemActivated(const QModelIndex&)));
    connect(StdActions::self()->removeAction, SIGNAL(triggered()), this,
            SLOT(removeItems()));
    connect(StdActions::self()->appendToPlayQueueAction, SIGNAL(triggered()),
            this, SLOT(appendToPlayQueue()));
    connect(StdActions::self()->replacePlayQueueAction, SIGNAL(triggered()),
            this, SLOT(replacePlayQueue()));
    connect(playQueue->removeFromAct(), SIGNAL(triggered()), this,
            SLOT(removeFromPlayQueue()));
    connect(clearPlayQueueAction, SIGNAL(triggered()), playQueueSearchWidget,
            SLOT(clear()));
    connect(clearPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(clearPlayQueue()));
    connect(centerPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(centerPlayQueue()));
    connect(cropPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(cropPlayQueue()));
    connect(songInfoAction, SIGNAL(triggered()), this, SLOT(showSongInfo()));
    connect(expandInterfaceAction, SIGNAL(triggered()), this,
            SLOT(expandOrCollapse()));
    connect(fullScreenAction, SIGNAL(triggered()), this, SLOT(fullScreen()));
#ifdef TAGLIB_FOUND
    connect(StdActions::self()->editTagsAction, SIGNAL(triggered()), this,
            SLOT(editTags()));
    connect(editPlayQueueTagsAction, SIGNAL(triggered()), this,
            SLOT(editTags()));
    connect(StdActions::self()->organiseFilesAction, SIGNAL(triggered()),
            SLOT(organiseFiles()));
#endif
    connect(context, SIGNAL(findArtist(QString)), this,
            SLOT(locateArtist(QString)));
    connect(context, SIGNAL(findAlbum(QString, QString)), this,
            SLOT(locateAlbum(QString, QString)));
    connect(context, SIGNAL(playSong(QString)), PlayQueueModel::self(),
            SLOT(playSong(QString)));
    ...
    connect(this, SIGNAL(playNext(QList<quint32>, quint32, quint32)),
            MPDConnection::self(),
            SLOT(move(QList<quint32>, quint32, quint32)));
    connect(playNextAction, SIGNAL(triggered()), this,
            SLOT(moveSelectionAfterCurrentSong()));
    connect(qApp, SIGNAL(paletteChanged(const QPalette&)), this,
            SLOT(paletteChanged()));

    connect(StdActions::self()->searchAction, SIGNAL(triggered()),
            SLOT(showSearch()));
    connect(searchPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(showPlayQueueSearch()));
    connect(playQueue, SIGNAL(focusSearch(QString)), playQueueSearchWidget,
            SLOT(activate(QString)));
    ...
    connect(addStreamToPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(addStreamToPlayQueue()));
    connect(addLocalFilesToPlayQueueAction, SIGNAL(triggered()), this,
            SLOT(addLocalFilesToPlayQueue()));
    connect(splitter, SIGNAL(splitterMoved(int, int)), this,
            SLOT(controlPlayQueueButtons()));
    connect(StdActions::self()->setCoverAction, SIGNAL(triggered()),
            SLOT(setCover()));
#ifdef ENABLE_REPLAYGAIN_SUPPORT
    connect(StdActions::self()->replaygainAction, SIGNAL(triggered()),
            SLOT(replayGain()));
#endif
    connect(PlaylistsModel::self(), SIGNAL(addToNew()), this,
            SLOT(addToNewStoredPlaylist()));
    connect(PlaylistsModel::self(), SIGNAL(addToExisting(const QString&)), this,
            SLOT(addToExistingStoredPlaylist(const QString&)));
    // TODO: Why is this here???
    //    connect(playlistsPage, SIGNAL(add(const QStringList &, bool, quint8)),
    //    PlayQueueModel::self(), SLOT(addItems(const QStringList &, bool,
    //    quint8)));
    connect(coverWidget, SIGNAL(clicked()), expandInterfaceAction,
            SLOT(trigger()));
#if !defined Q_OS_WIN && !defined Q_OS_MAC
    connect(MountPoints::self(), SIGNAL(updated()),
            SLOT(checkMpdAccessibility()));
    connect(qApp, SIGNAL(commitDataRequest(QSessionManager&)),
            SLOT(commitDataRequest(QSessionManager&)));
#endif
    ...
    connect(tabWidget, SIGNAL(styleChanged(int)), this,
            SLOT(sidebarModeChanged()));
    ...
    if (testAttribute(Qt::WA_TranslucentBackground)) {
        // BUG: 146 - Work-around non-showing main window on start-up with
        // transparent QtCurve windows.
...
MainWindow::~MainWindow() {
    ...
    if (!fullScreenAction->isChecked()) {
        if (expandInterfaceAction->isChecked()) {
            ...
            Settings::self()->saveMainWindowSize(isMaximized() ? previousSize
                                                               : size());
        } else {
            Settings::self()->saveMaximized(false);
            Settings::self()->saveMainWindowSize(expandedSize);
        }
        Settings::self()->saveMainWindowCollapsedSize(
            expandInterfaceAction->isChecked() ? collapsedSize : size());
        Settings::self()->saveShowPlaylist(expandInterfaceAction->isChecked());
        Settings::self()->saveMainWindowPos(pos());
    }
#ifdef ENABLE_HTTP_STREAM_PLAYBACK
    Settings::self()->savePlayStream(streamPlayAction->isVisible() &&
                                     streamPlayAction->isChecked());
#endif
    if (!fullScreenAction->isChecked()) {
    ...
    Settings::self()->savePage(
        tabWidget->currentWidget()->metaObject()->className());
    ...
    if (Utils::useSystemTray()) {
        ...
        Settings::self()->saveStartHidden(
            trayItem->isActive() && Settings::self()->minimiseOnClose() &&
    ...
    Configuration(playQueuePage->metaObject()->className())
        .set(ItemView::constSearchActiveKey, playQueueSearchWidget->isActive());
}
...
QStringList MainWindow::listActions() const {
    ...
    for (int i = 0; i < ActionCollection::get()->actions().count(); i++) {
        Action* action =
            qobject_cast<Action*>(ActionCollection::get()->actions().at(i));
        if (!action) {
            continue;
        }
        allActions.append(action->objectName() + " - " +
                          Action::settingsText(action));
    }
    return allActions;
}
...
void MainWindow::addMenuAction(QMenu* menu, QAction* action) {
    ...
    addAction(
        action);  // Bind action to window, so that it works when fullscreen!
}
#endif
...
void MainWindow::mpdConnectionStateChanged(bool connected) {
    serverInfoAction->setEnabled(connected &&
                                 !MPDConnection::self()->isMopidy());
    refreshDbAction->setEnabled(connected);
    addStreamToPlayQueueAction->setEnabled(connected);
    addLocalFilesToPlayQueueAction->setEnabled(
        connected && (HttpServer::self()->isAlive() ||
                      MPDConnection::self()->localFilePlaybackSupported()));
    partitionsAction->setVisible(connected &&
                                 MPDConnection::self()->canUsePartitions());
    ...
    if (connected) {
        ...
        if (CS_Connected != connectedState) {
            ...
            StdActions::self()->addWithPriorityAction->setVisible(
                MPDConnection::self()->canUsePriority());
            StdActions::self()->setPriorityAction->setVisible(
                MPDConnection::self()->canUsePriority());
        }
    } else {
        libraryPage->clear();
...
void MainWindow::playQueueItemsSelected(bool s) {
    ...
    bool singleSelection =
        1 == playQueue->selectedIndexes(false)
                 .count();  // Dont need sorted selection here...
    ...
    cropPlayQueueAction->setEnabled(playQueue->haveUnSelectedItems() &&
                                    haveItems);
#ifdef TAGLIB_FOUND
    editPlayQueueTagsAction->setEnabled(
        s && haveItems && MPDConnection::self()->getDetails().dirReadable);
#endif
...
void MainWindow::connectToMpd(const MPDConnectionDetails& details) {
    if (!MPDConnection::self()->isConnected() ||
        details != MPDConnection::self()->getDetails()) {
        libraryPage->clear();
        folderPage->mpd()->clear();
        PlaylistsModel::self()->clear();
        PlayQueueModel::self()->clear();
        searchPage->clear();
        if (!MPDConnection::self()->getDetails().isEmpty() &&
            details != MPDConnection::self()->getDetails()) {
            DynamicPlaylists::self()->stop();
        }
...
void MainWindow::streamUrl(const QString& u) {
    ...
    streamPlayAction->setChecked(streamPlayAction->isVisible() &&
                                 Settings::self()->playStream());
...
void MainWindow::refreshDbPromp() {
    ...
    if (QDialogButtonBox::GnomeLayout == btnLayout ||
        QDialogButtonBox::MacLayout == btnLayout) {
        messageWidget->setActions(QList<QAction*>()
                                  << cancelAction << doDbRefreshAction);
    } else {
        messageWidget->setActions(QList<QAction*>()
                                  << doDbRefreshAction << cancelAction);
    }
...
void MainWindow::showAboutDialog() {
    QMessageBox::about(
        this, tr("About Cantata"),
        tr("<b>Cantata %1</b><br/><br/>MPD client.<br/><br/>"
           "&copy; 2011-2022 Craig Drummond<br/>Released under the <a "
           "href=\"http://www.gnu.org/licenses/gpl.html\">GPLv3</a>")
                .arg(PACKAGE_VERSION_STRING) +
            QLatin1String("<br/><br/>") +
            tr("Please refer to <a "
               "href=\"https://github.com/CDrummond/cantata/issues\">Cantata's "
               "issue tracker</a> for a list of known issues, and to report "
               "new issues.") +
            QLatin1String("<br/><br/><i><small>") +
            tr("Based upon <a href=\"http://lowblog.nl\">QtMPC</a> - &copy; "
               "2007-2010 The QtMPC Authors<br/>") +
            tr("Context view backdrops courtesy of <a "
               "href=\"http://www.fanart.tv\">FanArt.tv</a>") +
            QLatin1String("<br/>") +
            tr("Context view metadata courtesy of <a "
               "href=\"http://www.wikipedia.org\">Wikipedia</a> and <a "
               "href=\"http://www.last.fm\">Last.fm</a>") +
            QLatin1String("<br/><br/>") +
            tr("Please consider uploading your own music fan-art to <a "
               "href=\"http://www.fanart.tv\">FanArt.tv</a>") +
    ...
    if (onlinePage->isDownloading() &&
        MessageBox::No ==
            MessageBox::warningYesNo(
                this,
                tr("A Podcast is currently being downloaded\n\nQuitting now "
                   "will abort the download."),
                QString(), GuiItem(tr("Abort download and quit")),
                GuiItem("Do not quit just yet"))) {
        return false;
    }
...
void MainWindow::showPreferencesDialog(const QString& page) {
    ...
    connect(this, SIGNAL(showPreferencesPage(QString)), pref,
            SLOT(showPage(QString)));
    if (!page.isEmpty()) {
        pref->showPage(page);
    }
    pref->show();
...
void MainWindow::commitDataRequest(QSessionManager& mgr) {
    Q_UNUSED(mgr)
#if !defined Q_OS_WIN && !defined Q_OS_MAC
    if (isVisible() && trayItem->isActive() &&
        Settings::self()->minimiseOnClose()) {
        // Issue 1183 If We are using a tray item, and have window open - then
        // cantata intercepts close events and hides the window. This seems to
        // prevent the logout dialog appwaring under GNOME Shell. To work-around
        // this if qApp emits commitDataRequest and our window is open, them
        // close.
        QMainWindow::close();
    }
#endif
...
void MainWindow::partitionsUpdated(const QList<Partition>& partitions) {
    ...
    for (const Partition& p : sortedPartitions) {
        QAction* act = menu->addAction(Utils::escapeActionText(p.name), this,
                                       SLOT(selectPartition()));
        act->setData(p.name);
    ...
    menu->addSeparator();
    menu->addAction(tr("Create new partition"), this,
                    SLOT(createNewPartition()));
    QMenu* deleteMenu = menu->addMenu(tr("Remove partition"));
    deleteMenu->menuAction()->setData("deletepartition");
    for (const Partition& p : sortedPartitions) {
        QAction* act = deleteMenu->addAction(Utils::escapeActionText(p.name),
                                             this, SLOT(deleteAPartition()));
        act->setData(p.name);
...
void MainWindow::outputsUpdated(const QList<Output>& outputs) {
    ...
    QSet<QString> lastEnabledMpd =
        Utils::listToSet(property(constMpdEnabledOuptuts).toStringList());
    ...
    if (menuItems != inCurrentPartitionMpd ||
        menuMoveableItems != inOtherPartitionMpd) {
        menu->clear();
        ...
        for (const Output& o : out) {
            ...
            QAction* act = menu->addAction(Utils::escapeActionText(o.name),
                                           this, SLOT(toggleOutput()));
            act->setData(o.id);
            act->setCheckable(true);
            act->setChecked(o.enabled);
            act->setShortcut(Qt::ControlModifier + Qt::AltModifier +
                             nextKey(i));
        }
        menu->addSeparator();
        ...
        for (const Output& o : out) {
            ...
            QAction* act =
                moveMenu->addAction(Utils::escapeActionText(o.name), this,
                                    SLOT(moveOutputToThisPartition()));
            act->setData(o.name);
        }
    } else {
        for (const Output& o : outputs) {
            ...
            for (QAction* act : menu->actions()) {
                if (act->isCheckable() &&
                    Utils::strippedText(act->text()) == o.name) {
                    act->setChecked(o.enabled);
                    break;
                }
            }
    ...
    if (newConn == lastConn && newPart == lastPart &&
        enabledMpd != lastEnabledMpd && !menuItems.isEmpty()) {
        ...
        if (!switchedOn.isEmpty() && switchedOff.isEmpty()) {
            ...
            trayItem->showMessage(
                tr("Outputs"),
                tr("Enabled: %1").arg(names.join(QLatin1String(", "))));
        } else if (!switchedOff.isEmpty() && switchedOn.isEmpty()) {
            QStringList names = switchedOff.values();
            std::sort(names.begin(), names.end());
            trayItem->showMessage(
                tr("Outputs"),
                tr("Disabled: %1").arg(names.join(QLatin1String(", "))));
...
void MainWindow::updateConnectionsMenu() {
    ...
    if (connections.count() < 2) {
        ...
        if (menuItems != cfg) {
            ...
            for (const MPDConnectionDetails& d : connections) {
                QAction* act =
                    menu->addAction(Utils::escapeActionText(d.getName()), this,
...
void MainWindow::setRating() {
    ...
    if (act) {
        PlayQueueModel::self()->setRating(
            playQueueProxyModel.mapToSourceRows(playQueue->selectedIndexes()),
            act->property(constRatingKey).toUInt());
    }
}
...
void MainWindow::initMpris() {
    ...
    if (Settings::self()->mpris()) {
        if (!mpris) {
            ...
            connect(mpris, SIGNAL(showMainWindow()), this,
                    SLOT(restoreWindow()));
        }
    } else if (mpris) {
        disconnect(mpris, SIGNAL(showMainWindow()), this,
                   SLOT(restoreWindow()));
        mpris->deleteLater();
        mpris = nullptr;
    }
    CurrentCover::self()->setEnabled(
        mpris || Settings::self()->showPopups() ||
...
void MainWindow::paletteChanged() {
    ...
    Icons::self()->initToolbarIcons(nowPlaying->textColor());
    StdActions::self()->prevTrackAction->setIcon(
        Icons::self()->toolbarPrevIcon);
    StdActions::self()->nextTrackAction->setIcon(
        Icons::self()->toolbarNextIcon);
    StdActions::self()->playPauseTrackAction->setIcon(
        MPDState_Playing == MPDStatus::self()->state()
            ? Icons::self()->toolbarPauseIcon
            : Icons::self()->toolbarPlayIcon);
    StdActions::self()->stopPlaybackAction->setIcon(
        Icons::self()->toolbarStopIcon);
    StdActions::self()->stopAfterCurrentTrackAction->setIcon(
        Icons::self()->toolbarStopIcon);
    StdActions::self()->stopAfterTrackAction->setIcon(
        Icons::self()->toolbarStopIcon);
...
void MainWindow::readSettings() {
#ifdef QT_QTDBUS_FOUND
    // It appears as if the KDE MPRIS code does not like the MPRIS interface to
    // be setup before the window is visible. to work-around this, initMpris in
    // the next event loop iteration. See #863
...
#ifdef ENABLE_DEVICES_SUPPORT
    StdActions::self()->deleteSongsAction->setVisible(
        Settings::self()->showDeleteAction());
#endif
    Song::setComposerGenres(Settings::self()->composerGenres());
    trayItem->setup();
    ...
    (defined Q_OS_MAC && defined IOKIT_FOUND)
    PowerManagement::self()->setInhibitSuspend(
        Settings::self()->inhibitSuspend());
#endif
    context->readConfig();
    tabWidget->setProperty(constUserSettingProp,
                           Settings::self()->hiddenPages());
    tabWidget->setProperty(constUserSetting2Prop, Settings::self()->sidebar());
    coverWidget->setProperty(constUserSettingProp,
                             Settings::self()->showCoverWidget());
    stopTrackButton->setProperty(constUserSettingProp,
                                 Settings::self()->showStopButton());
    ...
    MPDConnection::self()->setVolumeFadeDuration(
        Settings::self()->stopFadeDuration());
    MPDParseUtils::setSingleTracksFolders(
        Settings::self()->singleTracksFolders());
...
void MainWindow::updateSettings() {
    ...
    if (wasGrouped != playQueue->isGrouped() ||
        (playQueue->isGrouped() &&
         (wasAutoExpand != playQueue->isAutoExpand() ||
          wasStartClosed != playQueue->isStartClosed()))) {
        QModelIndex idx =
            playQueueProxyModel.mapFromSource(PlayQueueModel::self()->index(
                PlayQueueModel::self()->currentSongRow(), 0));
        playQueue->updateRows(
            idx.row(), current.key,
            autoScrollPlayQueue && playQueueProxyModel.isEmpty() &&
                MPDState_Playing == MPDStatus::self()->state());
    }
...
void MainWindow::createNewPartition() {
    QString name = InputDialog::getText(tr("Partition Name"),
                                        tr("Enter a name for the partition:"),
        ...
        if (MessageBox::Yes ==
            MessageBox::warningYesNo(
                ...
                tr("Are you sure you wish to remove partition \"%1\"?\n\nThis "
                   "cannot be undone.")
                    .arg(name),
                tr("Remove Partition"), StdGuiItem::remove(),
                StdGuiItem::cancel())) {
    ...
    MessageBox::information(
        ...
        tr("Server Information") + QLatin1String("<br/><br/>") +
            ...
            tr("<tr><td colspan=\"2\"><b>Server</b></td></tr>"
               "<tr><td "
               "align=\"right\">Protocol:&nbsp;</td><td>%1.%2.%3</td></tr>"
...
void MainWindow::stopAfterTrack() {
    QModelIndexList selected = playQueue->selectedIndexes(
        false);  // Dont need sorted selection here...
    if (1 == selected.count()) {
        QModelIndex idx = playQueueProxyModel.mapToSource(selected.first());
        PlayQueueModel::self()->setStopAfterTrack(
            PlayQueueModel::self()->getIdByRow(idx.row()));
    }
}
    ...
    switch (MPDStatus::self()->state()) {
        ...
        default:
            if (PlayQueueModel::self()->rowCount() > 0) {
                if (-1 != PlayQueueModel::self()->currentSong() &&
                    -1 != PlayQueueModel::self()->currentSongRow()) {
                    emit startPlayingSongId(
                        PlayQueueModel::self()->currentSong());
                } else {
                    emit play();
                }
            }
...
void MainWindow::searchPlayQueue() {
    if (playQueueSearchWidget->text().isEmpty()) {
        ...
        if (!playQueueSearchTimer) {
            ...
            connect(playQueueSearchTimer, SIGNAL(timeout()),
                    SLOT(realSearchPlayQueue()));
        }
...
void MainWindow::updatePlayQueue(const QList<Song>& songs, bool isComplete) {
    ...
    StdActions::self()->nextTrackAction->setEnabled(
        StdActions::self()->stopPlaybackAction->isEnabled() &&
        !songs.isEmpty() && MPDStatus::self()->nextSongId() != -1);
    StdActions::self()->prevTrackAction->setEnabled(
        StdActions::self()->stopPlaybackAction->isEnabled() &&
        !songs.isEmpty() && (songs.count() > 1 || current.time > 5));
    StdActions::self()->savePlayQueueAction->setEnabled(!songs.isEmpty());
    clearPlayQueueAction->setEnabled(!songs.isEmpty());

    int topRow = -1;
    QModelIndex topIndex = PlayQueueModel::self()->lastCommandWasUnodOrRedo()
                               ? playQueue->indexAt(QPoint(0, 0))
                               : QModelIndex();
    if (topIndex.isValid()) {
    ...
    if (songs.isEmpty()) {
        ...
        Song pqSong = PlayQueueModel::self()->getSongByRow(
            PlayQueueModel::self()->currentSongRow());
    ...
    QModelIndex idx =
        playQueueProxyModel.mapFromSource(PlayQueueModel::self()->index(
            PlayQueueModel::self()->currentSongRow(), 0));
    bool scroll = songChanged && autoScrollPlayQueue &&
                  playQueueProxyModel.isEmpty() &&
                  (wasEmpty || MPDState_Playing == MPDStatus::self()->state());
    playQueue->updateRows(idx.row(), current.key, scroll, wasEmpty);
    if (!scroll && topRow > 0 && topRow < PlayQueueModel::self()->rowCount()) {
        playQueue->scrollTo(
            playQueueProxyModel.mapFromSource(PlayQueueModel::self()->index(
                topRow, PlayQueueModel::COL_TITLE)),
...
void MainWindow::updateWindowTitle() {
    ...
    setWindowTitle((multipleConnections && !partition.isEmpty())
                   ...
                   : multipleConnections  ? tr("Cantata (%1)").arg(connection)
                   : !partition.isEmpty() ? tr("Cantata [%1]").arg(partition)
                                          : "Cantata");
}
...
void MainWindow::updateCurrentSong(Song song, bool wasEmpty) {
    ...
    if (current.time < 5 && MPDStatus::self()->songId() == current.id &&
        MPDStatus::self()->timeTotal() > 5) {
        current.time = MPDStatus::self()->timeTotal();
    }
    nowPlaying->setEnabled(-1 != current.id && !current.isCdda() &&
                           (!currentIsStream() || current.time > 5));
    ...
    playQueue->updateRows(
        idx.row(), current.key,
        autoScrollPlayQueue && playQueueProxyModel.isEmpty() && isPlaying,
        wasEmpty);
...
void MainWindow::scrollPlayQueue(bool wasEmpty) {
    if (autoScrollPlayQueue &&
        (wasEmpty || MPDState_Playing == MPDStatus::self()->state()) &&
        !playQueue->isGrouped()) {
        qint32 row = PlayQueueModel::self()->currentSongRow();
        if (row >= 0) {
            playQueue->scrollTo(playQueueProxyModel.mapFromSource(
                                    PlayQueueModel::self()->index(row, 0)),
                                QAbstractItemView::PositionAtCenter);
        }
    }
}

void MainWindow::updateStatus() { updateStatus(MPDStatus::self()); }

void MainWindow::updateStatus(MPDStatus* const status) {
    if (!status->error().isEmpty()) {
        showError(
            tr("MPD reported the following error: %1").arg(status->error()));
    }
            ...
            if (act->isCheckable()) {
                if (act->data().toString() == status->partition()) {
                    act->setChecked(true);
                       ...
                       act->data().toString() == "deletepartition") {
                for (QAction* subAct : act->menu()->actions()) {
                    subAct->setVisible(subAct->data().toString() !=
                                       status->partition());
                }
            }
        }

        // SongIDs are not unique between partitions, so reset the play queue's
        // state.
        PlayQueueModel::self()->clear();
    }

    if (MPDState_Stopped == status->state() ||
        MPDState_Inactive == status->state()) {
        ...
        nowPlaying->setRange(
            0, 0 == status->timeTotal() && 0 != current.time &&
                       (current.isCdda() || current.isCantataStream())
                   ? current.time
                   : status->timeTotal());
        ...
        if (0 == status->timeTotal() && 0 == status->timeElapsed()) {
            ...
            if (!id.isValid() || id.toInt() != current.id) {
                ...
                statusTimer->setProperty(
                    "count", statusTimer->property("count").toInt() + 1);
                statusTimer->start(250);
            }
        } else if (!nowPlaying->isEnabled()) {
            nowPlaying->setEnabled(
                -1 != current.id && !current.isCdda() &&
                (!currentIsStream() || status->timeTotal() > 5));
        }
    }
    ...
    if (status->timeElapsed() < 64800 &&
        (!currentIsStream() ||
         (status->timeTotal() > 0 &&
          status->timeElapsed() <= status->timeTotal()))) {
        if (status->state() == MPDState_Stopped ||
            status->state() == MPDState_Inactive) {
    ...
    StdActions::self()->playPauseTrackAction->setEnabled(
    ...
    switch (status->state()) {
        case MPDState_Playing:
            ...
            StdActions::self()->nextTrackAction->setEnabled(
                status->nextSongId() != -1);
            StdActions::self()->prevTrackAction->setEnabled(
                status->playlistLength() > 1 || status->timeTotal() > 5 ||
                current.time > 5);
        ...
        case MPDState_Stopped:
            ...
            current.id = 0;
            trayItem->setToolTip("cantata", tr("Cantata"),
                                 QLatin1String("<i>") + tr("Playback stopped") +
                                     QLatin1String("</i>"));
    ...
    // Check if song has changed or we're playing again after being stopped, and
    // update song info if needed
    if (MPDState_Inactive != status->state() &&
        (MPDState_Inactive == lastState ||
         (MPDState_Stopped == lastState &&
          MPDState_Playing == status->state()) ||
         lastSongId != status->songId())) {
        emit currentSong();
    }
    if (status->state() != lastState && (MPDState_Playing == status->state() ||
                                         MPDState_Stopped == status->state())) {
        startContextTimer();
    }
...
void MainWindow::playQueueItemActivated(const QModelIndex& index) {
    if (index.isValid()) {
        emit startPlayingSongId(PlayQueueModel::self()->getIdByRow(
            playQueueProxyModel.mapToSource(index).row()));
    }
}

void MainWindow::clearPlayQueue() {
    if (!Settings::self()->playQueueConfirmClear() ||
        MessageBox::Yes == MessageBox::questionYesNo(
                               this, tr("Remove all songs from play queue?"))) {
...
void MainWindow::addWithPriority() {
    ...
    if (-1 == prio) {
        InputDialog dlg(tr("Priority"), tr("Enter priority (0..255):"), 150, 0,
                        255, 5, this);
        QCheckBox* dec = new QCheckBox(
            tr("Decrease priority for each subsequent track"), this);
    ...
    if (prio >= 0 && prio <= 255) {
        if (isPlayQueue) {
            ...
            for (const QModelIndex& idx : pqItems) {
                ids.append(PlayQueueModel::self()->getIdByRow(
                    playQueueProxyModel.mapToSource(idx).row()));
            }
...
void MainWindow::addToNewStoredPlaylist() {
    ...
    for (;;) {
        QString name = InputDialog::getText(
            tr("Playlist Name"), tr("Enter a name for the playlist:"),
            QString(), nullptr, this);
        if (name.isEmpty()) {
            return;
        }
        ...
        if (name == MPDConnection::constStreamsPlayListName) {
            MessageBox::error(this, tr("'%1' is used to store favorite "
                                       "streams, please choose another name.")
                                        .arg(name));
            continue;
        }
        if (PlaylistsModel::self()->exists(name)) {
            switch (MessageBox::warningYesNoCancel(
                this,
                tr("A playlist named '%1' already exists!\n\nAdd to that "
                   "playlist?")
                    .arg(name),
    ...
    if (pq) {
        ...
        if (items.isEmpty()) {
            ...
            for (const QModelIndex& idx : items) {
                Song s = PlayQueueModel::self()->getSongByRow(
                    playQueueProxyModel.mapToSource(idx).row());
                if (!s.file.isEmpty()) {
                    files.append(s.file);
                }
...
void MainWindow::addStreamToPlayQueue() {
    ...
    if (QDialog::Accepted == dlg.exec()) {
        ...
        PlayQueueModel::self()->addItems(
            QStringList() << StreamsModel::modifyUrl(url, true, dlg.name()),
            MPDConnection::Append, 0, false);
    }
}
...
void MainWindow::addLocalFilesToPlayQueue() {
    ...
    QStringList files = QFileDialog::getOpenFileNames(
        this, tr("Select Music Files"), QString(),
        tr("Music Files ") + "(" + extensions + ")");
    if (!files.isEmpty()) {
...
void MainWindow::showSongInfo() {
    if (songInfoAction->isCheckable()) {
        stack->setCurrentWidget(songInfoAction->isChecked()
                                    ? (QWidget*)context
                                    : (QWidget*)splitter);
    } else {
    ...
    switch (index) {
        case PAGE_PLAYQUEUE:
            if (tabWidget->isEnabled(index)) {
                splitter->setAutohidable(
                    0, Settings::self()->splitterAutoHide() &&
        ...
        case PAGE_DEVICES:
            DevicesModel::self()->setEnabled(
                !DevicesModel::self()->isEnabled());
            StdActions::self()->copyToDeviceAction->setVisible(
                DevicesModel::self()->isEnabled());
            break;
#endif
        default:
...
void MainWindow::moveSelectionAfterCurrentSong() {
    ...
    if (!selectedSongIds.empty()) {
        emit playNext(selectedSongIds, currentSongIdx + 1,
                      PlayQueueModel::self()->rowCount());
    }
}
...
void MainWindow::showPlayQueueSearch() { playQueueSearchWidget->activate(); }
...
void MainWindow::showSearch() {
    ...
    } else if (!searchPlayQueueAction->isEnabled() &&
               (playQueuePage->isVisible() || playQueue->isVisible())) {
        playQueueSearchWidget->activate();
    }
}
...
void MainWindow::editTags() {
    ...
    if (!isPlayQueue && currentPage == devicesPage) {
        DevicesModel::self()->getDetails(artists, albumArtists, composers,
                                         albums, genres);
    ...
    MpdLibraryModel::self()->getDetails(artists, albumArtists, composers,
                                        albums, genres);
    TagEditor* dlg = new TagEditor(this, songs, artists, albumArtists,
                                   composers, albums, genres, udi);
    dlg->show();
#endif
}
...
void MainWindow::copyToDevice(const QString& from, const QString& to,
...
void MainWindow::updateNextTrack(int nextTrackId) {
    ...
    QString tt =
        StdActions::self()->nextTrackAction->property("tooltip").toString();
    if (-1 == nextTrackId && tt.isEmpty()) {
        StdActions::self()->nextTrackAction->setProperty(
            "tooltip", StdActions::self()->nextTrackAction->toolTip());
    } else if (-1 == nextTrackId) {
        StdActions::self()->nextTrackAction->setToolTip(tt);
        StdActions::self()->nextTrackAction->setProperty("trackid",
                                                         nextTrackId);
    } else if (nextTrackId != StdActions::self()
                                  ->nextTrackAction->property("trackid")
                                  .toInt()) {
        Song s = PlayQueueModel::self()->getSongByRow(
            PlayQueueModel::self()->getRowById(nextTrackId));
        if (!s.artist.isEmpty() && !s.title.isEmpty()) {
            tt += QLatin1String("<br/><i><small>") + s.artistSong() +
                  QLatin1String("</small></i>");
        } else {
            nextTrackId = -1;
        }
...
int MainWindow::calcMinHeight() {
    return tabWidget->style() & FancyTabWidget::Side &&
                   tabWidget->style() & FancyTabWidget::Large
               ? calcCollapsedSize() +
                     (tabWidget->visibleCount() * tabWidget->tabSize().height())
...
int MainWindow::calcCollapsedSize() {
    ...
    return toolbar->height() +
           (showMenubarAction && menuBar() && menuBar()->isVisible()
                ? menuBar()->height()
                : 0);
#else
    return toolbar->height();
...
void MainWindow::controlView(bool forceUpdate) {
    ...
    if (forceUpdate || -1 == lastWidth || qAbs(lastWidth - width()) > 20) {
        bool stopEnabled =
            stopTrackButton->property(constUserSettingProp).toBool();
        bool coverWidgetEnabled =
            coverWidget->property(constUserSettingProp).toBool();
        int tabWidgetStyle = tabWidget->property(constUserSetting2Prop).toInt();
        QStringList tabWidgetPages =
            tabWidget->property(constUserSettingProp).toStringList();

        if ((!stopEnabled && !coverWidgetEnabled) ||
            width() < Utils::scaleForDpi(450)) {
        ...
        if (expandInterfaceAction->isChecked() &&
            ...
            if (!responsiveSidebar || width() > Utils::scaleForDpi(450)) {
                ...
                if (forceUpdate || !singlePane) {
                    int index = tabWidget->currentIndex();
                    int smallStyle = FancyTabWidget::Small |
                                     FancyTabWidget::Top |
                                     FancyTabWidget::IconOnly;
                    ...
                    if (forceUpdate && !isVisible() &&
                        PAGE_PLAYQUEUE != index && PAGE_CONTEXT != index) {
                        QString page = Settings::self()->page();
                        for (int i = 0; i < tabWidget->count(); ++i) {
                            if (tabWidget->widget(i)
                                    ->metaObject()
                                    ->className() == page) {
                                index = i;
                                break;
...
void MainWindow::controlPlayQueueButtons() {
    ...
    savePlayQueueButton->setVisible(singlePane || playQueueWidget->width() >
                                                      Utils::scaleForDpi(320));
    centerPlayQueueButton->setVisible(
        singlePane ||
        playQueueWidget->width() >
            (Utils::scaleForDpi(320) + centerPlayQueueButton->width()));
    midSpacer->setVisible(singlePane || centerPlayQueueButton->isVisible());
}

void MainWindow::expandOrCollapse(bool saveCurrentSize) {
    if (!expandInterfaceAction->isChecked() &&
        (isFullScreen() || isMaximized() || messageWidget->isVisible())) {
        expandInterfaceAction->setChecked(true);
        return;
    }
    ...
    if (showing) {
        ...
        // Width also sometimes expands, so make sure this is no larger than it
        // was before...
        collapsedSize = QSize(collapsedSize.isValid()
                                  ? qMin(collapsedSize.width(), prevWidth)
                                  : prevWidth,
                              calcCollapsedSize());
        resize(collapsedSize);
...
void MainWindow::toggleContext() {
    if (songInfoButton->isVisible()) {
        if ((MPDState_Playing == MPDStatus::self()->state() &&
             !songInfoAction->isChecked()) ||
            (MPDState_Stopped == MPDStatus::self()->state() &&
             songInfoAction->isChecked())) {
            songInfoAction->trigger();
        }
    } else if (MPDState_Playing == MPDStatus::self()->state() &&
               PAGE_CONTEXT != tabWidget->currentIndex()) {
        int pp = prevPage;
        showTab(PAGE_CONTEXT);
        prevPage = pp;
    } else if (MPDState_Stopped == MPDStatus::self()->state() &&
               PAGE_CONTEXT == tabWidget->currentIndex() && -1 != prevPage) {
        showTab(prevPage);
    }
}
