...
#endif
// bool QxtGlobalShortcutPrivate::macEventFilter(EventHandlerCallRef caller,
// EventRef event)
{
...
quint32 QxtGlobalShortcutPrivate::nativeModifiers(
    ...
    quint32 native = 0;
    if (modifiers & Qt::ShiftModifier) native |= shiftKeyBit;
...
quint32 QxtGlobalShortcutPrivate::nativeKeycode(Qt::Key key) {
    ...
    if (key == Qt::Key_Up)
        ...
        ch = 0xF730;
    else if (key == Qt::Key_SysReq)
        ch = 0xF731;
    else if (key == Qt::Key_Stop)
        ch = 0xF734;
    else if (key == Qt::Key_Menu)
        ch = 0xF735;
    else if (key == Qt::Key_Select)
        ch = 0xF741;
    else if (key == Qt::Key_Execute)
        ch = 0xF742;
    else if (key == Qt::Key_Help)
        ch = 0xF746;
    else if (key == Qt::Key_Mode_switch)
        ch = 0xF747;
    ...
    KLGetKeyboardLayoutProperty(
        layout, kKLKind,
        const_cast<const void**>(reinterpret_cast<void**>(&layoutKind)));
    ...
    if (layoutKind == kKLKCHRKind) {  // no Unicode available
        ...
        KLGetKeyboardLayoutProperty(
            layout, kKLKCHRData,
            const_cast<const void**>(reinterpret_cast<void**>(&data)));
...
bool QxtGlobalShortcutPrivate::registerShortcut(quint32 nativeKey,
    ...
    if (!qxt_mac_handler_installed) {
        ...
        InstallApplicationEventHandler(&qxt_mac_handle_hot_key, 1, &t, NULL,
                                       NULL);
    }
