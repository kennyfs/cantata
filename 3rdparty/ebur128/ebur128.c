...
static interpolator* interp_create(unsigned int taps, unsigned int factor,
    ...
    for (j = 0; j < interp->factor; j++) {
        ...
        interp->filter[j].coeff =
            (double*)calloc(interp->delay, sizeof(double));
...
static int ebur128_init_resampler(ebur128_state* st) {
    ...
    CHECK_ERROR(!st->d->resampler_buffer_input, EBUR128_ERROR_NOMEM,
                free_interp)
    ...
    CHECK_ERROR(!st->d->resampler_buffer_output, EBUR128_ERROR_NOMEM,
                free_input)
...
#define EBUR128_FILTER(type, min_scale, max_scale)                            \
    static void ebur128_filter_##type(ebur128_state* st, const type* src,     \
        ...
        TURN_ON_FTZ                                                           \
                                                                              \
        if ((st->mode & EBUR128_MODE_SAMPLE_PEAK) ==                          \
            ...
            for (i = 0; i < frames; ++i) {                                    \
                for (c = 0; c < st->channels; ++c) {                          \
                    st->d->resampler_buffer_input[i * st->channels + c] =     \
                        (float)((double)src[i * st->channels + c] /           \
                                scaling_factor);                              \
                }                                                             \
            }                                                                 \
        ...
        for (c = 0; c < st->channels; ++c) {                                  \
            ...
            for (i = 0; i < frames; ++i) {                                    \
                st->d->v[c][0] = (double)((double)src[i * st->channels + c] / \
                                          scaling_factor) -                   \
                                 st->d->a[1] * st->d->v[c][1] - /**/          \
                                 st->d->a[2] * st->d->v[c][2] - /**/          \
                                 st->d->a[3] * st->d->v[c][3] - /**/          \
    ...
    int ebur128_add_frames_##type(ebur128_state* st, const type* src,         \
        ...
        while (frames > 0) {                                                  \
            if (frames >= st->d->needed_frames) {                             \
                ebur128_filter_##type(st, src + src_index,                    \
                                      st->d->needed_frames);                  \
                src_index += st->d->needed_frames * st->channels;             \
                frames -= st->d->needed_frames;                               \
                st->d->audio_data_index +=                                    \
                    st->d->needed_frames * st->channels;                      \
                /* calculate the new gating block */                          \
                if ((st->mode & EBUR128_MODE_I) == EBUR128_MODE_I) {          \
                    if (ebur128_calc_gating_block(                            \
                            st, st->d->samples_in_100ms * 4, NULL)) {         \
                        return EBUR128_ERROR_NOMEM;                           \
                    }                                                         \
                }                                                             \
                if ((st->mode & EBUR128_MODE_LRA) == EBUR128_MODE_LRA) {      \
                    ...
                    if (st->d->short_term_frame_counter ==                    \
                        ...
                        struct ebur128_dq_entry* block;                       \
                        double st_energy;                                     \
                        if (ebur128_energy_shortterm(st, &st_energy) ==       \
                                EBUR128_SUCCESS &&                            \
                            ...
                            if (st->d->use_histogram) {                       \
                                ...
                                if (st->d->st_block_list_size ==              \
                                    st->d->st_block_list_max) {               \
                                    block = STAILQ_FIRST(                     \
                                        &st->d->short_term_block_list);       \
                                    STAILQ_REMOVE_HEAD(                       \
                                        &st->d->short_term_block_list,        \
                                        entries);                             \
                                } else {                                      \
                                ...
                                block->z = st_energy;                         \
                                STAILQ_INSERT_TAIL(                           \
                                    &st->d->short_term_block_list, block,     \
                                    entries);                                 \
                            }                                                 \
                        }                                                     \
                        st->d->short_term_frame_counter =                     \
                            st->d->samples_in_100ms * 20;                     \
                    }                                                         \
                }                                                             \
        ...
        if (sts[i]) {
            ...
            if (i == 0 && sts[i]->mode & EBUR128_MODE_HISTOGRAM) {
                use_histogram = 1;
                       ...
                       !!(sts[i]->mode & EBUR128_MODE_HISTOGRAM)) {
                return EBUR128_ERROR_INVALID_MODE;
            }
        }
    }
