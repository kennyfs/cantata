...
namespace Ifaces {
/**
 * This class specifies the interface a device will have to comply to in order
 * to be used in the system.
 *
 * Backends will have to implement it to gather and modify data in the
 * underlying system. Each device has a set of key/values pair describing its
 * properties. It has also a list of interfaces describing what the device
 * actually is (a cdrom drive, a portable media player, etc.)
 *
...
class Device : public QObject {
   ...
   public:
     ...
     * @returns true if the device interface is provided by this device, false
     * otherwise
     */
    virtual bool queryDeviceInterface(
        const Solid::DeviceInterface::Type& type) const = 0;

    /**
     * Create a specialized interface to interact with the device corresponding
     * to a particular device interface.
     *
     * @param type the device interface type
     * @returns a pointer to the device interface if supported by the device, 0
     * otherwise
     */
    virtual QObject* createDeviceInterface(
        const Solid::DeviceInterface::Type& type) = 0;

    /**
     * Register an action for the given device. Each time the same device in
     * another process broadcast the begin or the end of such action, the
     * corresponding slots will be called in the current process.
     *
     * @param actionName name of the action to register
     * @param dest the object receiving the messages when the action begins and
     * ends
     ...
     */
    void registerAction(const QString& actionName, QObject* dest,
                        const char* requestSlot, const char* doneSlot) const;
