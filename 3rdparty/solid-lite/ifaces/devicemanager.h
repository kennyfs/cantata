...
class DeviceManager : public QObject {
   ...
   public:
     ...
     * @param type DeviceInterface type available on the devices we're looking
     * for, or DeviceInterface::Unknown if there's no constraint on the device
     * interfaces
     * @returns the UDIs of all the devices having the given parent and device
     * interface
     */
    virtual QStringList devicesFromQuery(
        const QString& parentUdi, Solid::DeviceInterface::Type type =
                                      Solid::DeviceInterface::Unknown) = 0;

    /**
     * Instantiates a new Device object from this backend given its UDI.
     *
     * @param udi the identifier of the device instantiated
     * @returns a new Device object if there's a device having the given UDI, 0
     * otherwise
     */
