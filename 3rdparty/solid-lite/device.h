...
class SOLID_EXPORT Device {
   public:
     ...
     *
     * @param type device interface type available on the devices we're looking
     * for, or DeviceInterface::Unknown if there's no constraint on the device
     * interfaces
     * @param parentUdi UDI of the parent of the devices we're searching for, or
     * QString() if there's no constraint on the parent
     ...
     *
     * @param predicate Predicate that the devices we're searching for must
     * verify
     * @param parentUdi UDI of the parent of the devices we're searching for, or
     * QString() if there's no constraint on the parent
     * @return the list of devices corresponding to the given constraints
     ...
     * when the hardware setup changed. The UDI is a unique identifier that is
     * local to the computer in question and for the current boot session. The
     * UDIs may change after a reboot. Similar hardware in other computers may
     * have different values; different hardware could have the same UDI.
     *
    ...
    /**
     * Retrieves a specialized interface to interact with the device
     * corresponding to a particular device interface.
     *
     * @param type the device interface type
     * @returns a pointer to the device interface interface if it exists, 0
     * otherwise
     */
     ...
     * otherwise
     */
    const DeviceInterface* asDeviceInterface(
        const DeviceInterface::Type& type) const;

    /**
     * Retrieves a specialized interface to interact with the device
     * corresponding to a given device interface.
     *
