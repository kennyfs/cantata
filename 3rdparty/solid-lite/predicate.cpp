...
Solid::Predicate::Predicate() : d(new Private()) {}
...
Solid::Predicate::Predicate(const QString& ifaceName, const QString& property,
...
Solid::Predicate::Predicate(const QString& ifaceName) : d(new Private()) {
    DeviceInterface::Type ifaceType = DeviceInterface::stringToType(ifaceName);
...
bool Solid::Predicate::isValid() const { return d->isValid; }
    ...
    switch (d->type) {
        ...
        case PropertyCheck: {
            const DeviceInterface* iface =
                device.asDeviceInterface(d->ifaceType);

            if (iface != nullptr) {
                const int index = iface->metaObject()->indexOfProperty(
                    d->property.toLatin1());
                QMetaProperty metaProp = iface->metaObject()->property(index);
                QVariant value =
                    metaProp.isReadable() ? metaProp.read(iface) : QVariant();
                QVariant expected = d->value;

                if (metaProp.isEnumType() &&
                    expected.type() == QVariant::String) {
                    QMetaEnum metaEnum = metaProp.enumerator();
                    int value =
                        metaEnum.keysToValue(d->value.toString().toLatin1());
                    if (value >= 0) {  // No value found for these keys,
                                       // resetting expected to invalid
                        expected = value;
                    } else {
                        expected = QVariant();
                    }
...
Solid::Predicate::Type Solid::Predicate::type() const { return d->type; }

Solid::DeviceInterface::Type Solid::Predicate::interfaceType() const {
    return d->ifaceType;
}

QString Solid::Predicate::propertyName() const { return d->property; }

QVariant Solid::Predicate::matchingValue() const { return d->value; }
