...
#define SOLID_GLOBAL_STATIC_WITH_ARGS(TYPE, NAME, ARGS)                   \
    ...
    static struct SOLID_GLOBAL_STATIC_STRUCT_NAME(NAME) {                 \
        bool isDestroyed() { return _solid_static_##NAME##_destroyed; }   \
        inline operator TYPE*() { return operator->(); }                  \
        inline TYPE* operator->() {                                       \
            if (!_solid_static_##NAME) {                                  \
                if (isDestroyed()) {                                      \
                    qFatal(                                               \
                        "Fatal Error: Accessed global static '%s *%s()' " \
                        "after destruction. "                             \
                        "Defined at %s:%d",                               \
                ...
                if (!_solid_static_##NAME.testAndSetOrdered(0, x) &&      \
                    ...
                    static Solid::CleanUpGlobalStatic cleanUpObject = {   \
                        destroy};                                         \
                }                                                         \
            }                                                             \
