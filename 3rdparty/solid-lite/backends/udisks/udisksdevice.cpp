...
UDisksDevice::~UDisksDevice() { delete m_device; }

QObject* UDisksDevice::createDeviceInterface(
...
bool UDisksDevice::queryDeviceInterface(
    ...
    switch (type) {
        ...
        case Solid::DeviceInterface::OpticalDrive:
            return !m_udi.endsWith(":media") &&
                   ...
                   !prop("DriveMediaCompatibility")
                        .toStringList()
                        .filter("optical_")
                        .isEmpty();
        case Solid::DeviceInterface::OpticalDisc:
            return m_udi.endsWith(":media") &&
                   prop("DeviceIsOpticalDisc").toBool();
        default:
            return false;
    }
}
...
QStringList UDisksDevice::emblems() const {
    ...
    if (queryDeviceInterface(Solid::DeviceInterface::StorageAccess)) {
        ...
        if (queryDeviceInterface(Solid::DeviceInterface::StorageVolume)) {
            const UDisks::UDisksStorageVolume volIface(
                const_cast<UDisksDevice*>(this));
            isEncrypted = (volIface.usage() == Solid::StorageVolume::Encrypted);
        }

        const UDisks::UDisksStorageAccess accessIface(
            const_cast<UDisksDevice*>(this));
...
QString UDisksDevice::volumeDescription() const {
    ...
    bool drive_is_encrypted_container =
        (storageVolume.usage() == Solid::StorageVolume::Encrypted);
...
QString UDisksDevice::icon() const {
    ...
    if (!iconName.isEmpty()) {
                          ...
                          // parent's icon
        {
            ...
            if (!parentUdi().isEmpty()) parent = new UDisksDevice(parentUdi());
            ...
            if (!iconName.isEmpty()) return iconName;
        }
...
QString UDisksDevice::product() const {
    ...
    const bool isDrive =
        prop("DeviceIsDrive").toBool() && !m_udi.endsWith(":media");
...
QString UDisksDevice::vendor() const { return prop("DriveVendor").toString(); }

QString UDisksDevice::udi() const { return m_udi; }
...
QMap<QString, QVariant> UDisksDevice::allProperties() const {
    QDBusMessage call = QDBusMessage::createMethodCall(
        m_device->service(), m_device->path(),
...
bool UDisksDevice::isDeviceBlacklisted() const {
    return prop("DevicePresentationHide").toBool() ||
           ...
           (prop("IdUsage").toString().isEmpty() &&
            !(prop("OpticalDiscIsBlank").toBool() ||
              (prop("OpticalDiscNumAudioTracks").toInt() > 0)));
}
