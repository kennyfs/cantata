...
QString UDisksStorageAccess::filePath() const {
    if (!isAccessible()) return QString();
...
void UDisksStorageAccess::updateCache() { m_isAccessible = isAccessible(); }
    ...
    if (m_setupInProgress) {
        ...
        if (isLuksDevice() && clearTextPath != "/")  // unlocked device, lock it
        {
            ...
            if (m_device->prop("DriveIsMediaEjectable").toBool() &&
                ...
                !m_device->prop("DeviceIsOpticalDisc")
                     .toBool())  // optical drives have their Eject method
            {
            ...
            QString drivePath = m_device->prop("PartitionSlave")
                                    .value<QDBusObjectPath>()
                                    .path();
            if (!drivePath.isEmpty() || drivePath != "/") {
                QDBusConnection c = QDBusConnection::systemBus();
                QDBusMessage msg = QDBusMessage::createMethodCall(
                    UD_DBUS_SERVICE, drivePath, UD_DBUS_INTERFACE_DISKS_DEVICE,
                    "DriveEject");
                ...
                if (drive.prop("DriveCanDetach").toBool()) {
                    QDBusMessage msg2 = QDBusMessage::createMethodCall(
                        UD_DBUS_SERVICE, drivePath,
                        UD_DBUS_INTERFACE_DISKS_DEVICE, "DriveDetach");
...
void UDisksStorageAccess::slotTeardownDone(int error,
...
bool UDisksStorageAccess::mount() {
    ...
    QDBusConnection c = QDBusConnection::systemBus();
    QDBusMessage msg = QDBusMessage::createMethodCall(
        UD_DBUS_SERVICE, path, UD_DBUS_INTERFACE_DISKS_DEVICE,
        "FilesystemMount");
...
bool UDisksStorageAccess::requestPassphrase() {
    ...
    uint wId = 0;
    if (activeWindow != 0) wId = (uint)activeWindow->winId();
...
void UDisksStorageAccess::callCryptoSetup(const QString& passphrase) {
    QDBusConnection c = QDBusConnection::systemBus();
    QDBusMessage msg = QDBusMessage::createMethodCall(
        UD_DBUS_SERVICE, m_device->udi(), UD_DBUS_INTERFACE_DISKS_DEVICE,
        "LuksUnlock");
...
bool UDisksStorageAccess::callCryptoTeardown(bool actOnParent) {
    ...
    QDBusMessage msg = QDBusMessage::createMethodCall(
        UD_DBUS_SERVICE,
        actOnParent ? (m_device->prop("LuksCleartextSlave")
                           .value<QDBusObjectPath>()
                           .path())
                    : m_device->udi(),
