...
bool OpticalDisc::isRewritable() const {
    ...
    return mediaType == "optical_cd_rw" || mediaType == "optical_dvd_rw" ||
           ...
           mediaType == "optical_hddvd_rw";  // TODO check completeness
}
...
Solid::OpticalDisc::ContentTypes OpticalDisc::availableContent() const {
    ...
    if (m_needsReprobe) {
        ...
        bool hasData = m_device->prop("OpticalDiscNumTracks").toInt() > 0 &&
                       m_device->prop("OpticalDiscNumTracks").toInt() >
                           m_device->prop("OpticalDiscNumAudioTracks").toInt();
