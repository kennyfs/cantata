...
UDevManager::Private::~Private() { delete m_client; }
...
bool UDevManager::Private::isOfInterest(const UdevQt::Device& device) {
    ...
    if (device.subsystem() == QLatin1String("tty")) {
        ...
        int lastSlash =
            path.length() - path.lastIndexOf(QLatin1String("/")) - 1;
        ...
        if (lastElement.startsWith("tty") &&
            !path.startsWith("/devices/virtual")) {
            return true;
        }
    }
    return device.subsystem() == QLatin1String("dvb") ||
           ...
           device.deviceProperty("ID_MEDIA_PLAYER").toString().isEmpty() ==
               false ||  // media-player-info recognized devices
...
UDevManager::UDevManager(QObject* parent)
    ...
    connect(d->m_client, SIGNAL(deviceAdded(UdevQt::Device)), this,
            SLOT(slotDeviceAdded(UdevQt::Device)));
    connect(d->m_client, SIGNAL(deviceRemoved(UdevQt::Device)), this,
...
UDevManager::~UDevManager() { delete d; }
...
QStringList UDevManager::devicesFromQuery(const QString& parentUdi,
    ...
    if (!parentUdi.isEmpty()) {
        foreach (const QString& udi, allDev) {
            UDevDevice device(d->m_client->deviceBySysfsPath(
                udi.right(udi.size() - udiPrefix().size())));
