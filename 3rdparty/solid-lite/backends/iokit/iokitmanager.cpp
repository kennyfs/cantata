...
class IOKitManagerPrivate {
   public:
    inline IOKitManagerPrivate() : port(0), source(0) {}
...
QStringList IOKitManagerPrivate::devicesFromRegistry(io_iterator_t it) {
    ...
    while ((obj = IOIteratorNext(it))) {
        kern_return_t ret =
            IORegistryEntryGetPath(obj, kIOServicePlane, pathName);
        if (ret != KERN_SUCCESS) {
...
IOKitManager::~IOKitManager() {
    ...
    if (d->port) IONotificationPortDestroy(d->port);
...
QStringList IOKitManager::allDevices() {
    // use an IORegistry Iterator to iterate over all devices in the service
    // plane

    io_iterator_t it;
    kern_return_t ret =
        IORegistryCreateIterator(kIOMasterPortDefault, kIOServicePlane,
                                 kIORegistryIterateRecursively, &it);
    if (ret != KERN_SUCCESS) {
...
QStringList IOKitManager::devicesFromQuery(const QString& parentUdi,
    ...
    if (type == Solid::DeviceInterface::Unknown) {
        ...
        if (!deviceClassName) return QStringList();

        CFMutableDictionaryRef matchingDict =
            IOServiceMatching(deviceClassName);

        if (!matchingDict) return QStringList();
...
QObject* IOKitManager::createDevice(const QString& udi) {
    ...
    // we have to do IOObjectConformsTo - comparing the class names is not good
    // enough
