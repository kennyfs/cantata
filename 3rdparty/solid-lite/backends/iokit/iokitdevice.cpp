...
// from cfhelper.cpp
extern QMap<QString, QVariant> q_toVariantMap(
    const CFMutableDictionaryRef& dict);
...
static Solid::DeviceInterface::Type typeFromEntry(
...
static QString getParentDeviceUdi(const io_registry_entry_t& entry) {
    ...
    if (ret == KERN_SUCCESS) result = QString::fromUtf8(pathName);
...
class IOKitDevicePrivate {
   public:
    inline IOKitDevicePrivate() : type(Solid::DeviceInterface::Unknown) {}
...
void IOKitDevicePrivate::init(const QString& udiString,
                              const io_registry_entry_t& entry) {
...
IOKitDevice::IOKitDevice(const QString& udi) : d(new IOKitDevicePrivate) {
    io_registry_entry_t entry = IORegistryEntryFromPath(
        kIOMasterPortDefault, udi.toLocal8Bit().constData());

    if (entry == MACH_PORT_NULL) {
        qDebug() << Q_FUNC_INFO << "Tried to create Device from invalid UDI"
                 << udi;
        return;
    }
...
IOKitDevice::~IOKitDevice() { delete d; }

QString IOKitDevice::udi() const { return d->udi; }

QString IOKitDevice::parentUdi() const { return d->parentUdi; }
