...
OpticalDisc::OpticalDisc(WmiDevice* device) : Volume(device) {
...
Solid::OpticalDisc::ContentTypes OpticalDisc::availableContent() const {
    ...
    if (files.length() > 0)
        if (files[0].endsWith(".cda")) content |= Solid::OpticalDisc::Audio;
...
Solid::OpticalDisc::DiscType OpticalDisc::discType() const {
    ...
    //    }
        ...
        qDebug() << "Solid::OpticalDisc::DiscType OpticalDisc::discType(): "
                    "Unknown Type"
                 << type;
...
bool OpticalDisc::isAppendable() const { return false; }
