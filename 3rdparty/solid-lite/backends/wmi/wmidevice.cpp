...
class Solid::Backends::Wmi::WmiDevicePrivate {
   public:
    WmiDevicePrivate(const QString& _udi)
        : parent(0), m_udi(_udi), m_wmiTable(), m_wmiProperty(), m_wmiValue() {}
    ...
    WmiQuery::Item sendQuery() {
        if (m_item.isNull()) {
            QString query("SELECT * FROM " + m_wmiTable + " WHERE " +
                          m_wmiProperty + "='" + m_wmiValue + "'");
    ...
    static bool convertUDItoWMI(const QString& udi, QString& wmiTable,
                                QString& wmiProperty, QString& wmiValue,
    ...
    static bool exists(const QString& udi) {
        ...
        QString query("SELECT * FROM " + wmiTable + " WHERE " + wmiProperty +
                      "='" + wmiValue + "'");
    ...
    static QList<Solid::DeviceInterface::Type> getInterfaces(
        ...
        switch (type) {
            ...
            case Solid::DeviceInterface::OpticalDrive:
                interfaceList << Solid::DeviceInterface::Block
    ...
    static QString getPropertyNameForUDI(
        const Solid::DeviceInterface::Type& type) {
        QString propertyName;
        switch (type) {
            case Solid::DeviceInterface::OpticalDrive:
    ...
    static QStringList generateUDIList(
        ...
        WmiQuery::ItemList list = WmiQuery::instance().sendQuery(
            "SELECT * FROM " + getWMITable(type) + getIgnorePattern(type));
...
QStringList WmiDevice::generateUDIList(
    const Solid::DeviceInterface::Type& type) {
...
bool WmiDevice::isValid() const {
    // does not work
    // return sendQuery( "SELECT * FROM Win32_SystemDevices WHERE
    // PartComponent='\\\\\\\\BEAST\root\cimv2:Win32_Processor.DeviceID=\"CPU0\"'"
    // ).count() == 1;
    return true;
}

QString WmiDevice::udi() const { return d->udi(); }

QString WmiDevice::parentUdi() const {
    if (!d->m_parent_uid.isEmpty()) return d->m_parent_uid;
    QString result;
    ...
    switch (d->m_type) {
        case Solid::DeviceInterface::StorageVolume:
        case Solid::DeviceInterface::StorageAccess:
            result = "/org/kde/solid/wmi/storage/" +
                     property("DiskIndex").toString();
            break;
        case Solid::DeviceInterface::OpticalDisc:
            result = "/org/kde/solid/wmi/storage.cdrom/" +
                     property("Drive").toString().toLower();
            break;
    }
...
QString WmiDevice::product() const {
    ...
    switch (type()) {
        ...
        case Solid::DeviceInterface::StorageAccess:
        case Solid::DeviceInterface::StorageVolume: {
            WmiQuery::Item item = win32LogicalDiskByDiskPartitionID(
                property("DeviceID").toString());
...
QString WmiDevice::icon() const {
    ...
    switch (type()) {
        ...
        case Solid::DeviceInterface::OpticalDisc: {
            ...
            if (disk.availableContent() |
                Solid::OpticalDisc::Audio)  // no other are recognized yet
...
const Solid::DeviceInterface::Type WmiDevice::type() const { return d->m_type; }
 ...
 * @return
 */
WmiQuery::Item WmiDevice::win32DiskPartitionByDeviceIndex(
    const QString& deviceID) {
    WmiQuery::Item result;
    QString id = deviceID;
    QString query("ASSOCIATORS OF {Win32_DiskDrive.DeviceID='" + id +
...
WmiQuery::Item WmiDevice::win32DiskDriveByDiskPartitionID(
    const QString& deviceID) {
    WmiQuery::Item result;
    QString id = deviceID;
    QString query("ASSOCIATORS OF {Win32_DiskPartition.DeviceID='" + id +
                  "'} WHERE AssocClass = Win32_DiskDriveToDiskPartition");
    WmiQuery::ItemList items = WmiQuery::instance().sendQuery(query);
    if (items.length() > 0) {
        result = items[0];
...
WmiQuery::Item WmiDevice::win32LogicalDiskByDiskPartitionID(
    const QString& deviceID) {
    WmiQuery::Item result;
    QString id = deviceID;
    QString query("ASSOCIATORS OF {Win32_DiskPartition.DeviceID='" + id +
...
WmiQuery::Item WmiDevice::win32DiskPartitionByDriveLetter(
    const QString& driveLetter) {
    WmiQuery::Item result;
    QString id = driveLetter;
    QString query("ASSOCIATORS OF {Win32_LogicalDisk.DeviceID='" + id +
                  "'} WHERE AssocClass = Win32_LogicalDiskToPartition");
    WmiQuery::ItemList items = WmiQuery::instance().sendQuery(query);
    if (items.length() > 0) {
        result = items[0];
...
WmiQuery::Item WmiDevice::win32LogicalDiskByDriveLetter(
    const QString& driveLetter) {
    WmiQuery::Item result;
    QString id = driveLetter;
...
bool WmiDevice::queryDeviceInterface(
...
QObject* WmiDevice::createDeviceInterface(
    const Solid::DeviceInterface::Type& type) {
...
void WmiDevice::slotPropertyModified(int /*count */,
