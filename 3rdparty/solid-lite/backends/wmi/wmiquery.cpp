...
QVariant WmiQuery::Item::msVariantToQVariant(VARIANT msVariant,
                                             CIMTYPE variantType) {
    ...
    switch (variantType) {
        ...
        case CIM_UINT32: {
            ...
            /// msVariant.ullVal); // 32-bit on x86, 64-bit on x64
            //        returnVariant = vb;
            //    }
            //        break;
            //    default:
...
WmiQuery::Item::~Item() {
    if (m_p != NULL &&
        !(qApp->closingDown() ||
          WmiQuery::instance()
              .m_bNeedUninit))  // this means we are in a QApplication, so qt
                                // already called CoUninitialize and all COM
                                // references are all ready freed
        m_p->Release();
}
...
bool WmiQuery::Item::isNull() const { return m_p == NULL; }

WmiQuery::WmiQuery() : m_failed(false), pLoc(0), pSvc(0) {
    HRESULT hres;

    hres = CoInitializeEx(0, COINIT_MULTITHREADED);
    if (FAILED(hres) && hres != S_FALSE && hres != RPC_E_CHANGED_MODE) {
        qCritical() << "Failed to initialize COM library.  Error code = 0x"
                    << hex << quint32(hres);
        m_failed = true;
    }
    ...
    if (!m_failed) {
        ...
        if (FAILED(hres) && hres != RPC_E_TOO_LATE) {
            qCritical() << "Failed to initialize security. "
                        << "Error code = " << hres;
            if (m_bNeedUninit) CoUninitialize();
            m_failed = true;
        }
    }
    if (!m_failed) {
        ...
        if (FAILED(hres)) {
            qCritical() << "Failed to create IWbemLocator object. "
                        << "Error code = " << hres;
            if (m_bNeedUninit) CoUninitialize();
            m_failed = true;
        }
    ...
    if (!m_failed) {
        hres = pLoc->ConnectServer(L"ROOT\\CIMV2", NULL, NULL, 0, NULL, 0, 0,
                                   &pSvc);
        if (FAILED(hres)) {
...
WmiQuery::~WmiQuery() {
    if (m_failed) return;  // already cleaned up properly
...
void WmiQuery::addDeviceListeners(WmiManager::WmiEventSink* sink) {
    ...
    if (FAILED(hr)) {
        qWarning() << "WmiQuery::addDeviceListeners " << sink->query()
                   << " failed!";
    }
}
...
WmiQuery::ItemList WmiQuery::sendQuery(const QString& wql) {
    ...
    hres =
        pSvc->ExecQuery(L"WQL", bstrQuery,
                        WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY,
                        NULL, &pEnumerator);
    ::SysFreeString(bstrQuery);

    if (FAILED(hres)) {
        qDebug() << "Query with string \"" << wql
                 << "\" failed. Error code = " << hres;
    } else {
        ULONG uReturn = 0;
...
bool WmiQuery::isLegit() const { return !m_failed; }

WmiQuery& WmiQuery::instance() {
    static WmiQuery* query = 0;
    if (!query) query = new WmiQuery;
    return *query;
}
