...
QStringList PortableMediaPlayer::supportedProtocols() const {
    return m_device->property("portable_audio_player.access_method.protocols")
        .toStringList();
}

QStringList PortableMediaPlayer::supportedDrivers(QString protocol) const {
    QStringList drivers =
        m_device->property("portable_audio_player.access_method.drivers")
            .toStringList();
    if (protocol.isNull()) return drivers;
    QStringList returnedDrivers;
    QString temp;
    for (int i = 0; i < drivers.size(); i++) {
        temp = drivers.at(i);
        if (m_device->property("portable_audio_player." + temp + ".protocol") ==
            protocol)
...
QVariant Solid::Backends::Wmi::PortableMediaPlayer::driverHandle(
