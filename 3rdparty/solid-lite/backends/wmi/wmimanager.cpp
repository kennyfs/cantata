...
class Solid::Backends::Wmi::WmiManagerPrivate {
   public:
    WmiManagerPrivate(WmiManager* parent) : m_parent(parent) {
        supportedInterfaces
    ...
    void init() {
        if (m_deviceCache.isEmpty()) {
            foreach (const Solid::DeviceInterface::Type& dev,
                     supportedInterfaces) {
    ...
    void updateDeviceCache(const Solid::DeviceInterface::Type& type) {
        QSet<QString> devSet =
            m_parent->findDeviceByDeviceInterface(type).toSet();
...
WmiManager::WmiManager(QObject* parent) : DeviceManager(parent) {
    ...
    types << Solid::DeviceInterface::StorageDrive
          << Solid::DeviceInterface::StorageVolume;
    // partition added
    WmiQuery::instance().addDeviceListeners(new WmiManager::WmiEventSink(
        this,
        "SELECT * FROM __InstanceCreationEvent WITHIN 10 WHERE TargetInstance "
        "ISA 'Win32_DiskPartition'",
        types));
    // partition removed
    WmiQuery::instance().addDeviceListeners(new WmiManager::WmiEventSink(
        this,
        "SELECT * FROM __InstanceDeletionEvent WITHIN 10 WHERE TargetInstance "
        "ISA 'Win32_DiskPartition'",
        types));
...
WmiManager::~WmiManager() { delete d; }

QString WmiManager::udiPrefix() const {
    return QString();  // FIXME: We should probably use a prefix there... has to
...
QStringList WmiManager::findDeviceStringMatch(const QString& key,
                                              const QString& value) {
...
QStringList WmiManager::findDeviceByDeviceInterface(
    Solid::DeviceInterface::Type type) {
...
WmiManager::WmiEventSink::WmiEventSink(
    WmiManager* parent, const QString& query,
    const QList<Solid::DeviceInterface::Type>& types)
    : m_parent(parent), m_query(query), m_types(types), m_count(0) {}
...
HRESULT STDMETHODCALLTYPE WmiManager::WmiEventSink::QueryInterface(REFIID riid,
                                                                   void** ppv) {
...
HRESULT STDMETHODCALLTYPE WmiManager::WmiEventSink::Indicate(
    long lObjectCount, IWbemClassObject** apObjArray) {
...
HRESULT STDMETHODCALLTYPE WmiManager::WmiEventSink::SetStatus(
    long lFlags, HRESULT hResult, BSTR strParam, IWbemClassObject* pObjParam) {
    return WBEM_S_NO_ERROR;
}

const QString& WmiManager::WmiEventSink::query() const { return m_query; }
