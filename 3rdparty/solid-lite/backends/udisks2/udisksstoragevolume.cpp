...
StorageVolume::StorageVolume(Device* device) : Block(device) {}
...
QString StorageVolume::label() const {
    ...
    if (label.isEmpty()) label = m_device->prop("IdLabel").toString();
    if (label.isEmpty()) label = m_device->prop("Name").toString();
    return label;
}
