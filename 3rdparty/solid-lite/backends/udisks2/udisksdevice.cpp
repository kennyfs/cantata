...
static QString formatByteSize(double size) {
    ...
    if (size >= 1073741824.0) {
        ...
        if (size > 1024)  // Tebi-byte
            s = QCoreApplication::translate("", "%1 TiB")
                    .arg(QLocale().toString(size / 1024.0, 'f', 1));
        else
            s = QCoreApplication::translate("", "%1 GiB")
                    .arg(QLocale().toString(size, 'f', 1));
    }
    // Mebi-byte
    else if (size >= 1048576.0) {
        size /= 1048576.0;
        s = QCoreApplication::translate("", "%1 MiB")
                .arg(QLocale().toString(size, 'f', 1));
    }
    // Kibi-byte
    else if (size >= 1024.0) {
        size /= 1024.0;
        s = QCoreApplication::translate("", "%1 KiB")
                .arg(QLocale().toString(size, 'f', 1));
    }
    // Just byte
    else if (size > 0) {
        s = QCoreApplication::translate("", "%1 B")
                .arg(QLocale().toString(size, 'f', 1));
    }
    // Nothing
    else {
...
Device::Device(const QString& udi)
    : Solid::Ifaces::Device(), m_backend(DeviceBackend::backendForUDI(udi)) {
    if (m_backend) {
...
QObject* Device::createDeviceInterface(
...
bool Device::queryDeviceInterface(
...
QString Device::description() const {
    ...
    if (!hintName.isEmpty()) return hintName;
...
QString Device::storageDescription() const {
    ...
    if (drive_type == Solid::StorageDrive::CdromDrive) {
        ...
        first = QCoreApplication::translate(
            "", "CD-ROM", "First item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Cdr)
            first = QCoreApplication::translate(
                "", "CD-R", "First item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Cdrw)
            first = QCoreApplication::translate(
                "", "CD-RW", "First item of %1%2 Drive sentence");

        if (mediumTypes & Solid::OpticalDrive::Dvd)
            second = QCoreApplication::translate(
                "", "/DVD-ROM", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Dvdplusr)
            second = QCoreApplication::translate(
                "", "/DVD+R", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Dvdplusrw)
            second = QCoreApplication::translate(
                "", "/DVD+RW", "Second item of %1%2 Drive sentence");
        ...
        if (mediumTypes & Solid::OpticalDrive::Dvdram)
            second = QCoreApplication::translate(
                "", "/DVD-RAM", "Second item of %1%2 Drive sentence");
        if ((mediumTypes & Solid::OpticalDrive::Dvdr) &&
        ...
        if (mediumTypes & Solid::OpticalDrive::Bd)
            second = QCoreApplication::translate(
                "", "/BD-ROM", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Bdr)
            second = QCoreApplication::translate(
                "", "/BD-R", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::Bdre)
            second = QCoreApplication::translate(
                "", "/BD-RE", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::HdDvd)
            second = QCoreApplication::translate(
                "", "/HD DVD-ROM", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::HdDvdr)
            second = QCoreApplication::translate(
                "", "/HD DVD-R", "Second item of %1%2 Drive sentence");
        if (mediumTypes & Solid::OpticalDrive::HdDvdrw)
            second = QCoreApplication::translate(
                "", "/HD DVD-RW", "Second item of %1%2 Drive sentence");
    ...
    if (drive_type == Solid::StorageDrive::Floppy) {
        if (drive_is_hotpluggable)
            description =
                QCoreApplication::translate("", "External Floppy Drive");
        else
    ...
    if (drive_type == Solid::StorageDrive::HardDisk && !drive_is_removable) {
        ...
        if (!size_str.isEmpty()) {
            if (drive_is_hotpluggable)
                description =
                    QCoreApplication::translate("", "%1 External Hard Drive",
                                                "%1 is the size")
                        .arg(size_str);
            else
                description = QCoreApplication::translate("", "%1 Hard Drive",
                                                          "%1 is the size")
                                  .arg(size_str);
        } else {
            if (drive_is_hotpluggable)
                description =
                    QCoreApplication::translate("", "External Hard Drive");
            else
    ...
    if (vendor_str.isEmpty()) {
        ...
        if (model.isEmpty())
            ...
            if (model.startsWith(vendor_str)) {
                // e.g. vendor is "Nokia" and model is "Nokia N950" we do not
                // want "Nokia Nokia N950" as description
...
QString Device::volumeDescription() const {
    ...
    if (volume_label.isEmpty()) volume_label = prop("Name").toString();
    if (!volume_label.isEmpty()) return volume_label;
    ...
    if (drive_type == Solid::StorageDrive::CdromDrive) {
        ...
        switch (disc.discType()) {
            ...
            case Solid::OpticalDisc::CdRewritable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank CD-RW");
                else
            ...
            case Solid::OpticalDisc::DvdRam:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank DVD-RAM");
                else
            ...
            case Solid::OpticalDisc::DvdRecordable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank DVD-R");
                else
            ...
            case Solid::OpticalDisc::DvdPlusRecordableDuallayer:
                if (disc.isBlank())
                    ...
                    description =
                        QCoreApplication::translate("", "DVD+R Dual-Layer");
                break;

            case Solid::OpticalDisc::DvdRewritable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank DVD-RW");
                else
            ...
            case Solid::OpticalDisc::DvdPlusRewritableDuallayer:
                if (disc.isBlank())
                    ...
                    description =
                        QCoreApplication::translate("", "DVD+RW Dual-Layer");
                break;
            ...
            case Solid::OpticalDisc::BluRayRewritable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank BD-RE");
                else
            ...
            case Solid::OpticalDisc::HdDvdRecordable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank HD DVD-R");
                else
            ...
            case Solid::OpticalDisc::HdDvdRewritable:
                if (disc.isBlank())
                    description =
                        QCoreApplication::translate("", "Blank HD DVD-RW");
                else
    ...
    if (isEncryptedContainer()) {
        if (!size_str.isEmpty())
            description = QCoreApplication::translate("", "%1 Encrypted Drive",
                                                      "%1 is the size")
                              .arg(size_str);
        else
        ...
        if (drive_is_removable)
            description = QCoreApplication::translate("", "%1 Removable Media",
                                                      "%1 is the size")
                              .arg(size_str);
        else
            description =
                QCoreApplication::translate("", "%1 Media", "%1 is the size")
                    .arg(size_str);
    }
...
QString Device::icon() const {
    ...
    if (!iconName.isEmpty()) {
        ...
        if (!media.isEmpty()) {
            if (drive.prop("Optical").toBool())  // optical stuff
            {
                ...
                if (availContent & Solid::OpticalDisc::VideoDvd)  // Video DVD
                    return "media-optical-dvd-video";
                         ...
                         (availContent &
                          Solid::OpticalDisc::SuperVideoCd))  // Video CD
                    return "media-optical-video";
                else if ((availContent & Solid::OpticalDisc::Data) &&
                         (availContent &
                          Solid::OpticalDisc::Audio))  // Mixed CD
            ...
            else if (media == "flash_sd" || media == "flash_sdhc" ||
                     media == "flash_sdxc" || media == "flash_mmc")
        ...
        if (drive.prop("ConnectionBus").toString() ==
            "sdio")  // hack for SD cards connected thru sdio bus
            return "media-flash-sd-mmc";
...
bool Device::isBlock() const { return hasInterface(UD2_DBUS_INTERFACE_BLOCK); }
...
bool Device::isDrive() const { return hasInterface(UD2_DBUS_INTERFACE_DRIVE); }

bool Device::isOpticalDrive() const {
    return isDrive() && !prop("MediaCompatibility")
                             .toStringList()
                             .filter("optical_")
                             .isEmpty();
}
...
bool Device::isOpticalDisc() const {
    const QString drv = drivePath();
    if (drv.isEmpty() || drv == "/") return false;
...
bool Device::isSwap() const { return hasInterface(UD2_DBUS_INTERFACE_SWAP); }

bool Device::isLoop() const { return hasInterface(UD2_DBUS_INTERFACE_LOOP); }
