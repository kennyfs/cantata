...
Manager::Manager(QObject* parent)
    ...
    if (!serviceFound) {
        ...
        if (reply.isValid() && reply.value().contains(UD2_DBUS_SERVICE)) {
            QDBusConnection::systemBus().interface()->startService(
                UD2_DBUS_SERVICE);
            serviceFound = true;
        }
    }
    ...
    if (serviceFound) {
        connect(&m_manager,
                SIGNAL(InterfacesAdded(QDBusObjectPath, QVariantMapMap)), this,
                SLOT(slotInterfacesAdded(QDBusObjectPath, QVariantMapMap)));
        connect(&m_manager,
                SIGNAL(InterfacesRemoved(QDBusObjectPath, QStringList)), this,
...
QObject* Manager::createDevice(const QString& udi) {
    if (udi == udiPrefix()) {
        ...
        root->setDescription(
            QCoreApplication::translate("", "Storage devices"));
        root->setIcon(
            "server-database");  // Obviously wasn't meant for that, but maps
                                 // nicely in oxygen icon set :-p
...
QStringList Manager::devicesFromQuery(const QString& parentUdi,
    ...
    if (!parentUdi.isEmpty()) {
        ...
        Q_FOREACH (const QString& udi, deviceCache()) {
            Device device(udi);
            if (device.queryDeviceInterface(type)) result << udi;
        }
...
void Manager::introspect(const QString& path, bool checkOptical) {
    QDBusMessage call = QDBusMessage::createMethodCall(
        UD2_DBUS_SERVICE, path, DBUS_INTERFACE_INTROSPECT, "Introspect");
                ...
                if (checkOptical) {
                    ...
                    if (device.mightBeOpticalDisc()) {
                        QDBusConnection::systemBus().connect(
                            UD2_DBUS_SERVICE, udi, DBUS_INTERFACE_PROPS,
...
QString Manager::udiPrefix() const { return UD2_UDI_DISKS_PREFIX; }

void Manager::slotInterfacesAdded(
    ...
    qDebug() << udi
             << "has new interfaces:" << interfaces_and_properties.keys();
    ...
    // re-emit in case of 2-stage devices like N9 or some Android phones
             ...
             interfaces_and_properties.keys().contains(
                 UD2_DBUS_INTERFACE_FILESYSTEM)) {
        Q_EMIT deviceAdded(udi);
    }
}

void Manager::slotInterfacesRemoved(const QDBusObjectPath& object_path,
    ...
    if (!udi.isEmpty() &&
        (interfaces.isEmpty() || device.interfaces().isEmpty())) {
...
void Manager::slotMediaChanged(const QDBusMessage& msg) {
    ...
    if (!m_deviceCache.contains(udi) &&
        size > 0) {  // we don't know the optdisc, got inserted
        m_deviceCache.append(udi);
        Q_EMIT deviceAdded(udi);
    }

    if (m_deviceCache.contains(udi) &&
        size == 0) {  // we know the optdisc, got removed
        Q_EMIT deviceRemoved(udi);
        m_deviceCache.removeAll(udi);
        DeviceBackend::destroyBackend(udi);
    }
...
const QStringList& Manager::deviceCache() {
    if (m_deviceCache.isEmpty()) allDevices();
...
void Manager::updateBackend(const QString& udi) {
    ...
    DeviceBackend* driveBackend =
        DeviceBackend::backendForUDI(drivePath.path(), false);
