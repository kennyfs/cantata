...
Block::Block(Device* dev) : DeviceInterface(dev) {
    ...
    // corresponding (real) block device
    if (m_devNum == 0 || m_devFile.isEmpty()) {
        ...
        if (reply.isValid()) {
            ...
            QDomNodeList nodeList =
                dom.documentElement().elementsByTagName("node");
            for (int i = 0; i < nodeList.count(); i++) {
                ...
                if (!nodeElem.isNull() && nodeElem.hasAttribute("name")) {
                    ...
                    if (device.drivePath() == dev->udi()) {
                        ...
                        m_devFile = QFile::decodeName(
                            device.prop("Device").toByteArray());
                        break;
                    }
                }
            }
...
QString Block::device() const { return m_devFile; }

int Block::deviceMinor() const { return MINOR(m_devNum); }

int Block::deviceMajor() const { return MAJOR(m_devNum); }
