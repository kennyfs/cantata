...
Cdrom::Cdrom(HalDevice* device) : Storage(device), m_ejectInProgress(false) {
    connect(device, SIGNAL(conditionRaised(QString, QString)), this,
            SLOT(slotCondition(QString, QString)));
...
QList<int> Cdrom::writeSpeeds() const {
    QList<int> speeds;
    QStringList speed_strlist =
        m_device->prop("storage.cdrom.write_speeds").toStringList();
...
bool Cdrom::callHalDriveEject() {
    ...
    if (m_device->prop("storage.removable.media_available").toBool()) {
        ...
        QDBusReply<QStringList> reply =
            manager.call("FindDeviceStringMatch", "info.parent", udi);
...
void Solid::Backends::Hal::Cdrom::slotProcessFinished(
