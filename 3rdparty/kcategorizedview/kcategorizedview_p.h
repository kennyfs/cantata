...
class Q_DECL_HIDDEN KCategorizedView::Private {
   public:
     ...
     *
     * @note see that here we cannot take out items between first and last (as
     * we could do with the rubberband).
     *
     * Complexity: O(log(n)) where n is model()->rowCount().
     */
    QPair<QModelIndex, QModelIndex> intersectingIndexesWithRect(
        const QRect& rect) const;

    /**
     * Returns the position of the block of @p category.
     *
     * Complexity: O(n) where n is the number of different categories when the
     * block has been marked as in quarantine. O(1) the rest of the times (the
     * vast majority).
     */
    ...
    /**
     * Update internal information, and keep sync with the real information that
     * the model contains.
     */
    void rowsInserted(const QModelIndex& parent, int start, int end);

    /**
     * Returns @p rect in viewport terms, taking in count horizontal and
     * vertical offsets.
     */
    ...
    /**
     * Returns the height of the highest element in last row. This is only
     * applicable if there is no grid set and uniformItemSizes is false.
     *
     * @param block in which block are we searching. Necessary to stop the
     * search if we hit the first item in this block.
     */
