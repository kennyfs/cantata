...
/**
 * @class KCategorizedSortFilterProxyModel kcategorizedsortfilterproxymodel.h
 ...
 * categorization is disabled by default.
 *
...
class /*KITEMVIEWS_EXPORT*/ KCategorizedSortFilterProxyModel
   ...
   public:
    enum AdditionalRoles {
        ...
        CategoryDisplayRole = 0x17CE990A,  ///< This role is used for asking the
                                           ///< category to a given index

        CategorySortRole =
            0x27857E60  ///< This role is used for sorting categories. You can
                        ///< return a string or a long long value. Strings will
                        ///< be sorted alphabetically while long long will be
                        ///< sorted by their value. Please note that this value
                        ///< won't be shown on the view, is only for sorting
                        ///< purposes. What will be shown as "Category" on the
     ...
     *
     * @param categorizedModel whether to enable or disable the categorization
     * feature.
     */
     ...
     *
     * @param sortCategoriesByNaturalComparison whether to sort using a natural
     * comparison or not.
     */
    void setSortCategoriesByNaturalComparison(
    ...
    /**
     * Does a natural comparing of the strings. A negative value is returned if
     * \a a is smaller than \a b. A positive value is returned if \a a is
     * greater than \a b. 0 is returned if both values are equal.
     * @deprecated
     * Use QCollator instead.
     */
    /*
    #ifndef KITEMVIEWS_NO_DEPRECATED
        KITEMVIEWS_DEPRECATED static int naturalCompare(const QString &a, const
    QString &b); #endif
    */
   protected:
    /**
     ...
     *
     * It calls compareCategories() to sort by category.  If the both items are
     * in the same category (i.e. compareCategories returns 0), then
     * subSortLessThan is called.
     *
     * @return Returns true if the item @p left is less than the item @p right
     * when sorting.
     *
     * @warning You usually won't need to reimplement this method when
     * subclassing from KCategorizedSortFilterProxyModel.
     */
     ...
     * QSortFilterProxyModel. It is used for sorting items that are in the same
     * category.
     *
     * @return Returns true if the item @p left is less than the item @p right
     * when sorting.
     ...
     * categories, the data() metod of the model should return a qlonglong (or
     * numeric) value, or a QString object. QString objects will be sorted with
     * QString::localeAwareCompare if sortCategoriesByNaturalComparison() is
     * true.
     *
     ...
     * index, you should return always QStrings for all indexes for role
     * CategorySortRole in order to correctly sort categories. You can't mix by
     * returning a QString for one index, and a qlonglong for other.
     *
     ...
     * before the category of @p right. 0 if @p left and @p right are on the
     * same category, and a positive value if the category of @p left should be
     * placed after the category of @p right.
     */
