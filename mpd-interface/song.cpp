...
static const QString constMpdQueue =
    QLatin1String(".cue/track");  // .cue.track0123
...
const QString& Song::unknown() { return unknownStr; }

const QString& Song::variousArtists() { return variousArtistsStr; }

const QString& Song::singleTracks() { return singleTracksStr; }
...
static int songType(const Song& s) {
    static QStringList extensions =
        QStringList() << QLatin1String(".flac") << QLatin1String(".wav")
                      << QLatin1String(".dff") << QLatin1String(".dsf")
                      << QLatin1String(".aac") << QLatin1String(".m4a")
                      << QLatin1String(".m4b") << QLatin1String(".m4p")
                      << QLatin1String(".mp4") << QLatin1String(".ogg")
                      << QLatin1String(".opus") << QLatin1String(".mp3")
                      << QLatin1String(".wma");
...
QString Song::decodePath(const QString& file, bool cdda) {
    ...
    if (file.startsWith(constMopidyLocal)) {
        return QUrl::fromPercentEncoding(
            file.mid(constMopidyLocal.length()).toLatin1());
    }
...
const QSet<QString>& Song::composerGenres() { return compGenres; }

void Song::setComposerGenres(const QSet<QString>& g) { compGenres = g; }
...
bool Song::operator==(const Song& o) const { return 0 == compareTo(o); }

bool Song::operator<(const Song& o) const { return compareTo(o) < 0; }
...
int Song::compareTo(const Song& o) const {
    ...
    if (Playlist != type) {
        ...
        int compare =
            COMPARE(albumArtistOrComposer(), o.albumArtistOrComposer());
        if (0 != compare) {
            return compare;
        }
...
bool Song::sameMetadata(const Song& o) const {
    return disc == o.disc && track == o.track && time == o.time &&
           year == o.year && origYear == o.origYear && artist == o.artist &&
           albumartist == o.albumartist && album == o.album && title == o.title;
}
    ...
    if (isEmpty() && !isStream()) {
        ...
        if (3 == parts.length()) {
            ...
            artist = parts.at(0);
                   ...
                   parts.at(0).contains(constAlbumArtistSep)) {
            title = parts.at(1);
            QStringList albumArtistParts =
                parts.at(0).split(constAlbumArtistSep, CANTATA_SKIP_EMPTY);
        ...
        if (!title.isEmpty()) {
            ...
            static const QSet<QChar> constSeparators =
                QSet<QChar>() << QLatin1Char(' ') << QLatin1Char('-')
            ...
            if ((1 == separator && title[separator - 1].isDigit()) ||
                (2 == separator && title[separator - 2].isDigit() &&
                 title[separator - 1].isDigit())) {
                if (0 == track) {
...
void Song::revertGuessedTags() { title = artist = album = unknownStr; }
...
QString Song::displayAlbum(const QString& albumName, quint16 albumYear) {
    return albumYear > 0 ? albumName + QLatin1String(" (") +
                               QString::number(albumYear) + QLatin1Char(')')
                         : albumName;
}

static QSet<QString> prefixesToIngore = QSet<QString>() << QLatin1String("The");

QSet<QString> Song::ignorePrefixes() { return prefixesToIngore; }
...
bool Song::useOriginalYear() { return useOrigYear; }

void Song::setUseOriginalYear(bool u) { useOrigYear = u; }
...
quint16 Song::setKey(int location) {
    ...
    if (it != store.keys.end()) {
        ...
        store.currentKey++;  // Key 0 is for streams, so we need to increment
                             // before setting...
...
bool Song::isUnknownAlbum() const {
    return (album.isEmpty() || album == unknownStr) &&
           (albumArtist().isEmpty() || albumArtist() == unknownStr);
}

bool Song::isInvalid() const {
    return 0 == time && guessed && !file.contains("://") &&
           (genres[0].isEmpty() || genres[0] == unknownStr) && name().isEmpty();
}
...
QString Song::albumName() const {
    if (useComposer()) {
        ...
        if (!c.isEmpty() && c != albumArtist()) {
            return album + QLatin1String(" (") + albumArtist() +
                   QLatin1Char(')');
        }
    }
    return album;
...
#if !defined CANTATA_NO_UI_FUNCTIONS
    if ((OnlineSvrTrack == type || CantataStream == type) &&
        OnlineService::showLogoAsCover(*this)) {
        return artistSong();
    }
#endif
    //    if (isFromOnlineService()) {
    //        return (disc>0 && disc!=constOnlineDiscId ?
    //        (QString::number(disc)+QLatin1Char('.')) : QString())+
    //               (track>0 ? (track>9 ? QString::number(track) :
    //               (QLatin1Char('0')+QString::number(track))) : QString())+
    //               QLatin1Char(' ')+(addArtist ? artistSong() : title);
    //    }
    return  //(disc>0 ? (QString::number(disc)+QLatin1Char('.')) : QString())+
        (track > 0 && SingleTracks != type
             ? (track > 9 ? QString::number(track) + QLatin1Char(' ')
                          : (QLatin1Char('0') + QString::number(track) +
                             QLatin1Char(' ')))
             : QString()) +
        (showArtistIfDifferent && diffArtist() ? artistSong() : title) +
        (origYear > 0 && origYear != year
             ? QLatin1String(" (") + QString::number(origYear) +
                   QLatin1Char(')')
             : QString());
}
...
QString Song::toolTip() const {
    ...
    if (origYear > 0) {
        addField(QObject::tr("Original Year"), QString::number(origYear),
                 toolTip);
    }
    ...
    return toolTip + QLatin1String("<br/><br/><small><i>") + filePath() +
           QLatin1String("</i></small>");
#endif
}
...
bool Song::diffArtist() const {
    return /*isVariousArtists() || */ (
        !albumartist.isEmpty() && !artist.isEmpty() && albumartist != artist);
}
...
bool Song::revertVariousArtists() {
    if (artist ==
        albumartist) {  // Then real artist is embedded in track title...
...
bool Song::capitalise() {
    ...
    */
    ...
    return artist != origArtist || albumartist != origAlbumArtist ||
           album != origAlbum || title != origTitle ||
           (!c.isEmpty() &&
            c != composer()) /*|| (!p.isEmpty() && p!=performer())*/;
}
...
#endif
    return albumArtistOrComposer() + QLatin1Char(':') +
           albumId();  //+QLatin1Char(':')+QString::number(disc);
}

static QString basic(const QString& str,
                     const QStringList& extraToStrip = QStringList()) {
    QStringList toStrip = QStringList()
                          << QLatin1String("ft. ") << QLatin1String("feat. ")
                          << QLatin1String("featuring ") << QLatin1String("f. ")
                          << extraToStrip;
...
QString Song::basicArtist(bool orComposer) const {
    ...
    if (!albumartist.isEmpty() && (artist.isEmpty() || albumartist == artist ||
                                   (albumartist.length() < artist.length() &&
                                    artist.startsWith(albumartist)))) {
        return albumartist;
    }
...
QString Song::basicTitle() const {
    return basic(title, QStringList()
                            << QLatin1String("prod. ") << QLatin1String("prod ")
                            << QLatin1String("producer ")
...
bool Song::isMpdCueTrack() const {
    return file.indexOf(constMpdQueue) ==
           file.length() - (constMpdQueue.length() + 4);
}
...
#if !defined CANTATA_NO_UI_FUNCTIONS
    if (OnlineSvrTrack == type &&
        PodcastService::constName == onlineService()) {
        return title;
    }
#endif

    QString albumText =
        album.isEmpty() ? name() : displayAlbum(album, Song::albumYear(*this));

    if (title.isEmpty()) {
        return QLatin1String("<b>") + albumText + QLatin1String("</b>");
    }
    return artist.isEmpty()
               ? QObject::tr("<b>%1</b> on <b>%2</b>", "Song on Album")
                     .arg(title)
                     .arg(albumText)
               : QObject::tr("<b>%1</b> by <b>%2</b> on <b>%3</b>",
...
QString Song::mainText() const {
    ...
    if (isStream() && !isCantataStream() && !isCdda() && !isDlnaStream()) {
        return n.isEmpty() ? Song::unknown() : n;
               ...
               (!n.isEmpty() || !file.isEmpty())) {
        return n.isEmpty() ? file : n;
    } else {
        return title +
               (origYear > 0 && !Song::useOriginalYear() && origYear != year
                    ? QLatin1String(" (") + QString::number(origYear) +
                          QLatin1Char(')')
                    : QString());
    }
    ...
    if (isStream() && !isCantataStream() && !isCdda() && !isDlnaStream()) {
        if (artist.isEmpty() && title.isEmpty() && !name().isEmpty()) {
            ...
            return artist.isEmpty() ? title : artistSong();
               ...
               (!useComposer() || composer().isEmpty())) {
        return mainText().isEmpty() ? QString() : Song::unknown();
    } else {
        QString comp = useComposer() ? composer() : QString();
        if (album.isEmpty()) {
            return artist.isEmpty() ? comp
                   : comp.isEmpty() ? artist
                                    : (comp + constSep + artist);
        } else {
            // Artist here is always artist (or 'composer - artist'), and not
            // album artist
            return (artist.isEmpty() ? comp
                    : comp.isEmpty() ? artist
                                     : (comp + constSep + artist)) +
...
QDataStream& operator<<(QDataStream& stream, const Song& song) {
    stream << song.id << song.file << song.album << song.artist
           << song.albumartist << song.title << song.disc << song.priority
           << song.time << song.track << (quint16)song.year  // << song.origYear
           << (quint16)song.type << (bool)song.guessed << song.size
           << song.extra << song.extraFields;
...
QDataStream& operator>>(QDataStream& stream, Song& song) {
    ...
    bool guessed;
    stream >> song.id >> song.file >> song.album >> song.artist >>
        song.albumartist >> song.title >> disc >> song.priority >> song.time >>
        song.track >> year  // >> song.origYear
