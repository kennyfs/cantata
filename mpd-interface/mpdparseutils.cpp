...
#define DBUG \
    if (debugEnabled) qWarning() << "MPDParseUtils"
void MPDParseUtils::enableDebug() { debugEnabled = true; }
...
void MPDParseUtils::setCueFileSupport(CueSupport cs) { cueSupport = cs; }

MPDParseUtils::CueSupport MPDParseUtils::cueFileSupport() { return cueSupport; }
...
QList<Playlist> MPDParseUtils::parsePlaylists(const QByteArray& data) {
    ...
    for (int i = 0; i < amountOfLines; i++) {
        ...
        if (line.startsWith(constPlaylistKey)) {
            Playlist playlist;
            playlist.name =
                QString::fromUtf8(line.mid(constPlaylistKey.length()));
            i++;

            if (i < amountOfLines) {
                const QByteArray& line = lines.at(i);
                if (line.startsWith(constLastModifiedKey)) {
                    playlist.lastModified = QDateTime::fromString(
                        QString::fromUtf8(
                            line.mid(constLastModifiedKey.length())),
                        Qt::ISODate);
...
MPDStatusValues MPDParseUtils::parseStatus(const QByteArray& data) {
    ...
    for (const QByteArray& line : lines) {
        if (line.startsWith(constStatusVolumeKey)) {
            ...
            v.partition =
                QString::fromUtf8(line.mid(constStatusPartitionKey.length()));
            ...
            v.playlistLength =
                line.mid(constStatusPlaylistLengthKey.length()).toInt();
            ...
            QList<QByteArray> values =
                line.mid(constStatusTimeKey.length()).split(':');
            if (values.length() > 1) {
            ...
            QList<QByteArray> values =
                line.mid(constStatusAudioKey.length()).split(':');
            if (3 == values.length()) {
...
static QSet<QString> constStdProtocols =
    QSet<QString>() << constHttpProtocol << QLatin1String("https://")
                    << QLatin1String("mms://") << QLatin1String("mmsh://")
                    << QLatin1String("mmst://") << QLatin1String("mmsu://")
                    << QLatin1String("gopher://") << QLatin1String("rtp://")
                    << QLatin1String("rtsp://") << QLatin1String("rtmp://")
                    << QLatin1String("rtmpt://") << QLatin1String("rtmps://");

Song MPDParseUtils::parseSong(const QList<QByteArray>& lines,
                              Location location) {
    Song song;
    for (const QByteArray& line : lines) {
        if (line.startsWith(constFileKey)) {
            ...
            song.albumartist =
                QString::fromUtf8(line.mid(constAlbumArtistKey.length()));
        } else if (line.startsWith(constGroupingKey)) {
            song.setGrouping(
                QString::fromUtf8(line.mid(constGroupingKey.length())));
        } else if (line.startsWith(constComposerKey)) {
            song.setComposer(
                QString::fromUtf8(line.mid(constComposerKey.length())));
            ...
            song.track = v < 0 ? 0 : v;
        } else if (Loc_Library != location && Loc_Search != location &&
                   line.startsWith(constIdKey)) {
            ...
            int v =
                value.length() > 4 ? value.left(4).toUInt() : value.toUInt();
            song.year = v < 0 ? 0 : v;
        ...
        } else if ((Loc_Search == location || Loc_Library == location) &&
                   line.startsWith(constLastModifiedKey)) {
            song.lastModified =
                QDateTime::fromString(
                    QString::fromUtf8(line.mid(constLastModifiedKey.length())),
                    Qt::ISODate)
                    .toTime_t();
        } else if ((Loc_Search == location || Loc_Playlists == location ||
                    Loc_PlayQueue == location) &&
                   line.startsWith(constPerformerKey)) {
            if (song.hasPerformer()) {
                song.setPerformer(
                    song.performer() + QLatin1String(", ") +
                    QString::fromUtf8(line.mid(constPerformerKey.length())));
            } else {
                song.setPerformer(
                    QString::fromUtf8(line.mid(constPerformerKey.length())));
            }
            ...
            if (line.startsWith(constPriorityKey)) {
                ...
                song.setComment(
                    QString::fromUtf8(line.mid(constCommentKey.length())));
            }
        } else if (Loc_Library == location) {
            if (line.startsWith(constAlbumSortKey)) {
                song.setAlbumSort(
                    QString::fromUtf8(line.mid(constAlbumSortKey.length())));
            } else if (line.startsWith(constArtistSortKey)) {
                song.setArtistSort(
                    QString::fromUtf8(line.mid(constArtistSortKey.length())));
            } else if (line.startsWith(constAlbumArtistSortKey)) {
                song.setAlbumArtistSort(QString::fromUtf8(
                    line.mid(constAlbumArtistSortKey.length())));
            }
        }
    }
    ...
    if (Loc_Library == location) {
...
#ifdef ENABLE_HTTP_SERVER
        if (!song.file.isEmpty() && song.file.startsWith(constHttpProtocol) &&
            HttpServer::self()->isOurs(song.file)) {
            ...
            if (song.isStream()) {
                if (song.isCantataStream()) {
                    ...
                    if (OnlineService::decode(song)) {
                       ...
                       Song::Standard == song.type &&
                       !singleTracksFolders.isEmpty() &&
                       singleTracksFolders.contains(
                           Utils::getDir(song.file, false))) {
        ...
        // HTTP server, and OnlineServices, modify the path. But this then
        // messes up undo/restore of playqueue. Therefore, set path back to
        // original value...
        if (modifiedFile) {
        ...
        if (Song::OnlineSvrTrack != song.type &&
            PodcastService::isPodcastFile(song.file)) {
...
QList<MPDParseUtils::IdPos> MPDParseUtils::parseChanges(
...
QStringList MPDParseUtils::parseList(const QByteArray& data,
    ...
    for (const QByteArray& line : lines) {
        ...
        if (line.startsWith(key)) {
            items.append(
                QString::fromUtf8(line.mid(keyLen).replace("://", "")));
        }
    }
    ...
    for (const QByteArray& line : lines) {
        ...
        if (line.startsWith(constChannel)) {
            ...
            messages[channel].append(
                QString::fromUtf8(line.mid(constMessage.length())));
        }
    }
    return messages;
}

void MPDParseUtils::parseDirItems(const QByteArray& data, const QString& mpdDir,
                                  long mpdVersion, QList<Song>& songList,
                                  const QString& dir, QStringList& subDirs,
                                  Location loc) {
    QList<QByteArray> currentItem;
    QList<QByteArray> lines = data.split('\n');
    int amountOfLines = lines.size();
    bool parsePlaylists = "/" != dir && "" != dir;
    bool setSingleTracks = parsePlaylists &&
                           singleTracksFolders.contains(dir) &&
                           Loc_Browse != loc;
    QList<Song> songs;
    ...
    for (int i = 0; i < amountOfLines; i++) {
        ...
        if (line.startsWith(constDirectoryKey)) {
            subDirs.append(
                QString::fromUtf8(line.mid(constDirectoryKey.length())));
        }
        currentItem.append(line);
        if (i == amountOfLines - 1 ||
            lines.at(i + 1).startsWith(constFileKey) ||
            lines.at(i + 1).startsWith(constPlaylistKey)) {
            ...
            if (Song::Playlist == currentSong.type) {
                // lsinfo will return all stored playlists - but this is
                // deprecated.
                if (!parsePlaylists) {
                    continue;
                }
                ...
                QSet<QString> cueFiles;  // List of source (flac, mp3, etc)
                                         // files referenced in cue file

                DBUG << "Got playlist item" << currentSong.file;

                bool canSplitCue = mpdVersion >= CANTATA_MAKE_VERSION(0, 17, 0);
                bool parseCue = canSplitCue && currentSong.isCueFile() &&
                                !mpdDir.startsWith(constHttpProtocol) &&
                                QFile::exists(mpdDir + currentSong.file);
                bool cueParseStatus = false;
                double lastTrackIndex = 0.0;
                if (parseCue) {
                    DBUG << "Parsing cue file:" << currentSong.file
                         << "mpdDir:" << mpdDir;
                    cueParseStatus =
                        CueFile::parse(currentSong.file, mpdDir, cueSongs,
                                       cueFiles, lastTrackIndex);
                    if (!cueParseStatus) {
                        DBUG << "Failed to parse cue file!";
                        continue;
                    } else
                        DBUG << "Parsed cue file, songs:" << cueSongs.count()
                             << "files:" << cueFiles;
                }
                if (cueParseStatus && cueSongs.count() >= songs.count() &&
                    (cueFiles.count() < cueSongs.count() ||
                     (firstSong.albumArtist().isEmpty() &&
                      firstSong.album.isEmpty()))) {
                    bool canUseThisCueFile = true;
                    for (const Song& s : cueSongs) {
                        if (!QFile::exists(mpdDir + s.name())) {
                            DBUG << QString(mpdDir + s.name())
                                 << "is referenced in cue file, but does not "
                                    "exist in MPD folder";
                    ...
                    QList<Song> fixedCueSongs;  // Songs taken from cueSongs
                    ...
                    if (songs.size() == cueFiles.size()) {
                        ...
                        bool setTimeFromSource =
                            origFiles.size() == cueSongs.size();
                        DBUG << "setTimeFromSource" << setTimeFromSource << "at"
                             << albumTime << "#c" << cueFiles.size();
                        for (const Song& orig : cueSongs) {
                            Song s = orig;
                            Song albumSong = origFiles[s.name()];
                            s.setName(QString());  // CueFile has placed source
                                                   // file name here!
                            if (s.artist.isEmpty() &&
                                !albumSong.artist.isEmpty()) {
                                s.artist = albumSong.artist;
                                DBUG << "Get artist from album"
                                     << albumSong.artist;
                            }
                            if (s.composer().isEmpty() &&
                                !albumSong.composer().isEmpty()) {
                                s.setComposer(albumSong.composer());
                                DBUG << "Get composer from album"
                                     << albumSong.composer();
                            }
                            if (s.album.isEmpty() &&
                                !albumSong.album.isEmpty()) {
                                s.album = albumSong.album;
                                DBUG << "Get album from album"
                                     << albumSong.album;
                            }
                            if (s.albumartist.isEmpty() &&
                                !albumSong.albumartist.isEmpty()) {
                                s.albumartist = albumSong.albumartist;
                                DBUG << "Get albumartist from album"
                                     << albumSong.albumartist;
                            }
                            ...
                            if (0 == s.time && setTimeFromSource) {
                                ...
                                DBUG << "Set time of last track" << s.title
                                     << s.time << albumTime
                                     << (lastTrackIndex / 1000.0);
                                // Try to set duration of last track by
                                // subtracting previous track durations from
                                // album duration...
                        ...
                        DBUG << "ERROR: file count mismatch" << songs.size()
                             << cueFiles.size();

                    if (!canUseCueFileTracks) {
                        // Album had a different number of source files to the
                        // CUE file. If so, then we need to ensure all tracks
                        // have meta data - otherwise just fallback to listing
                        // file + cue
                        for (const Song& orig : cueSongs) {
                            Song s = orig;
                            s.setName(QString());  // CueFile has placed source
                        ...
                        if (fixedCueSongs.count() == cueSongs.count()) {
                            ...
                            DBUG << "ERROR: Not all cue tracks had meta data";
                    }
                ...
                if (!firstSong.albumArtist().isEmpty() &&
                    !firstSong.album.isEmpty()) {
...
QByteArray MPDParseUtils::parseSticker(const QByteArray& data,
                                       const QByteArray& sticker) {
    QList<QByteArray> lines = data.split('\n');
...
QList<MPDParseUtils::Sticker> MPDParseUtils::parseStickers(
    const QByteArray& data, const QByteArray& sticker) {
...
QString MPDParseUtils::addStreamName(const QString& url, const QString& name,
                                     bool singleHash) {
    return Utils::addHashParam(url, singleHash ? QString() : constStreamName,
                               name);
}
