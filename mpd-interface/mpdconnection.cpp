...
#define DBUG          \
    if (debugEnabled) \
    qWarning() << "MPDConnection" << QThread::currentThreadId()
void MPDConnection::enableDebug() { debugEnabled = true; }

// Uncomment the following to report error strings in MPDStatus to the UI
// ...disabled, as stickers (for ratings) can cause lots of errors to be
// reported - and these all need clearing, etc. #define REPORT_MPD_ERRORS
...
static inline int socketTimeout(int dataSize) {
    ...
    return ((dataSize / constDataBlock) +
            ((dataSize % constDataBlock) ? 1 : 0)) *
...
static QByteArray log(const QByteArray& data) {
    if (data.length() < 256) {
        ...
        return data.left(96) + "... ..." + data.right(96) + " (" +
               QByteArray::number(data.length()) + " bytes)";
    }
}

GLOBAL_STATIC(MPDConnection, instance)

const QString MPDConnection::constModifiedSince =
    QLatin1String("modified-since");
const int MPDConnection::constMaxPqChanges = 1000;
const QString MPDConnection::constStreamsPlayListName =
    QLatin1String("[Radio Streams]");
...
static QByteArray readFromSocket(MpdSocket& socket,
                                 int timeout = constSocketCommsTimeout) {
    QByteArray data;
    int attempt = 0;
    while (QAbstractSocket::ConnectedState == socket.state()) {
        ...
        if (data.endsWith(constOkNlValue) || data.startsWith(constOkValue) ||
            data.startsWith(constAckValue)) {
            break;
        }
    }
...
static MPDConnection::Response readReply(
    MpdSocket& socket, int timeout = constSocketCommsTimeout) {
...
QString MPDConnection::Response::getError(const QByteArray& command) {
    ...
    if (!ok && data.size() > 0) {
        ...
        if (-1 == cmdEnd) {
            return tr("Unknown") + QLatin1String(" (") + command +
                   QLatin1Char(')');
        } else {
            cmdEnd += 2;
            QString rv = data.mid(
                cmdEnd,
                data.length() - (data.endsWith('\n') ? cmdEnd + 1 : cmdEnd));
            if (data.contains("{listplaylists}")) {
                // NOTE: NOT translated, as refers to config item
                return QLatin1String("playlist_directory - ") + rv;
            }

            // If we are reporting a stream error, remove any stream name added
            // by Cantata...
...
QString MPDConnectionDetails::getName() const {
    ...
    return name.isEmpty()
               ? QObject::tr("Default")
               : (name == MPDUser::constName ? MPDUser::translatedName()
                                             : name);
#else
...
QString MPDConnectionDetails::description() const {
    if (hostname.isEmpty()) {
        ...
        return QObject::tr("\"%1\" (%2:%3)", "name (host:port)")
            .arg(getName())
            .arg(hostname)
...
MPDConnectionDetails& MPDConnectionDetails::operator=(
    const MPDConnectionDetails& o) {
    name = o.name;
    hostname = o.hostname;
    port = o.port;
    password = o.password;
...
MPDConnection::MPDConnection()
    ...
    connect(PowerManagement::self(), SIGNAL(resuming()), this,
            SLOT(reconnect()));
#endif
    MPDParseUtils::setSingleTracksFolders(Utils::listToSet(
        Configuration().get("singleTracksFolders", QStringList())));
}
...
bool MPDConnection::localFilePlaybackSupported() const {
    return details.isLocal() ||
           (ver >=
                CANTATA_MAKE_VERSION(
                    0, 19, 0) && /*handlers.contains(QLatin1String("file")) &&*/
            (QLatin1String("127.0.0.1") == details.hostname ||
             QLatin1String("localhost") == details.hostname));
}

MPDConnection::ConnectionReturn MPDConnection::connectToMPD(MpdSocket& socket,
                                                            bool enableIdle) {
    if (QAbstractSocket::ConnectedState != socket.state()) {
        DBUG << (void*)(&socket) << "Connecting"
             << (enableIdle ? "(idle)" : "(std)");
        if (details.isEmpty()) {
        ...
        if (socket.waitForConnected(constSocketCommsTimeout)) {
            ...
            int min, maj, patch;
            if (3 == sscanf(&(recvdata.constData()[7]), "%3d.%3d.%3d", &maj,
                            &min, &patch)) {
                long v =
            ...
            if (!details.partition.isEmpty()) {
                ...
                if (!readReply(socket).ok) {
                    DBUG << (void*)(&socket)
                         << "partition rejected, staying on default";
                }
            }
            ...
            if (enableIdle) {
                ...
                connect(&socket, SIGNAL(readyRead()), this,
                        SLOT(idleDataReady()), Qt::QueuedConnection);
                connect(
            ...
            DBUG << (void*)(&socket) << "Couldn't connect - "
                 << socket.errorString() << socket.error();
...
MPDConnection::ConnectionReturn MPDConnection::convertSocketCode(
    MpdSocket& socket) {
    switch (socket.error()) {
        ...
        default:
            if (QNetworkProxy::DefaultProxy != socket.proxyType() &&
                QNetworkProxy::NoProxy != socket.proxyType()) {
                return MPDConnection::ProxyError;
            }
            if (socket.errorString().contains(QLatin1String("proxy"),
                                              Qt::CaseInsensitive)) {
                return MPDConnection::ProxyError;
            }
...
QString MPDConnection::errorString(ConnectionReturn status) const {
    switch (status) {
        ...
        case Success:
            return QString();
        case Failed:
            return tr("Connection to %1 failed").arg(details.description());
        case ProxyError:
            return tr("Connection to %1 failed - please check your proxy "
                      "settings")
                .arg(details.description());
        case IncorrectPassword:
            return tr("Connection to %1 failed - incorrect password")
                .arg(details.description());
    }
}
...
MPDConnection::ConnectionReturn MPDConnection::connectToMPD() {
    ...
    if (State_Connected == state &&
        (QAbstractSocket::ConnectedState != sock.state() ||
         QAbstractSocket::ConnectedState != idleSocket.state())) {
    ...
    for (int connAttempt = 0; connAttempt < maxConnAttempts; ++connAttempt) {
        if (Success == (status = connectToMPD(sock)) &&
            Success == (status = connectToMPD(idleSocket, true))) {
...
// This function is mainly intended to be called after the computer (laptop) has
// been 'resumed'
void MPDConnection::reconnect() {
    ...
    if (isConnected()) {  // Perhaps the user pressed a button which caused the
                          // reconnect???
        reconnectStart = 0;
        return;
    }
    ...
    switch (status) {
        case Success:
            // Issue #1041 - MPD does not seem to persist user/client made
            // replaygain changes, so use the values read from Cantata's config.
            if (replaygainSupported() && details.applyReplayGain &&
                !details.replayGain.isEmpty()) {
                sendCommand("replay_gain_mode " +
                            details.replayGain.toLatin1());
            }
            serverInfo.detect();
            listPartitions();
            getStatus();
        ...
        case Failed:
            if (0 == reconnectStart || std::abs(now - reconnectStart) < 15) {
                ...
                if (!reconnectTimer) {
                    ...
                    connect(reconnectTimer, SIGNAL(timeout()), this,
                            SLOT(reconnect()), Qt::QueuedConnection);
                }
                if (std::abs(now - reconnectStart) > 1) {
                    emit info(
                        tr("Connecting to %1").arg(details.description()));
                }
...
void MPDConnection::setDetails(const MPDConnectionDetails& d) {
    ...
    const MPDConnectionDetails& det =
        isUser ? MPDUser::self()->details(true) : d;
#else
    ...
    details = det;
    // Issue #1041 - If this is a user MPD, then the call to
    // MPDUser::self()->details() will clear the replayGain setting We can
    // safely use that of the passed in details.
    details.replayGain = d.replayGain;

    if (diffDetails || State_Connected != state) {
        emit connectionChanged(details);
        DBUG << "setDetails" << details.hostname << details.port
             << (details.password.isEmpty() ? false : true);
        if (State_Connected == state && fadingVolume()) {
            sendCommand("stop");
            stopVolumeFade();
        }
        ...
        switch (status) {
            case Success:
                // Issue #1041 - MPD does not seem to persist user/client made
                // replaygain changes, so use the values read from Cantata's
                // config.
                if (replaygainSupported() && details.applyReplayGain &&
                    !details.replayGain.isEmpty()) {
                    sendCommand("replay_gain_mode " +
...
MPDConnection::Response MPDConnection::sendCommand(const QByteArray& command,
    ...
    static bool reconnected =
        false;  // If we reconnect, and send playlistinfo - dont want that call
                // causing reconnects, and recursion!
    DBUG << (void*)(&sock) << "sendCommand:" << log(command) << emitErrors
         << retry;
    ...
    if (QAbstractSocket::ConnectedState != sock.state() && !reconnected) {
        DBUG << (void*)(&sock) << "Socket (state:" << sock.state()
             << ") need to reconnect";
        sock.close();
        ConnectionReturn status = connectToMPD();
        if (Success != status) {
    ...
    if (!response.ok) {
        ...
        if (response.data.isEmpty() && retry &&
            QAbstractSocket::ConnectedState != sock.state() && !reconnected) {
            // Try one more time...
            // This scenario, where socket seems to be closed during/after
            // 'write' seems to occur more often when dynamizer is running.
            // However, simply reconnecting seems to resolve the issue.
            return sendCommand(command, emitErrors, false);
        }
        clearError();
        if (emitErrors) {
            bool emitError = true;
            // Mopidy returns "incorrect arguments" for commands it does not
            // support. The docs state that crossfade and replaygain mode
            // setting commands are not supported. So, if we get this error then
            // just ignore it.
            if (!isMpd() && (command.startsWith("crossfade ") ||
                             command.startsWith("replay_gain_mode "))) {
                emitError = false;
            } else if (isMpd() && command.startsWith("albumart ")) {
                // MPD will report a generic "file not found" error if it can't
                // find album art; this can happen several times in a large
                // playlist so hide this from the GUI (but report it using DBUG
                // here).
                emitError = false;
                const auto start = command.indexOf(' ');
                const auto end = command.lastIndexOf(' ') - start;
                if (start > 0 &&
                    (end > 0 && (start + end) < command.length())) {
            ...
            if (emitError) {
                if ((command.startsWith("add ") ||
                     command.startsWith("command_list_begin\nadd ")) &&
                    -1 != command.indexOf("\"file:///")) {
                    if (details.isLocal() &&
                        -1 != response.data.indexOf("Permission denied")) {
                        emit error(
                            tr("Failed to load. Please check user \"mpd\" has "
                               "read permission."));
                    } else if (!details.isLocal() &&
                               -1 != response.data.indexOf("Access denied")) {
                        emit error(
                            tr("Failed to load. MPD can only play local files "
                               "if connected via a local socket."));
                    } else if (!response.getError(command).isEmpty()) {
                        emit error(tr("MPD reported the following error: %1")
                                       .arg(response.getError(command)));
                    } else {
                        disconnectFromMPD();
                        emit stateChanged(false);
                        emit error(
                            tr("Failed to send command. Disconnected from %1")
                                .arg(details.description()),
                            true);
                    }
                } else if (!response.getError(command).isEmpty()) {
                    emit error(tr("MPD reported the following error: %1")
                                   .arg(response.getError(command)));
                } /*else if ("listallinfo"==command &&
                ver>=CANTATA_MAKE_VERSION(0,18,0)) { disconnectFromMPD(); emit
                stateChanged(false); emit error(tr("Failed to load library.
                Please increase \"max_output_buffer_size\" in MPD's config
                file."));
                } */
                else {
                    disconnectFromMPD();
...
bool MPDConnection::isPlaylist(const QString& file) {
    static QSet<QString> extensions =
        QSet<QString>() << QLatin1String("asx") << QLatin1String("cue")
...
void MPDConnection::add(const QStringList& files, quint32 pos, quint32 size,
                        int action, quint8 priority, bool decreasePriority) {
...
void MPDConnection::add(const QStringList& origList, quint32 pos, quint32 size,
                        int action, const QList<quint8>& priority) {
    add(origList, pos, size, action, priority, false);
}

void MPDConnection::add(const QStringList& origList, quint32 pos, quint32 size,
                        int action, QList<quint8> priority,
                        bool decreasePriority) {
    quint32 playPos = 0;
    if (0 == pos && 0 == size &&
        (AddAfterCurrent == action || AppendAndPlay == action ||
         AddAndPlay == action)) {
    ...
    if (priority.count() <= 1 && files.count() > constMaxFilesPerAddCommand) {
        int numChunks = (files.count() / constMaxFilesPerAddCommand) +
                        (files.count() % constMaxFilesPerAddCommand ? 1 : 0);
        for (int i = 0; i < numChunks; ++i) {
            fileLists.append(files.mid(i * constMaxFilesPerAddCommand,
                                       constMaxFilesPerAddCommand));
        }
    } else {
    ...
    bool usePrio = !priority.isEmpty() && canUsePriority() &&
                   (1 == priority.count() || priority.count() == files.count());
    ...
    for (const QStringList& files : fileLists) {
        ...
        for (int i = 0; i < files.size(); i++) {
            ...
            if (fileName.startsWith(QLatin1String("http://")) &&
                fileName.contains(QLatin1String("cantata=song"))) {
            ...
            if (!havePlaylist) {
                if (0 != size) {
                    send +=
                        "move " + quote(curSize) + " " + quote(curPos) + '\n';
                }
                if (usePrio && !havePlaylist) {
                    send += "prio " +
                            quote(singlePrio || i >= priority.count()
                                      ? singlePrio
                                      : priority.at(i)) +
                            " " + quote(curPos) + " " + quote(curPos) + '\n';
                }
            }
            curSize++;
            curPos++;
    ...
    if (sentOk) {
        ...
        if ((ReplaceAndplay == action || AddAndPlay == action) /*&& addedFile */
            && !files.isEmpty()) {
            // Dont emit error if play fails, might be that playlist was not
            // loaded...
...
void MPDConnection::move(const QList<quint32>& items, quint32 pos,
                         quint32 size) {
...
void MPDConnection::setOrder(const QList<quint32>& items) {
    ...
    if (!send.isEmpty()) {
        send += "command_list_end";
        // If there are more than X changes made to the playqueue, then doing
        // partial updates can hang the UI. Therefore, set
        // lastUpdatePlayQueueVersion to 0 to cause playlistInfo to be used to
...
void MPDConnection::currentSong() {
    ...
    if (response.ok) {
        emit currentSongUpdated(MPDParseUtils::parseSong(
            response.data, MPDParseUtils::Loc_PlayQueue));
    }
}

/*
 * Call "plchangesposid" to recieve a list of positions+ids that have been
 * changed since the last update. If we have ids in this list that we don't know
 * about, then these are new songs - so we call "playlistinfo <pos>" to get the
 * song information.
 *
 * Any songs that are know about, will actually be sent with empty data - as the
 * playqueue model will already hold these songs.
 */
void MPDConnection::playListChanges() {
    DBUG << "playListChanges" << lastUpdatePlayQueueVersion
         << playQueueIds.size();
    ...
    Response status =
        sendCommand("status");  // We need an updated status so as to detect
                                // deletes at end of list...
    ...
    if (response.ok && status.ok && isPlayQueueIdValid()) {
        ...
        QList<MPDParseUtils::IdPos> changes =
            MPDParseUtils::parseChanges(response.data);
        if (!changes.isEmpty()) {
            ...
            for (const MPDParseUtils::IdPos& idp : changes) {
                if (first) {
                    ...
                    if (idp.pos != 0) {
                        for (quint32 i = 0; i < idp.pos; ++i) {
                            Song s;
                            if (i >= (unsigned int)playQueueIds
                                         .count()) {  // Just for safety...
                                playListInfo();
                                return;
                            }
                ...
                if (prevIds.contains(idp.id) && !streamIds.contains(idp.id)) {
                    ...
                    Song s = MPDParseUtils::parseSong(
                        response.data, MPDParseUtils::Loc_PlayQueue);
                    s.id = idp.id;
                    //                     s.pos=idp.pos;
                    songs.append(s);
            ...
            // Dont think this section is ever called, but leave here to be
            // safe!!! For some reason if we have 10 songs in our playlist and
            // we move pos 2 to pos 1, MPD sends all ids from pos 1 onwards
            if (firstPos + changes.size() <= sv.playlistLength &&
                (sv.playlistLength <= (unsigned int)playQueueIds.length())) {
                for (quint32 i = firstPos + changes.size();
                     i < sv.playlistLength; ++i) {
                    Song s;
                    s.id = playQueueIds.at(i);
                    songs.append(s);
                    ids.append(s.id);
...
void MPDConnection::setReplayGain(const QString& v) {
    if (replaygainSupported()) {
        // Issue #1041 - MPD does not seem to persist user/client made
        // replaygain changes, so store in Cantata's config file.
...
void MPDConnection::getReplayGain() {
    if (replaygainSupported()) {
        QStringList lines = QString(sendCommand("replay_gain_status").data)
                                .split('\n', CANTATA_SKIP_EMPTY);

        if (2 == lines.count() && "OK" == lines[1] &&
            lines[0].startsWith(QLatin1String("replay_gain_mode: "))) {
...
void MPDConnection::play() { playFirstTrack(true); }
...
void MPDConnection::clearStopAfter() { toggleStopAfterCurrent(false); }
...
void MPDConnection::getStats() {
    ...
    if (response.ok) {
        ...
        if (isMopidy()) {
            // Set version to 1 so that SQL cache is updated - it uses 0 as
            // intial value
    ...
    if (response.ok) {
        ...
        if (stopAfterCurrent &&
            (currentSongId != sv.songId ||
             (songPos > 0 && sv.timeElapsed < (qint32)songPos))) {
            stopVolumeFade();
...
void MPDConnection::getUrlHandlers() {
    ...
    if (response.ok) {
        handlers = Utils::listToSet(
            MPDParseUtils::parseList(response.data, QByteArray("handler: ")));
        DBUG << handlers;
    }
}
...
void MPDConnection::getTagTypes() {
    ...
    if (response.ok) {
        tagTypes = Utils::listToSet(
            MPDParseUtils::parseList(response.data, QByteArray("tagtype: ")));
    }
}
...
void MPDConnection::getCover(const Song& song) {
    ...
    while (dataToRead != 0) {
        Response response = sendCommand(
            "albumart " + encodeName(path) + " " +
            QByteArray::number(firstRun ? 0 : (imageSize - dataToRead)));
        if (!response.ok) {
        ...
        auto sizeStart =
            strstr(response.data.constData(), constSize.constData());
        if (!sizeStart) {
        ...
        if (firstRun) {
            imageSize = QByteArray(sizeStart + constSize.length(),
                                   sizeEnd - (sizeStart + constSize.length()))
                            .toUInt();
        ...
        int chunkSize =
            QByteArray(chunkSizeStart + constBinary.length(),
                       chunkSizeEnd - (chunkSizeStart + constBinary.length()))
                .toUInt();
 ...
 * handle disconnects.
 */
void MPDConnection::onSocketStateChanged(
    QAbstractSocket::SocketState socketState) {
    if (socketState == QAbstractSocket::ClosingState) {
        bool wasConnected = State_Connected == state;
        disconnect(&idleSocket,
...
void MPDConnection::parseIdleReturn(const QByteArray& data) {
    ...
    for (const QByteArray& line : lines) {
        if (line.startsWith(constIdleChangedKey)) {
            ...
            } else if (!statusUpdated && (constIdlePlayerValue == value ||
                                          constIdleMixerValue == value ||
                                          constIdleOptionsValue == value)) {
                getStatus();
                getReplayGain();
                statusUpdated = true;
...
void MPDConnection::listPartitions() {
    ...
    if (response.ok) {
        ...
        // Send an empty list to indicate lack of partition support for this MPD
        // server.
    ...
    if (response.ok) {
        ...
        if (!details.partition.isEmpty() && details.partition != "default") {
            ...
            if (defaultResponse.ok) {
                ...
                QList<Output> defaultOutputs =
                    MPDParseUtils::parseOuputs(defaultResponse.data);
...
void MPDConnection::enableOutput(quint32 id, bool enable) {
    if (sendCommand((enable ? "enableoutput " : "disableoutput ") + quote(id))
            .ok) {
        outputs();
    }
}
...
void MPDConnection::update() {
    if (isMopidy()) {
        // Mopidy does not support MPD's update command. So, when user presses
        // update DB, what we just reload the library.
        loadLibrary();
    } else {
        sendCommand("update");
    }
...
void MPDConnection::listFolder(const QString& folder) {
    ...
    Response response =
        sendCommand(topLevel ? "lsinfo" : ("lsinfo " + encodeName(folder)));
    QStringList subFolders;
    QList<Song> songs;
    if (response.ok) {
        MPDParseUtils::parseDirItems(response.data, QString(), ver, songs,
                                     folder, subFolders,
...
void MPDConnection::listPlaylists() {
    // Don't report errors here. If user has disabled playlists, then MPD will
    // report an error Issues #1090 #1284
    Response response = sendCommand("listplaylists", false);
    if (response.ok) {
        QList<Playlist> playlists =
            MPDParseUtils::parsePlaylists(response.data);
...
void MPDConnection::playlistInfo(const QString& name) {
    ...
    if (response.ok) {
        emit playlistInfoRetrieved(
            name, MPDParseUtils::parseSongs(response.data,
                                            MPDParseUtils::Loc_Playlists));
    }
}
...
void MPDConnection::renamePlaylist(const QString oldName,
                                   const QString newName) {
    if (sendCommand("rename " + encodeName(oldName) + ' ' + encodeName(newName),
                    false)
            .ok) {
        emit playlistRenamed(oldName, newName);
    } else {
        emit error(tr("Failed to rename <b>%1</b> to <b>%2</b>")
                       .arg(oldName)
                       .arg(newName));
    }
}
    ...
    if (!name.isEmpty()) {
        for (const QString& song : songs) {
            if (CueFile::isCue(song)) {
                emit error(
                    tr("You cannot add parts of a cue sheet to a playlist!") +
                    QLatin1String(" (") + song + QLatin1Char(')'));
                return;
            } else if (isPlaylist(song)) {
                emit error(
                    tr("You cannot add a playlist to another playlist!") +
                    QLatin1String(" (") + song + QLatin1Char(')'));
                return;
            }
        }
...
void MPDConnection::removeFromPlaylist(const QString& name,
...
void MPDConnection::search(const QString& field, const QString& value, int id) {
    ...
    if (field == constModifiedSince) {
        ...
        if (QRegExp("\\d*").exactMatch(value)) {
            ...
            v = QDateTime::currentDateTime().date().startOfDay().toTime_t() -
                (value.toInt() * 24 * 60 * 60);
#else
            v = QDateTime(QDateTime::currentDateTime().date()).toTime_t() -
                (value.toInt() * 24 * 60 * 60);
#endif
        } else if (QRegExp("^((19|20)\\d\\d)[-/](0[1-9]|1[012])[-/"
                           "](0[1-9]|[12][0-9]|3[01])$")
                       .exactMatch(value)) {
            QDateTime dt = QDateTime::fromString(
                QString(value).replace("/", "-"), Qt::ISODate);
            if (dt.isValid()) {
                v = dt.toTime_t();
            }
        }
    ...
    if (!cmd.isEmpty()) {
        ...
        if (response.ok) {
            songs = MPDParseUtils::parseSongs(response.data,
                                              MPDParseUtils::Loc_Search);

            if (QLatin1String("any") == field) {
                // When searching on 'any' MPD ignores filename/paths! So, do
                // another search on these, and combine results.
                response = sendCommand("search file " + encodeName(value));
                if (response.ok) {
                    QList<Song> otherSongs = MPDParseUtils::parseSongs(
                        response.data, MPDParseUtils::Loc_Search);
...
void MPDConnection::search(const QByteArray& query, const QString& id) {
    ...
    if (query.isEmpty()) {
        ...
        if (response.ok) {
            ...
            for (const QByteArray& line : lines) {
                if (line.startsWith("AlbumArtist: ")) {
                    Response resp = sendCommand(
                        "find albumartist " +
                            encodeName(QString::fromUtf8(line.mid(13))),
                        false, false);
                    if (resp.ok) {
                        songs += MPDParseUtils::parseSongs(
                            resp.data, MPDParseUtils::Loc_Search);
                    }
                }
            }
        }
        ...
        if (3 == parts.length()) {
            Response response =
                sendCommand("sticker find song \"\" rating", false, false);
            if (response.ok) {
                int min = parts.at(1).toInt();
                int max = parts.at(2).toInt();
                QList<MPDParseUtils::Sticker> stickers =
                    MPDParseUtils::parseStickers(response.data,
                                                 constRatingSticker);
                if (!stickers.isEmpty()) {
                    for (const MPDParseUtils::Sticker& sticker : stickers) {
                        if (!sticker.file.isEmpty() &&
                            !sticker.value.isEmpty()) {
                            int val = sticker.value.toInt();
                            if (val >= min && val <= max) {
                                Response resp = sendCommand(
                                    "find file " + encodeName(QString::fromUtf8(
                                                       sticker.file)),
                                    false, false);
                                if (resp.ok) {
                                    songs += MPDParseUtils::parseSong(
                                        resp.data, MPDParseUtils::Loc_Search);
                                }
                            }
                        }
...
void MPDConnection::listStreams() {
    Response response = sendCommand(
        "listplaylistinfo " + encodeName(constStreamsPlayListName), false);
    QList<Stream> streams;
    if (response.ok) {
        QList<Song> songs = MPDParseUtils::parseSongs(
            response.data, MPDParseUtils::Loc_Streams);
...
void MPDConnection::saveStream(const QString& url, const QString& name) {
    if (sendCommand("playlistadd " + encodeName(constStreamsPlayListName) +
                    " " +
                    encodeName(MPDParseUtils::addStreamName(url, name, true)))
            .ok) {
...
void MPDConnection::editStream(const QString& url, const QString& name,
                               quint32 position) {
    QByteArray encodedName = encodeName(constStreamsPlayListName);
    if (sendCommand("playlistdelete " + encodedName + " " + quote(position))
            .ok &&
        sendCommand("playlistadd " + encodedName + " " +
                    encodeName(MPDParseUtils::addStreamName(url, name)))
            .ok) {
...
void MPDConnection::sendClientMessage(const QString& channel,
                                      const QString& msg,
                                      const QString& clientName) {
    if (!sendCommand("sendmessage " + channel.toUtf8() + " " + msg.toUtf8(),
                     false)
             .ok) {
        emit error(tr("Failed to send '%1' to %2. Please check %2 is "
                      "registered with MPD.")
                       .arg(msg)
                       .arg(clientName.isEmpty() ? channel : clientName));
        emit clientMessageFailed(channel, msg);
    }
}

void MPDConnection::sendDynamicMessage(const QStringList& msg) {
    // Check whether cantata-dynamic is still alive, by seeing if its channel is
    // still open...
    if (1 == msg.count() && QLatin1String("ping") == msg.at(0)) {
        Response response = sendCommand("channels");
        if (!response.ok || !(Utils::listToSet(MPDParseUtils::parseList(
                                  response.data, QByteArray("channel: "))))
                                 .contains(constDynamicIn)) {
...
void MPDConnection::moveInPlaylist(const QString& name,
                                   const QList<quint32>& items, quint32 pos,
                                   quint32 size) {
...
bool MPDConnection::doMoveInPlaylist(const QString& name,
                                     const QList<quint32>& items, quint32 pos,
                                     quint32 size) {
    if (name.isEmpty()) {
        toggleStopAfterCurrent(false);
    ...
    QByteArray cmd =
        name.isEmpty() ? "move " : ("playlistmove " + encodeName(name) + " ");
    QByteArray send = "command_list_begin\n";
    ...
    for (int i = moveItems.size() - 1; i >= 0; i--) {
        if (moveItems.at(i) < pos && moveItems.at(i) != size - 1) {
            // we are moving away an item that resides before the destination
            // row, manipulate destination row
            posOffset++;
        }
        send += cmd;
        send += quote(moveItems.at(i));
...
bool MPDConnection::recursivelyListDir(const QString& dir, QList<Song>& songs) {
    ...
    if (topLevel && isMpd()) {
        // UPnP database backend does not list separate metadata items, so if
        // "list genre" returns empty response assume this is a UPnP backend and
        // dont attempt to get rest of data... Although we dont use "list XXX",
        // lsinfo will return duplciate items (due to the way most UPnP servers
        // returing directories of classifications - Genre/Album/Tracks,
        // Artist/Album/Tracks, etc...
        ...
        if (!response.ok ||
            ...
            if (!response.ok || response.data.split('\n').length() <
                                    3) {  // 2 lines - OK and blank
                return false;
            }
        }
    }

    Response response = sendCommand(topLevel ? serverInfo.getTopLevelLsinfo()
                                             : ("lsinfo " + encodeName(dir)));
    if (response.ok) {
        QStringList subDirs;
        QList<Song> dirSongs;
        MPDParseUtils::parseDirItems(response.data, details.dir, ver, dirSongs,
                                     dir, subDirs, MPDParseUtils::Loc_Library);
        // If we have only 1 sug dir and its ".cue" then this is (probably)
        // MPD's trat CUE as a directory therefore we ignore any files in this
        // directory as they will be the source files of the CUE
        if (1 != subDirs.size() || !subDirs.at(0).endsWith(".cue")) {
            ...
            DBUG << "IGNORING:" << dirSongs.size()
                 << "track(s) as they are source files of cue?"
                 << subDirs.at(0);
        }
...
QStringList MPDConnection::getPlaylistFiles(const QString& name) {
    ...
    if (response.ok) {
        QList<Song> songs = MPDParseUtils::parseSongs(
            response.data, MPDParseUtils::Loc_Playlists);
...
QStringList MPDConnection::getAllFiles(const QString& dir) {
    ...
    if (response.ok) {
        QStringList subDirs;
        QList<Song> songs;
        MPDParseUtils::parseDirItems(response.data, details.dir, ver, songs,
                                     dir, subDirs, MPDParseUtils::Loc_Browse);
        for (const Song& song : songs) {
...
bool MPDConnection::checkRemoteDynamicSupport() {
    if (ver >= CANTATA_MAKE_VERSION(0, 17, 0)) {
        ...
        if (-1 != idleSocket.write("channels\n")) {
            ...
            if (response.ok) {
                return Utils::listToSet(
                           MPDParseUtils::parseList(response.data,
                                                    QByteArray("channel: ")))
                    .contains(constDynamicIn);
            }
        }
...
bool MPDConnection::subscribe(const QByteArray& channel) {
    if (-1 != idleSocket.write("subscribe \"" + channel + "\"\n")) {
        ...
        if (response.ok || response.data.startsWith(
                               "ACK [56@0]")) {  // ACK => already subscribed...
    ...
    if (-1 != idleSocket.write("readmessages\n")) {
        ...
        if (response.ok) {
            MPDParseUtils::MessageMap messages =
                MPDParseUtils::parseMessages(response.data);
            if (!messages.isEmpty()) {
                QList<QByteArray> channels =
                    QList<QByteArray>()
                    << constDynamicOut << constDynamicOut + '-' + dynamicId;
...
void MPDConnection::setRating(const QString& file, quint8 val) {
    ...
    if (!canUseStickers) {
        emit error(
            tr("Cannot store ratings, as the 'sticker' MPD command is not "
               "supported."));
        return;
    }

    bool ok = 0 == val
                  ? sendCommand("sticker delete song " + encodeName(file) +
                                    ' ' + constRatingSticker,
                                0 != val)
                        .ok
                  : sendCommand("sticker set song " + encodeName(file) + ' ' +
                                constRatingSticker + ' ' + quote(val))
                        .ok;
...
void MPDConnection::setRating(const QStringList& files, quint8 val) {
    ...
    for (const QStringList& list : fileLists) {
        ...
        for (const QString& f : list) {
            if (0 == val) {
                cmd += "sticker delete song " + encodeName(f) + ' ' +
                       constRatingSticker + '\n';
            } else {
                cmd += "sticker set song " + encodeName(f) + ' ' +
                       constRatingSticker + ' ' + quote(val) + '\n';
            }
        }
...
void MPDConnection::getRating(const QString& file) {
    ...
    if (canUseStickers) {
        Response resp = sendCommand(
            "sticker get song " + encodeName(file) + ' ' + constRatingSticker,
            false);
        if (resp.ok) {
            QByteArray val =
                MPDParseUtils::parseSticker(resp.data, constRatingSticker);
            if (!val.isEmpty()) {
                r = val.toUInt();
            }
...
void MPDConnection::getStickerSupport() {
    ...
    canUseStickers =
        response.ok &&
        Utils::listToSet(
            MPDParseUtils::parseList(response.data, QByteArray("command: ")))
            .contains("sticker");
}
...
void MPDConnection::determineIfaceIp() {
    ...
    if (!details.isLocal() && !details.hostname.isEmpty() &&
        ip4Local != details.hostname &&
        QLatin1String("localhost") != details.hostname) {
...
void MpdSocket::connectToHost(const QString& hostName, quint16 port,
    ...
    if (hostName.startsWith('/') ||
        ...
        if (!local) {
            ...
            connect(local, SIGNAL(stateChanged(QLocalSocket::LocalSocketState)),
                    this,
                    SLOT(localStateChanged(QLocalSocket::LocalSocketState)));
...
void MpdSocket::deleteLocal() {
    if (local) {
        disconnect(local, SIGNAL(stateChanged(QLocalSocket::LocalSocketState)),
                   this,
                   SLOT(localStateChanged(QLocalSocket::LocalSocketState)));
...
// ONLY use this method to detect Non-MPD servers. The code which uses this will
// default to MPD
MPDServerInfo::ResponseParameter MPDServerInfo::lsinfoResponseParameters[] = {
    // github
    {"{lsinfo} Directory info not found for virtual-path '/", true,
     MPDServerInfo::ForkedDaapd, "forked-daapd"},
...
void MPDServerInfo::detect() {
    ...
    if (isUndetermined()) {
        ...
        if (response.ok) {
            ...
            if (0 == stats.artists && 0 == stats.albums && 0 == stats.songs &&
                0 == stats.uptime && 0 == stats.playtime &&
                0 == stats.dbPlaytime && 0 == stats.dbUpdate) {
    ...
    if (isUndetermined()) {
        MPDConnection::Response response =
            conn->sendCommand(lsinfoCommand, false, false);
        QList<QByteArray> lines = response.data.split('\n');
        bool match = false;
        unsigned int indx;
        for (const QByteArray& line : lines) {
            for (indx = 0; indx < sizeof(lsinfoResponseParameters) /
                                      sizeof(ResponseParameter);
                 ++indx) {
    ...
    if (isUndetermined()) {
        // Default to MPD if cannot determine otherwise. Cantata is an *MPD*
    ...
    if (isForkedDaapd()) {
        ...
        message += " ";
        message += QString()
                       .number(Song::Rating_Step)
                       .toUtf8();  // rating step (optional)
        message += "\"";
