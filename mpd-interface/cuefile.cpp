...
#define DBUG \
    if (debugEnabled) qWarning() << "CueFile"
void CueFile::enableDebug() { debugEnabled = true; }
...
QByteArray CueFile::getLoadLine(const QString& str) {
    ...
    if (q.hasQueryItem("pos")) {
        ...
        return MPDConnection::encodeName(path) + " \"" + pos.toLatin1() + ":" +
               QString::number(pos.toInt() + 1).toLatin1() + "\"";
    }
...
//   the 2nd item contains the CUE command (which is empty only for the standard
//   'REM comment' commands) the 3rd item contains the remaining part in the
//   line
static QStringList splitCueLine(const QString& line) {
    ...
    reCueLine.setPatternOptions(
        QRegularExpression::
            DotMatchesEverythingOption);  // dot metacharacter (.) match
...
static bool indexTime(const QString& index, const QString& nextIndex,
                      int& time) {
    ...
    if (beginning < 0 || end < 0) {
        DBUG << "Failed to calculate time - index:" << index
             << "nextIndex:" << nextIndex << "beginning:" << beginning
             << "end:" << end;
        return false;
    }
...
// Updates the lastTrackIndex time (in seconds) using the index of the last
// track (song). This one must be used ONLY for the last track (song).
//      note: the last song time will be calculate dinamically later
//            (see: MPDParseUtils::parseDirItems in
...
static bool indexLastTime(const QString& index, double& lastTrackIndex) {
    ...
    if (beginning < 0) {
        DBUG << "Failed to calculate *last* time - index:" << index
             << "beginning:" << beginning;
        return false;
    }
...
static inline bool isVariousArtists(const QString& str) {
    ...
    return QLatin1String("various") == lower ||
           QLatin1String("various artists") == lower;
}
...
// -----
//
// - The official CUESHEET specification does not offer a specific command to
// indicate the album artist;
//   and the MPD documentation says: "albumartist: On multi-artist albums, this
//   is the artist name which shall be used for the whole album (the exact
//   meaning of this tag is not well-defined)."
//
// - The MPD documentation says: "artist: The artist name (its meaning is not
// well-defined; see composer
//   and performer for more specific tags)."
//   The official CUESHEET specification by CDRWIN state the rule: "If the
//   PERFORMER command appears before any TRACK commands, then the string will
//   be encoded as the performer of the entire disc. If the command appears
//   after a TRACK command, then the string will be encoded as the performer of
//   the current track." The PERFORMER command, when present in the header
//   section of a CUE file, is generally and conventionally used with reference
//   to the entire album to indicate the artist (band or singer), or the
//   composer (and not who is performing the song) in the case of genres such is
//   classical music. Therefore:
//   * when in the header section of a CUE file, PERFORMER should mean the
//   artist to whom the album refers to;
//   * when in the tracks section of a CUE file, PERFORMER should mean the
//   artist performing the song.
//
// - MPD has a composer tag, but the official CUESHEET specification does not
// offer a specific command
//   to indicate the composer; nevertheless, especially in the CUE files
//   associated with classical music CDs, the REM COMPOSER (which is a
//   standard-compliant use trick of the REM command) or the COMPOSER (relaxed
//   non-compliant syntax) is sometimes used, mainly in the tracks section.
//   Similar to PERFORMER, a compliant rule for COMPOSER should be: "If the
//   COMPOSER command appears before any TRACK commands, then the string will be
//   encoded as the composer of the entire disc. If the command appears after a
//   TRACK command, then the string will be encoded as the composer of the
//   current track." Therefore:
//   * when in the header section of a CUE file, COMPOSER should mean the
//   composer to whom the album refers to;
//   * when in the tracks section of a CUE file, COMPOSER should mean the
//   composer of the song.
//
bool CueFile::parse(const QString& fileName, const QString& dir,
                    QList<Song>& songList, QSet<QString>& files,
                    double& lastTrackIndex) {
    DBUG << fileName;
    ...
    if (f.open(QIODevice::ReadOnly)) {
        // First attempt to use QTextDecoder to decode cue file contents into a
        // QString
        ...
        for (QTextCodec* codec : codecList()) {
            ...
            if (!decoder.hasFailure()) {
                textStream.reset(
                    new QTextStream(&decoded, QIODevice::ReadOnly));
                break;
            }
        }
        f.close();
        ...
        if (!textStream) {
            ...
            textStream->setCodec(QTextCodec::codecForUtfText(
                f.peek(1024), QTextCodec::codecForName("UTF-8")));
        }
    }
    ...
    // file dir
    QString fileDir =
        fileName.contains("/") ? Utils::getDir(fileName) : QString();

    // vars to store the current values from the lines in the FILE (header)
    // section of the CUE file
    QStringList genre;
    QString album;
    QString albumArtist;
    QString date;
    ...
    QString discNo;
    //// QString remark;
    // auxiliary vars for other data coming from the FILE section of the CUE
    // file
    QString albumComposer;
    // variables to store the current values from the lines in the TRACKs
    // section of the CUE file
    QString file;
    QString trackNo;
    QString trackType;
    ...
    // initialize the hash
    //      note: using reserve() here should NOT be necessary since
    //      QHash'automatically shrinks or grows to provide
    //            good performance without wasting memory (see:
    //            https://doc.qt.io/qt-5/qhash.html#reserve)
    CueTrack cueTrack;

    // vector where to save CUE tracks hashes (Red Book standard: CDDA can
    // contain up to 99 tracks...)
    QVector<CueTrack> tracks;
    // note: reserve() prevent reallocations and memory fragmentation (see:
    ...
    while (!textStream->atEnd()) {
        // read current line removing whitespace characters from the start and
        // the end
        ...
        DBUG << "cmdRem:" << cmdRem << ", cmdCmd:" << cmdCmd
             << ", cmdVal:" << cmdVal;
        ...
        if (cmdCmd == constFile) {
            ...
            // check if is a valid audio file (else if this is a data file, all
            // of its tracks will be ignored...)
            isValidFile = fileType.compare("BINARY", Qt::CaseInsensitive) &&
                          fileType.compare("MOTOROLA", Qt::CaseInsensitive);

            DBUG << "FILE: file:" << file << ", fileType:" << fileType
                 << ", fileValid?" << isValidFile;
            DBUG << "HEADER: genre:" << genre << ", album:" << album
                 << ", discNo:" << discNo << ", year:" << date
            ...
            isCueHeader = false;
            // jump to next line which will be the first in the TRACKs
            // section...
            continue;
        }
        if (isCueHeader) {
            // this is a standard 'REM comment' OR a tricky 'REM COMMENT
            // comment'
            if ((cmdRem == constRemark && cmdCmd.isEmpty()) ||
                cmdCmd == constComment) {
                // skip comments since are not handled by the Cantata library
                // db...
                continue;
                // continue parsing the FILE section (header of the CUE file)...
            } else if (cmdCmd == constGenre) {
                // if GENRE is a list (separated by one of: , ; | \t), then
                // split
            ...
            if (cmdCmd == constTrack) {
                // if there is a *previous* track and is a *valid* track, then
                // finalize it...
                //      note: a track is valid when the file which it belongs is
                //      a valid audio file (not BINARY or MOTOROLA)
                //            AND has an index AND is an audio (AUDIO) track
                // the code section starting below is repeated after, in order
                // to save the last track also
                //      note: Cantata has code (Song::albumArtistOrComposer)
                //      which checks if use artist (performer) or
                //            composer based upon the 'Composer support' tweak
                //            setting...

                // -- start of repeated code --
                if (isValidFile && !index.isEmpty() &&
                    (trackType == constAudioTrackType || trackType.isEmpty())) {
                    ...
                    if (composer.isEmpty() && !albumComposer.isEmpty() &&
                        !isVariousArtists(albumComposer)) {
                    ...
                    DBUG << "file:" << file << ", trackNo:" << trackNo
                         << ", title:" << title << ", type:" << trackType
                         << ", index:" << index << ", artist:" << artist
                         << ", composer:" << composer;
                    // update track hash values
                    //      note: using insert() means that if there is already
                    //      an item with the key, its value is replaced with the
                    //            new value avoiding allocation of unnecessary
                    //            items (see:
                    ...
                    // save track
                    //     note: using append() means growing without
                    //     reallocating the entire vector each time (see:
                    //     https://doc.qt.io/qt-5/qvector.html#append)
                    tracks.append(cueTrack);
                    // clear the state for the next track
                    trackType = trackNo = index = title = artist = composer =
                        QString();
                }
                // -- end of repeated code --

                // here is a new track... get the actual track number and the
                // track type
                if (!cmdVal.isEmpty()) {
                    cmdVal.remove('"').remove("'");
                    QStringList cmdValSplitted = cmdVal.split(QRegExp("\\s+"));
                ...
                if (!cmdVal.isEmpty()) {
                    ...
                    if (cmdValSplitted.size() == 2 &&
                        (cmdValSplitted[0] == QLatin1String("01") ||
                         cmdValSplitted[0] == QLatin1String("1") ||
                         index.isEmpty())) {
    ...
    if (isValidFile && !index.isEmpty() &&
        ...
        DBUG << "file:" << file << ", trackNo:" << trackNo
             ...
             << ", composer:" << composer;
        // update track hash values
        //      note: using insert() means that if there is already an item with
        //      the key, its value is replaced with the
        //            new value avoiding allocation of unnecessary items (see:
        //            https://doc.qt.io/qt-5/qhash.html#insert)
        cueTrack.insert("file", file);
        cueTrack.insert("trackNo", trackNo);
        cueTrack.insert("index", index);
        ...
        cueTrack.insert("composer", composer);
        // save track
        //     note: using append() means growing without reallocating the
        //     entire vector each time (see:
        //     https://doc.qt.io/qt-5/qvector.html#append)
        tracks.append(cueTrack);
        // clear the state for the next track
    ...
    for (int i = 0; i < tracks.size(); i++) {
        // note: using at() to lookup values from the track vector is slightly
        // faster than
        //       using operator[] or value() (see:
        //       https://doc.qt.io/qt-5/qvector.html#value)
        // note: using value() to lookup values from the track hash is the
        // recommended way
        ...
        song.track =
            static_cast<quint8>(tracks.at(i).value("trackNo").toUInt());
        ...
        // set time...
        //      note: the last TRACK for every FILE gets its 'end' marker from
        //      the media file's length
        //            (this will be calculated elsewhere when calling
        //            MPDParseUtils::parseDirItems)
        if (i + 1 < tracks.size() &&
            tracks.at(i).value("file") == tracks[i + 1].value("file")) {
            int time = 0;
            // incorrect indices?
            if (!indexTime(tracks.at(i).value("index"),
                           tracks[i + 1].value("index"), time)) {
                continue;  // if incorrect, then skip the track (jump to the
                           // next one)
            }
        ...
        //        }
        DBUG << "SONG file:" << song.file << ", songFile:" << songFile
             << ", genre:" << song.displayGenre() << ", album:" << song.album
             << ", year:" << song.year << ", originalYear:" << song.origYear
             << ", discNo:" << song.disc << ", trackNo:" << song.track
             << ", title:" << song.title << ", albumArtist:" << song.albumartist
             << ", artist:" << song.artist << ", composer:" << song.composer();
