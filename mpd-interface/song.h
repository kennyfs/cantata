...
struct Song {
    ...
    enum Blank { BlankTitle = 0x01, BlankArtist = 0x02, BlankAlbum = 0x04 };
    ...
    static bool isComposerGenre(const QString& genre) {
        return composerGenres().contains(genre);
    }
    ...
    virtual ~Song() {}
    bool isEmpty() const;
    bool isDifferent(const Song& s) const {
        return file != s.file || year != s.year || track != s.track ||
               disc != s.disc || artist != s.artist || album != s.album ||
               title != s.title || name() != s.name();
    }
    ...
    QString displayTitle() const {
        return !albumartist.isEmpty() && albumartist != artist ? artistSong()
                                                               : title;
    }
    ...
    QString extraField(quint32 f) const {
        return hasExtraField(f) ? extra[f] : QString();
    }
    ...
    void setMbAlbumId(const QString& v) {
        setExtraField(MusicBrainzAlbumId, v);
    }
    ...
    void setAlbumArtistSort(const QString& v) {
        setExtraField(AlbumArtistSort, v);
    }
    bool hasAlbumArtistSort() const { return hasExtraField(AlbumArtistSort); }

    QString artistSortString() const {
        return hasAlbumArtistSort() ? albumArtistSort()
               : hasArtistSort()    ? artistSort()
                                    : QString();
    }
    ...
    bool isDlnaStream() const {
        return Stream == type && !albumArtist().isEmpty() && !album.isEmpty() &&
               track > 0;
    }
    bool isNonMPD() const {
        return isStream() || OnlineSvrTrack == type || Cdda == type ||
               (!file.isEmpty() && file.startsWith(Utils::constDirSep));
    }
    bool hasProtocolOrIsAbsolute() const {
        return !file.isEmpty() && (file.startsWith(Utils::constDirSep) ||
                                   (!file.startsWith(constForkedDaapdLocal) &&
                                    file.contains(":/")));
    }
    bool isCantataStream() const { return CantataStream == type; }
    bool isCdda() const { return Cdda == type; }
    QString albumKey() const;
    bool isCueFile() const {
        return Playlist == type &&
               file.endsWith(QLatin1String(".cue"), Qt::CaseInsensitive);
    }
    ...
    QString displayAlbum(bool useComp = true) const {
        return displayAlbum(useComp ? albumName() : album, displayYear());
    }
    ...
    //
    // The following sections contain various 'hacks' - where fields of Song are
    // abused for other purposes. This is to keep the overall size of Song
    // lower, as its used all over the place...
    //

    // We pass 'Song' around to cover requester. When we want the artist image,
    // and not album image, then we blank certain fields to indicate this!
    ...
    bool isArtistImageRequest() const {
        return 0x1F == disc && 0xFF == priority && 0xFFFF == key &&
               album.isEmpty();
    }
    ...
    bool isComposerImageRequest() const {
        return 0x1E == disc && 0xFE == priority && 0xFEFE == key &&
               album.isEmpty();
    }

    // In Covers, the following is used to indicate that a specfic size is
    // requested...
    void setSpecificSizeRequest(int sz) {
        size = track = id = sz;
        time = 0xFFFF;
    }
    bool isSpecificSizeRequest() const {
        return size > 4 && size < 1024 && track == size && id == size &&
               0xFFFF == time;
    }
    ...
    void setPodcastPublishedDate(const QString& pd) {
        setExtraField(PodcastPublishedDate, pd);
    }
    QString podcastPublishedDate() const {
        return extraField(PodcastPublishedDate);
    }
    ...
    void setIsFromOnlineService(const QString& service) {
        setExtraField(OnlineServiceName, service);
    }
    bool isFromOnlineService() const {
        return hasExtraField(OnlineServiceName);
    }
