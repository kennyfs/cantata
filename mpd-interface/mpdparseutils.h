...
struct IdPos {
    IdPos(qint32 i, quint32 p) : id(i), pos(p) {}
    qint32 id;
    quint32 pos;
};
...
inline Song parseSong(const QByteArray& data, Location location) {
    return parseSong(data.split('\n'), location);
}
...
extern void parseDirItems(const QByteArray& data, const QString& mpdDir,
...
extern QByteArray parseSticker(const QByteArray& data,
                               const QByteArray& sticker);
extern QList<Sticker> parseStickers(const QByteArray& data,
                                    const QByteArray& sticker);
