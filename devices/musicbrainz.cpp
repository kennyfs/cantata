...
static inline int secondsToFrames(int s) { return constFramesPerSecond * s; }
...
MusicBrainz::MusicBrainz(const QString& device) : dev(device) {
    thread = new Thread(metaObject()->className());
    moveToThread(thread);
    thread->start();
}

MusicBrainz::~MusicBrainz() { thread->stop(); }
...
void MusicBrainz::readDisc() {
    ...
    if (ioctl(fd, CDIOCREADSUBCHANNEL, (char*)&cdsc) >= 0 &&
        0 == ioctl(fd, CDIOREADTOCHEADER, &th)) {
        te.address_format = CD_LBA_FORMAT;
        for (int i = th.starting_track; i <= th.ending_track; i++) {
            te.track = i;
            if (0 == ioctl(fd, CDIOREADTOCENTRY, &te)) {
                tracks.append(Track(te.entry.addr.lba + secondsToFrames(2),
                                    te.entry.control & 0x04));
            }
        }
        te.track = 0xAA;
        if (0 == ioctl(fd, CDIOREADTOCENTRY, &te)) {
            tracks.append((ntohl(te.entry.addr.lba) + secondsToFrames(2)) /
                          secondsToFrames(1));
        }
    }
#elif defined(__linux__)
    ...
    if ((CDS_AUDIO == status || CDS_MIXED == status) &&
        0 == ioctl(fd, CDROMREADTOCHDR, &th)) {
        te.cdte_format = CDROM_LBA;
        for (int i = th.cdth_trk0; i <= th.cdth_trk1; i++) {
            te.cdte_track = i;
            if (0 == ioctl(fd, CDROMREADTOCENTRY, &te)) {
                tracks.append(Track(te.cdte_addr.lba + secondsToFrames(2),
    ...
    if (tracks.count() > 1) {
        for (int i = 0; i < tracks.count() - 1; ++i) {
            ...
            s.time = framesToSeconds((next.offset - trk.offset) -
                                     (next.isData ? constDataTrackAdjust : 0));
...
void MusicBrainz::lookup(bool full) {
    ...
    QList<CdAlbum> m;
    QList<QNetworkProxy> proxies = NetworkProxyFactory::self()->queryProxy(
        QNetworkProxyQuery(QUrl("http://musicbrainz.org")));
    ...
    try {
        ...
        if (Metadata.Disc() && Metadata.Disc()->ReleaseList()) {
            ...
            for (int i = 0; i < releaseList->NumItems(); i++) {
                ...
                // The releases returned from LookupDiscID don't contain full
                // information

                MusicBrainz5::CQuery::tParamMap params;
                params["inc"] =
                    "artists labels recordings release-groups url-rels discids "
                    "artist-credits";

                std::string releaseId = release->ID();
                MusicBrainz5::CMetadata Metadata2 =
                    Query.Query("release", releaseId, "", params);

                if (Metadata2.Release()) {
                    MusicBrainz5::CRelease* fullRelease = Metadata2.Release();

                    // However, these releases will include information for all
                    // media in the release So we need to filter out the only
                    // the media we want.
                    MusicBrainz5::CMediumList mediaList =
                        fullRelease->MediaMatchingDiscID(
                            discId.toLatin1().constData());

                    if (mediaList.NumItems() > 0) {
                        DBUG << "Found " << mediaList.NumItems()
                             << " media item(s)";

                        for (int i = 0; i < mediaList.NumItems(); i++) {
                            MusicBrainz5::CMedium* medium = mediaList.Item(i);

                            /*DBUG << "Found media: '" << medium.Title() << "',
                             * position " << medium.Position();*/
                            CdAlbum album;

                            album.name =
                                QString::fromUtf8(fullRelease->Title().c_str());

                            if (fullRelease->MediumList()->NumItems() > 1) {
                                album.name = tr("%1 (Disc %2)")
                                                 .arg(album.name)
                                                 .arg(medium->Position());
                                album.disc = medium->Position();
                            }
                            album.artist = artistFromCreditList(
                                fullRelease->ArtistCredit());
                            album.genre = Song::unknown();

                            QString date =
                                QString::fromUtf8(fullRelease->Date().c_str());
                            ...
                            MusicBrainz5::CTrackList* trackList =
                                medium->TrackList();
                            if (trackList) {
                                for (int i = 0; i < trackList->NumItems();
                                     i++) {
                                    // Ensure we have the same number of tracks
                                    // are read from disc!
                                    if (album.tracks.count() >=
                                        initial.tracks.count()) {
                                        break;
                                    }
                                    MusicBrainz5::CTrack* track =
                                        trackList->Item(i);
                                    MusicBrainz5::CRecording* recording =
                                        track->Recording();
                                    Song song;
                                    ...
                                    song.file =
                                        QString("%1.wav").arg(song.track);

                                    // Prefer title and artist from the track
                                    // credits, but it appears to be empty if
                                    // same as in Recording Noticable in the
                                    // musicbrainztest-fulldate test, where the
                                    // title on the credits of track 18 are
                                    // "Bara om min älskade väntar", but the
                                    // recording has title "Men bara om min
                                    // älskade"
                                    if (recording &&
                                        0 == track->ArtistCredit()) {
                                        song.artist = artistFromCreditList(
                                    ...
                                    if (recording && track->Title().empty()) {
                                        song.title = QString::fromUtf8(
                                            recording->Title().c_str());
                                    } else {
                                        song.title = QString::fromUtf8(
                                            track->Title().c_str());
                                    }
                            ...
                            // Ensure we have the same number of tracks as read
                            // from disc!
                            if (album.tracks.count() < initial.tracks.count()) {
                                for (int i = album.tracks.count();
                                     i < initial.tracks.count(); ++i) {
