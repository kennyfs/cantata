...
bool StretchHeaderView::RestoreState(const QByteArray& data) {
    ...
    if ((magic_number != kMagicNumber &&
         magic_number != constPrevMagicNumber) ||
        s.atEnd()) {
        return false;
    }
    ...
    setSortIndicator(sort_indicator_section,
    ...
    // Set column visible state, visual indices and, if we're not in stretch
    // mode, pixel widths.
    for (int i = 0; i < count() && i < persisted_column_count; ++i) {
        ...
        if (m && magic_number == kMagicNumber) {
            m->setHeaderData(i, Qt::Horizontal, alignments[i],
...
QByteArray StretchHeaderView::SaveState() const {
    ...
    for (int i = 0; i < count(); ++i) {
        ...
        alignments << (m ? m->headerData(i, Qt::Horizontal,
                                         Qt::TextAlignmentRole)
                               .toInt()
                         : -1);
    }
