...
class MusicLibraryItemArtist : public MusicLibraryItemContainer {
   public:
    ...
    MusicLibraryItemArtist(const Song& song,
                           MusicLibraryItemContainer* parent = nullptr);
    ...
    // 'data' could be 'Composer' if we are set to use that, but need to save
    // real artist...
   ...
   private:
    QString
        m_sortString;  // Do we have an actual artist-sort, or is m_sortString
                       // just "Artist, The" ??? - needed for cache saving
