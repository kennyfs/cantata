...
bool MusicLibraryItemArtist::lessThan(const MusicLibraryItem* a,
                                      const MusicLibraryItem* b) {
    const MusicLibraryItemArtist* aa =
        static_cast<const MusicLibraryItemArtist*>(a);
    const MusicLibraryItemArtist* ab =
        static_cast<const MusicLibraryItemArtist*>(b);
...
MusicLibraryItemArtist::MusicLibraryItemArtist(
    const Song& song, MusicLibraryItemContainer* parent)
    : MusicLibraryItemContainer(song.albumArtistOrComposer(), parent),
      m_sortString(song.hasAlbumArtistSort() ? song.albumArtistSort()
                                             : QString()),
...
MusicLibraryItemAlbum* MusicLibraryItemArtist::createAlbum(const Song& s) {
    // If grouping via composer - then album name *might* need to include artist
    // name (if this is different to composer) So, when creating an album entry
    // we need to use the "Album (Artist)" value for display/sort, and still
    // store just "Album" (for saving to cache, tag editing, etc.)
...
Song MusicLibraryItemArtist::coverSong() const {
    Song song;
    song.albumartist = song.title =
        m_itemData;  // If title is empty, then Song::isUnknown() will be
                     // true!!!

    if (childCount()) {
        MusicLibraryItemAlbum* firstAlbum =
            static_cast<MusicLibraryItemAlbum*>(childItem(0));
        MusicLibraryItemSong* firstSong =
            firstAlbum
                ? static_cast<MusicLibraryItemSong*>(firstAlbum->childItem(0))
                : nullptr;
...
MusicLibraryItemAlbum* MusicLibraryItemArtist::getAlbum(
    ...
    if (m_indexes.count() == m_childItems.count()) {
        ...
        if (*idx >= 0 && *idx < m_childItems.count()) {
            MusicLibraryItemAlbum* a =
                static_cast<MusicLibraryItemAlbum*>(m_childItems.at(*idx));
    ...
    for (int i = 0; it != end; ++it, ++i) {
        MusicLibraryItemAlbum* currenAlbum =
            static_cast<MusicLibraryItemAlbum*>(*it);
