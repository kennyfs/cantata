...
QString PlaylistsModel::headerText(int col) {
    switch (col) {
        case COL_TITLE:
            return PlayQueueModel::headerText(PlayQueueModel::COL_TITLE);
        case COL_ARTIST:
            return PlayQueueModel::headerText(PlayQueueModel::COL_ARTIST);
        case COL_ALBUM:
            return PlayQueueModel::headerText(PlayQueueModel::COL_ALBUM);
        case COL_LENGTH:
            return PlayQueueModel::headerText(PlayQueueModel::COL_LENGTH);
        case COL_YEAR:
            return PlayQueueModel::headerText(PlayQueueModel::COL_YEAR);
        case COL_ORIGYEAR:
            return PlayQueueModel::headerText(PlayQueueModel::COL_ORIGYEAR);
        case COL_GENRE:
            return PlayQueueModel::headerText(PlayQueueModel::COL_GENRE);
        case COL_COMPOSER:
            return PlayQueueModel::headerText(PlayQueueModel::COL_COMPOSER);
        case COL_PERFORMER:
            return PlayQueueModel::headerText(PlayQueueModel::COL_PERFORMER);
        case COL_GROUPING:
            return PlayQueueModel::headerText(PlayQueueModel::COL_GROUPING);
        case COL_FILENAME:
            return PlayQueueModel::headerText(PlayQueueModel::COL_FILENAME);
        case COL_PATH:
            return PlayQueueModel::headerText(PlayQueueModel::COL_PATH);
        default:
            return QString();
    }
}
...
PlaylistsModel::PlaylistsModel(QObject* parent)
    ...
    connect(MPDConnection::self(),
            SIGNAL(playlistsRetrieved(const QList<Playlist>&)), this,
            SLOT(setPlaylists(const QList<Playlist>&)));
    connect(MPDConnection::self(),
            SIGNAL(playlistInfoRetrieved(const QString&, const QList<Song>&)),
            this,
            SLOT(playlistInfoRetrieved(const QString&, const QList<Song>&)));
    connect(MPDConnection::self(),
            SIGNAL(removedFromPlaylist(const QString&, const QList<quint32>&)),
            this,
            SLOT(removedFromPlaylist(const QString&, const QList<quint32>&)));
    connect(MPDConnection::self(),
            SIGNAL(playlistRenamed(const QString&, const QString&)), this,
            SLOT(playlistRenamed(const QString&, const QString&)));
    connect(
        MPDConnection::self(),
        SIGNAL(movedInPlaylist(const QString&, const QList<quint32>&, quint32)),
        this,
        SLOT(movedInPlaylist(const QString&, const QList<quint32>&, quint32)));
    connect(this, SIGNAL(listPlaylists()), MPDConnection::self(),
            SLOT(listPlaylists()));
    connect(this, SIGNAL(playlistInfo(const QString&)), MPDConnection::self(),
            SLOT(playlistInfo(const QString&)));
    connect(this,
            SIGNAL(addToPlaylist(const QString&, const QStringList, quint32,
                                 quint32)),
            MPDConnection::self(),
            SLOT(addToPlaylist(const QString&, const QStringList, quint32,
                               quint32)));
    connect(this,
            SIGNAL(moveInPlaylist(const QString&, const QList<quint32>&,
                                  quint32, quint32)),
            MPDConnection::self(),
            SLOT(moveInPlaylist(const QString&, const QList<quint32>&, quint32,
                                quint32)));
    connect(Covers::self(), SIGNAL(loaded(Song, int)), this,
            SLOT(coverLoaded(Song, int)));
    newAction = new QAction(
        MonoIcon::icon(FontAwesome::asterisk, Utils::monoIconColor()),
        tr("New Playlist..."), this);
    ...
    alignments[COL_LENGTH] = alignments[COL_YEAR] = alignments[COL_ORIGYEAR] =
...
QString PlaylistsModel::name() const { return QLatin1String("playlists"); }

QString PlaylistsModel::title() const { return tr("Stored Playlists"); }

QString PlaylistsModel::descr() const { return tr("Standard playlists"); }
...
bool PlaylistsModel::canFetchMore(const QModelIndex& index) const {
    ...
    Item* item = static_cast<Item*>(index.internalPointer());
    return item && item->isPlaylist() &&
           !static_cast<PlaylistItem*>(item)->loaded;
}
...
bool PlaylistsModel::hasChildren(const QModelIndex& parent) const {
    return !parent.isValid() ||
           static_cast<Item*>(parent.internalPointer())->isPlaylist();
}
...
QModelIndex PlaylistsModel::index(int row, int col,
    ...
    if (parent.isValid()) {
        ...
        if (p->isPlaylist()) {
            ...
            return row < pl->songs.count()
                       ? createIndex(row, col, pl->songs.at(row))
                       : QModelIndex();
        }
    }

    return row < items.count() ? createIndex(row, col, items.at(row))
                               : QModelIndex();
}
        ...
        switch (role) {
            ...
            case Cantata::Role_InitiallyHidden:
                return COL_YEAR == section || COL_ORIGYEAR == section ||
                       COL_GENRE == section || COL_COMPOSER == section ||
                       COL_PERFORMER == section || COL_FILENAME == section ||
                       COL_PATH == section || COL_GROUPING == section;
...
bool PlaylistsModel::setHeaderData(int section, Qt::Orientation orientation,
...
QVariant PlaylistsModel::data(const QModelIndex& index, int role) const {
    ...
    if (item->isPlaylist()) {
        ...
        switch (role) {
                ...
                if (multiCol) {
                    switch (index.column()) {
                        ...
                        case COL_LENGTH:
                            ...
                            return pl->loaded && !pl->isSmartPlaylist
                                       ? Utils::formatTime(pl->totalTime())
                                       : QVariant();
                        case COL_YEAR:
                        case COL_ORIGYEAR:
                        case COL_GENRE:
            ...
            case Qt::ToolTipRole:
                ...
                return 0 == pl->songs.count()
                           ? pl->visibleName()
                           : pl->visibleName() + "\n" +
                                 tr("%n Tracks (%1)", "", pl->songs.count())
                                     .arg(Utils::formatTime(pl->totalTime()));
            case Qt::DecorationRole:
                return multiCol ? QVariant()
                                : (pl->isSmartPlaylist
                                       ? Icons::self()->smartPlaylistIcon
            ...
            case Cantata::Role_SubText:
                ...
                return tr("%n Tracks (%1)", "", pl->songs.count())
                    .arg(Utils::formatTime(pl->totalTime()));
        ...
        switch (role) {
            ...
            case Cantata::Role_AlbumDuration: {
                quint32 d = s->time;
                for (int i = index.row() + 1; i < s->parent->songs.count();
                     ++i) {
            ...
            case Qt::DisplayRole:
                if (multiCol) {
                    switch (index.column()) {
                        ...
                        case COL_ARTIST:
                            return s->artist.isEmpty() ? Song::unknown()
                                                       : s->artist;
                        case COL_ALBUM:
            ...
            case Cantata::Role_MainText:
                return s->title.isEmpty()
                           ? s->name().isEmpty() ? s->file : s->name()
                           : s->title;
            case Cantata::Role_SubText:
                return (s->artist.isEmpty() ? QString()
                                            : (s->artist + Song::constSep)) +
                       (s->displayAlbum().isEmpty()
                            ? QString()
                            : (s->displayAlbum() + Song::constSep)) +
...
bool PlaylistsModel::setData(const QModelIndex& index, const QVariant& value,
                             int role) {
...
QStringList PlaylistsModel::filenames(const QModelIndexList& indexes,
    ...
    for (QModelIndex index : indexes) {
        ...
        } else if (!selectedPlaylists.contains(
                       static_cast<SongItem*>(item)->parent)) {
            if (!filesOnly ||
                !static_cast<SongItem*>(item)->file.contains(':')) {
...
QList<Song> PlaylistsModel::songs(const QModelIndexList& indexes) const {
    ...
    for (const QModelIndex& idx : indexes) {
        PlaylistsModel::Item* item =
            static_cast<PlaylistsModel::Item*>(idx.internalPointer());
        if (!item->isPlaylist()) {
            SongItem* song = static_cast<SongItem*>(item);
    ...
    for (QModelIndex index : indexes) {
        ...
        if (item->isPlaylist()) {
            ...
            if (playlist->songs.isEmpty()) {
                filenames << MPDConnection::constPlaylistPrefix +
                                 playlist->name;
            ...
            positions << static_cast<SongItem*>(item)->parent->songs.indexOf(
                static_cast<SongItem*>(item));
        }
    }
...
bool PlaylistsModel::dropMimeData(const QMimeData* data, Qt::DropAction action,
    ...
    if (origRow < 0 && dropAdjust) {  // Unlike the playqueue, we 'copy' items,
                                      // so need to adjust 1 more place...
        dropAdjust++;
    }
    row += dropAdjust;
    ...
    if (data->hasFormat(PlayQueueModel::constFileNameMimeType)) {
        ...
        QStringList filenames = PlayQueueModel::decode(
            *data, PlayQueueModel::constFileNameMimeType);

        if (data->hasFormat(PlayQueueModel::constMoveMimeType)) {
            emit addToPlaylist(
                pl->name, filenames,
                row < 0 || (origRow < 0 && item->isPlaylist()) ? 0 : row,
                row < 0 || (origRow < 0 && item->isPlaylist())
                    ? 0
                    : pl->songs.size());
            return true;
        } else if (data->hasFormat(constPlaylistNameMimeType)) {
            QStringList playlists =
                PlayQueueModel::decode(*data, constPlaylistNameMimeType);
            ...
            if (fromThis) {
                emit moveInPlaylist(
                    pl->name,
                    PlayQueueModel::decodeInts(*data, constPositionsMimeType),
                    row < 0 ? pl->songs.size() : row, pl->songs.size());
                return true;
            } else {
                emit addToPlaylist(
                    pl->name, filenames,
...
void PlaylistsModel::getPlaylists() { emit listPlaylists(); }
...
void PlaylistsModel::setPlaylists(const QList<Playlist>& playlists) {
    if (items.isEmpty()) {
        ...
        if (added.count()) {
            beginInsertRows(parent, items.count(),
                            items.count() + added.count() - 1);
            for (const QString& p : added) {
                int idx = playlists.indexOf(Playlist(p));
                if (-1 != idx) {
                    items.append(
                        new PlaylistItem(playlists.at(idx), allocateKey()));
                }
            }
            endInsertRows();
        }
...
void PlaylistsModel::playlistInfoRetrieved(const QString& name,
...
void PlaylistsModel::removedFromPlaylist(const QString& name,
...
void PlaylistsModel::movedInPlaylist(const QString& name,
                                     const QList<quint32>& idx, quint32 pos) {
    PlaylistItem* pl = nullptr;
    ...
    while (indexes.count()) {
        ...
        if (from != pos) {
            beginMoveRows(
                ...
                pos > from && pos < (quint32)pl->songs.count() ? pos + 1 : pos);
...
PlaylistsModel::PlaylistItem::PlaylistItem(const Playlist& pl, quint32 k)
    ...
    loaded = isSmartPlaylist =
        MPDConnection::self()->isMopidy() && name.startsWith("Smart Playlist:");
    if (isSmartPlaylist) {
        shortName = name.mid(16);
    }
}

PlaylistsModel::PlaylistItem::~PlaylistItem() { clearSongs(); }
...
PlaylistsModel::SongItem* PlaylistsModel::PlaylistItem::getSong(
    const Song& song, int offset) {
    int i = 0;
