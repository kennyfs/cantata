...
const MusicLibraryItemRoot* MusicLibraryModel::root(
    const MusicLibraryItem* item) const {
    ...
    if (MusicLibraryItem::Type_Album == item->itemType()) {
        return static_cast<const MusicLibraryItemRoot*>(
...
QVariant MusicLibraryModel::data(const QModelIndex& index, int role) const {
    ...
    if (Qt::CheckStateRole == role) {
        return static_cast<MusicLibraryItem*>(index.internalPointer())
            ->checkState();
    }
    ...
    switch (role) {
        ...
        case Qt::DisplayRole:
            if (MusicLibraryItem::Type_Song == item->itemType()) {
                ...
                if (Song::Playlist == song->song().type) {
                    return song->song().isCueFile() ? tr("Cue Sheet")
                                                    : tr("Playlist");
                }
        ...
        case Qt::ToolTipRole:
            ...
            if (MusicLibraryItem::Type_Song == item->itemType()) {
                return static_cast<MusicLibraryItemSong*>(item)
                    ->song()
                    .toolTip();
            }
            ...
            return parentData(item) +
                   (0 == item->childCount()
                        ...
                        : (item->displayData(true) + "<br/>" +
                           data(index, Cantata::Role_SubText).toString()));
        case Cantata::Role_SubText:
            switch (item->itemType()) {
                case MusicLibraryItem::Type_Root: {
                    MusicLibraryItemRoot* collection =
                        static_cast<MusicLibraryItemRoot*>(item);
                ...
                case MusicLibraryItem::Type_Album:
                    return tr("%n Tracks (%1)", "",
                              static_cast<MusicLibraryItemAlbum*>(item)
                                  ->trackCount())
                        .arg(Utils::formatTime(
                            static_cast<MusicLibraryItemAlbum*>(item)
                                ->totalTime()));
                default:
                    return QVariant();
        ...
        case Cantata::Role_CoverSong: {
            ...
            switch (item->itemType()) {
                case MusicLibraryItem::Type_Album:
                    v.setValue<Song>(
                        static_cast<MusicLibraryItemAlbum*>(item)->coverSong());
                    break;
                case MusicLibraryItem::Type_Artist:
                    v.setValue<Song>(static_cast<MusicLibraryItemArtist*>(item)
                                         ->coverSong());
                    break;
                default:
                    break;
        ...
        case Cantata::Role_TitleText:
            if (MusicLibraryItem::Type_Album == item->itemType()) {
                return tr("%1 by %2", "Album by Artist")
                    .arg(item->data())
                    .arg(item->parentItem()->data());
            }
            return item->data();
        default:
            break;
...
bool MusicLibraryModel::setData(const QModelIndex& idx, const QVariant& value,
                                int role) {
    if (Qt::CheckStateRole == role) {
        if (!idx.isValid()) {
            return false;
        ...
        switch (item->itemType()) {
            case MusicLibraryItem::Type_Artist: {
                MusicLibraryItemArtist* artistItem =
                    static_cast<MusicLibraryItemArtist*>(item);
                QModelIndex artistIndex =
                    index(artistItem->row(), 0, QModelIndex());
                item->setCheckState(check);
                for (MusicLibraryItem* album : artistItem->childItems()) {
                    if (check != album->checkState()) {
                        MusicLibraryItemAlbum* albumItem =
                            static_cast<MusicLibraryItemAlbum*>(album);
                        QModelIndex albumIndex =
                            index(albumItem->row(), 0, artistIndex);
                        album->setCheckState(check);
                        for (MusicLibraryItem* song : albumItem->childItems()) {
                            song->setCheckState(check);
                        }
                        emit dataChanged(
                            index(0, 0, albumIndex),
                            index(0, albumItem->childCount(), albumIndex));
                    }
                    emit dataChanged(
                        index(0, 0, artistIndex),
                        index(0, artistItem->childCount(), artistIndex));
                }
            ...
            case MusicLibraryItem::Type_Album: {
                MusicLibraryItemArtist* artistItem =
                    static_cast<MusicLibraryItemArtist*>(item->parentItem());
                MusicLibraryItemAlbum* albumItem =
                    static_cast<MusicLibraryItemAlbum*>(item);
                QModelIndex artistIndex =
                    index(artistItem->row(), 0, QModelIndex());
            ...
            case MusicLibraryItem::Type_Song: {
                item->setCheckState(check);
                MusicLibraryItemAlbum* albumItem =
                    static_cast<MusicLibraryItemAlbum*>(item->parentItem());
                MusicLibraryItemArtist* artistItem =
                    static_cast<MusicLibraryItemArtist*>(
...
void MusicLibraryModel::setParentState(const QModelIndex& parent) {
    MusicLibraryItemContainer* parentItem =
        static_cast<MusicLibraryItemContainer*>(parent.internalPointer());
    ...
    for (MusicLibraryItem* child : parentItem->childItems()) {
        switch (child->checkState()) {
            ...
            case Qt::Unchecked:
                ...
                parentCheck =
                    haveCheckedChildren ? Qt::PartiallyChecked : Qt::Unchecked;
                stop = haveCheckedChildren && haveUncheckedChildren;
                break;
            case Qt::Checked:
                haveCheckedChildren = true;
                parentCheck =
                    haveUncheckedChildren ? Qt::PartiallyChecked : Qt::Checked;
                stop = haveCheckedChildren && haveUncheckedChildren;
                break;
        }
        if (stop) {
...
void MusicLibraryModel::setSongs(const QSet<Song>& songs) {
    ...
    //        *artistItem=static_cast<MusicLibraryItemArtist *>(artist);
    //        artistItem->setCheckState(Qt::Checked);

    //        for (MusicLibraryItem *album: artistItem->childItems()) {
    //            MusicLibraryItemAlbum
    //            *albumItem=static_cast<MusicLibraryItemAlbum *>(album);
