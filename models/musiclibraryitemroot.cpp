...
QSet<Song> MusicLibraryItemRoot::allSongs(bool revertVa) const {
    ...
    for (const MusicLibraryItem* child : m_childItems) {
        if (MusicLibraryItem::Type_Song == child->itemType()) {
            if (revertVa) {
                Song s =
                    static_cast<const MusicLibraryItemSong*>(child)->song();
            ...
            for (const MusicLibraryItem* album :
                 static_cast<const MusicLibraryItemContainer*>(child)
                     ->childItems()) {
                for (const MusicLibraryItem* song :
                     static_cast<const MusicLibraryItemContainer*>(album)
                         ->childItems()) {
                    if (revertVa) {
                        Song s = static_cast<const MusicLibraryItemSong*>(song)
                                     ->song();
                        s.revertVariousArtists();
                        songs.insert(s);
                    } else {
                        songs.insert(
                            static_cast<const MusicLibraryItemSong*>(song)
                                ->song());
                    }
                }
            }
...
void MusicLibraryItemRoot::getDetails(QSet<QString>& artists,
            ...
            for (const MusicLibraryItem* album :
                ...
                for (const MusicLibraryItem* song :
                     static_cast<const MusicLibraryItemContainer*>(album)
                         ->childItems()) {
                    const Song& s =
                        static_cast<const MusicLibraryItemSong*>(song)->song();
                    artists.insert(s.artist);
                    albumArtists.insert(s.albumArtist());
                    composers.insert(s.composer());
                    albums.insert(s.album);
                    for (int i = 0;
                         i < Song::constNumGenres && !s.genres[i].isEmpty();
                         ++i) {
                        genres += s.genres[i];
                    }
                }
    ...
    if (art) {
        ...
        if (alb) {
            for (MusicLibraryItem* song : alb->childItems()) {
                if (static_cast<MusicLibraryItemSong*>(song)->file() ==
                    from.file) {
...
void MusicLibraryItemRoot::toXML(const QString& filename,
                                 MusicLibraryProgressMonitor* prog) const {
    if (isFlat) {
        return;
    }
...
static const QString constAlbumArtistSortAttribute =
    QLatin1String("albumartistsort");
...
void MusicLibraryItemRoot::toXML(QXmlStreamWriter& writer,
                                 MusicLibraryProgressMonitor* prog) const {
    if (isFlat) {
        return;
    }
    ...
    for (const MusicLibraryItem* a : childItems()) {
        for (const MusicLibraryItem* al :
             static_cast<const MusicLibraryItemArtist*>(a)->childItems()) {
            total += al->childCount();
    ...
    for (const MusicLibraryItem* a : childItems()) {
        const MusicLibraryItemArtist* artist =
            static_cast<const MusicLibraryItemArtist*>(a);
        for (const MusicLibraryItem* al : artist->childItems()) {
            if (prog && prog->wasStopped()) {
                return;
            }
            const MusicLibraryItemAlbum* album =
                static_cast<const MusicLibraryItemAlbum*>(al);
            for (const MusicLibraryItem* t : album->childItems()) {
                const MusicLibraryItemSong* track =
                    static_cast<const MusicLibraryItemSong*>(t);
                ...
                if (0 != track->time()) {
                    writer.writeAttribute(constTimeAttribute,
                                          QString::number(track->time()));
                }
                if (track->track()) {
                    writer.writeAttribute(constTrackAttribute,
                                          QString::number(track->track()));
                }
                if (track->disc()) {
                    writer.writeAttribute(constDiscAttribute,
                                          QString::number(track->disc()));
                }
                if (!song.artist.isEmpty()) {
                    writer.writeAttribute(constArtistAttribute, song.artist);
                }
                if (!song.albumartist.isEmpty()) {
                    writer.writeAttribute(constAlbumArtistAttribute,
                                          song.albumartist);
                }
                ...
                if (song.hasComposer()) {
                    writer.writeAttribute(constComposerAttribute,
                                          song.composer());
                }
                if (song.hasMbAlbumId()) {
                    writer.writeAttribute(constMbAlbumIdAttribute,
                                          song.mbAlbumId());
                }
                if (song.hasAlbumSort()) {
                    writer.writeAttribute(constAlbumSortAttribute,
                                          song.albumSort());
                }
                if (song.hasArtistSort()) {
                    writer.writeAttribute(constArtistSortAttribute,
                                          song.artistSort());
                }
                if (song.hasAlbumArtistSort()) {
                    writer.writeAttribute(constAlbumArtistSortAttribute,
                                          song.albumArtistSort());
                }
                QString trackGenre = track->song().genres[0];
                for (int i = 1; i < Song::constNumGenres &&
                                !track->song().genres[i].isEmpty();
                     ++i) {
                ...
                if (Song::Playlist == song.type) {
                    writer.writeAttribute(constPlaylistAttribute,
                                          constTrueValue);
                }
                if (song.year) {
                    writer.writeAttribute(constYearAttribute,
                                          QString::number(song.year));
                }
                if (song.guessed) {
                    writer.writeAttribute(constGuessedAttribute,
                                          constTrueValue);
                }
...
bool MusicLibraryItemRoot::fromXML(const QString& filename,
                                   const QString& baseFolder,
                                   MusicLibraryProgressMonitor* prog,
                                   MusicLibraryErrorMonitor* em) {
    if (isFlat) {
        return false;
    }
...
bool MusicLibraryItemRoot::fromXML(QXmlStreamReader& reader,
                                   const QString& baseFolder,
                                   MusicLibraryProgressMonitor* prog,
                                   MusicLibraryErrorMonitor* em) {
    if (isFlat) {
    ...
    while (!reader.atEnd() && (!prog || !prog->wasStopped())) {
        ...
        if (reader.error()) {
            if (em) {
                em->loadError(
                    QObject::tr("Parse error loading cache file, please check "
                                "your songs tags."));
            }
            ...
            if (constTopTag == element) {
                quint32 version =
                    attributes.value(constVersionAttribute).toString().toUInt();
                if (version < constVersion) {
                    return false;
                }
                if (prog) {
                    total = attributes.value(constnumTracksAttribute)
                                .toString()
                                .toUInt();
                }
                valid = true;
                ...
                if (attributes.hasAttribute(constTitleAttribute)) {
                    song.title =
                        attributes.value(constTitleAttribute).toString();
                }
                if (attributes.hasAttribute(constTimeAttribute)) {
                    song.time = attributes.value(constTimeAttribute)
                                    .toString()
                                    .toUInt();
                }
                if (attributes.hasAttribute(constTrackAttribute)) {
                    song.track = attributes.value(constTrackAttribute)
                                     .toString()
                                     .toUInt();
                }
                if (attributes.hasAttribute(constDiscAttribute)) {
                    song.disc = attributes.value(constDiscAttribute)
                                    .toString()
                                    .toUInt();
                }
                if (attributes.hasAttribute(constArtistAttribute)) {
                    song.artist =
                        attributes.value(constArtistAttribute).toString();
                }
                if (attributes.hasAttribute(constAlbumArtistAttribute)) {
                    song.albumartist =
                        attributes.value(constAlbumArtistAttribute).toString();
                }
                if (attributes.hasAttribute(constAlbumAttribute)) {
                    song.album =
                        attributes.value(constAlbumAttribute).toString();
                }
                if (attributes.hasAttribute(constComposerAttribute)) {
                    song.setComposer(
                        attributes.value(constComposerAttribute).toString());
                }
                if (attributes.hasAttribute(constMbAlbumIdAttribute)) {
                    song.setMbAlbumId(
                        attributes.value(constMbAlbumIdAttribute).toString());
                }
                if (attributes.hasAttribute(constAlbumSortAttribute)) {
                    song.setAlbumSort(
                        attributes.value(constAlbumSortAttribute).toString());
                }
                if (attributes.hasAttribute(constArtistSortAttribute)) {
                    song.setArtistSort(
                        attributes.value(constArtistSortAttribute).toString());
                }
                if (attributes.hasAttribute(constAlbumArtistSortAttribute)) {
                    song.setAlbumArtistSort(
                        attributes.value(constAlbumArtistSortAttribute)
                            .toString());
                }
                if (attributes.hasAttribute(constGenreAttribute)) {
                    QStringList genres = attributes.value(constGenreAttribute)
                                             .toString()
                                             .split(",", CANTATA_SKIP_EMPTY);
                    for (int i = 0;
                         i < Song::constNumGenres && i < genres.count(); ++i) {
                        song.addGenre(genres[i]);
                    }
                }
                if (attributes.hasAttribute(constPlaylistAttribute) &&
                    constTrueValue ==
                        attributes.value(constPlaylistAttribute).toString()) {
                    song.type = Song::Playlist;
                }
                if (attributes.hasAttribute(constYearAttribute)) {
                    song.disc = attributes.value(constYearAttribute)
                                    .toString()
                                    .toUInt();
                }
                if (attributes.hasAttribute(constGuessedAttribute) &&
                    constTrueValue ==
                        attributes.value(constGuessedAttribute).toString()) {
                    song.guessed = true;
                }
...
void MusicLibraryItemRoot::add(const QSet<Song>& songs) {
    ...
    for (const Song& s : songs) {
        ...
        if (!artistItem ||
            (supportsAlbumArtist ? s.albumArtist() != artistItem->data()
                                 : s.album != artistItem->data())) {
            artistItem = artist(s);
        }
        if (!albumItem || albumItem->parentItem() != artistItem ||
            s.album != albumItem->data()) {
...
const MusicLibraryItem* MusicLibraryItemRoot::findSong(const Song& s) const {
    if (isFlat) {
        ...
        MusicLibraryItemArtist* artistItem =
            const_cast<MusicLibraryItemRoot*>(this)->artist(s, false);
        if (artistItem) {
            MusicLibraryItemAlbum* albumItem = artistItem->album(s, false);
            if (albumItem) {
                for (const MusicLibraryItem* songItem :
                     albumItem->childItems()) {
                    if (songItem->data() == s.displayTitle() &&
                        static_cast<const MusicLibraryItemSong*>(songItem)
                                ->song()
                                .track == s.track &&
                        static_cast<const MusicLibraryItemSong*>(songItem)
                                ->song()
...
bool MusicLibraryItemRoot::updateSong(const Song& orig, const Song& edit) {
    ...
    if (isFlat) {
        ...
        for (MusicLibraryItem* song : childItems()) {
            if (static_cast<MusicLibraryItemSong*>(song)->song().file ==
                orig.file) {
    ...
    } else if ((supportsAlbumArtist ? orig.albumArtist() == edit.albumArtist()
                                    : orig.artist == edit.artist) &&
               orig.album == edit.album) {
        ...
        for (MusicLibraryItem* song : albumItem->childItems()) {
            if (static_cast<MusicLibraryItemSong*>(song)->song().file ==
                ...
                static_cast<MusicLibraryItemSong*>(song)->setSong(edit);
                bool yearUpdated =
                    orig.year != edit.year && albumItem->updateYear();
                QModelIndex idx = m_model->createIndex(songRow, 0, song);
                emit m_model->dataChanged(idx, idx);
                if (yearUpdated) {
...
void MusicLibraryItemRoot::addSongToList(const Song& s) {
    ...
    if (!albumItem) {
        m_model->beginInsertRows(
            m_model->createIndex(artistItem->row(), 0, artistItem),
            artistItem->childCount(), artistItem->childCount());
    ...
    for (const MusicLibraryItem* songItem : albumItem->childItems()) {
        if (static_cast<const MusicLibraryItemSong*>(songItem)->song().file ==
            s.file) {
            return;
        }
    }
    ...
    m_model->beginInsertRows(
        m_model->createIndex(albumItem->row(), 0, albumItem),
        albumItem->childCount(), albumItem->childCount());
...
void MusicLibraryItemRoot::removeSongFromList(const Song& s) {
    ...
    if (1 == albumItem->childCount()) {
        ...
        m_model->beginRemoveRows(
            m_model->createIndex(artistItem->row(), 0, artistItem), row, row);
    ...
    m_model->beginRemoveRows(
        m_model->createIndex(albumItem->row(), 0, albumItem), songRow, songRow);
    quint32 year = albumItem->year();
...
QString MusicLibraryItemRoot::artistName(const Song& s, bool forceComposer) {
    if (Song::Standard == s.type || Song::Cdda == s.type ||
        Song::OnlineSvrTrack == s.type ||
        (Song::Playlist == s.type && !s.albumArtist().isEmpty())) {
        return forceComposer && s.hasComposer() ? s.composer()
                                                : s.albumArtistOrComposer();
    }
    return Song::variousArtists();
}

QString MusicLibraryItemRoot::songArtist(const Song& s,
                                         bool forceComposer) const {
...
MusicLibraryItemArtist* MusicLibraryItemRoot::getArtist(
    ...
    if (m_indexes.count() == m_childItems.count()) {
        ...
        if (*idx >= 0 && *idx < m_childItems.count()) {
            MusicLibraryItemArtist* a =
                static_cast<MusicLibraryItemArtist*>(m_childItems.at(*idx));
    ...
    for (int i = 0; it != end; ++it, ++i) {
        MusicLibraryItemArtist* currenArtist =
            static_cast<MusicLibraryItemArtist*>(*it);
