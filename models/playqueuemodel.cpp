...
static QSet<QString> constM3uPlaylists =
    QSet<QString>() << QLatin1String("m3u") << QLatin1String("m3u8");
static const QString constPlsPlaylist = QLatin1String("pls");
static const QString constXspfPlaylist = QLatin1String("xspf");
QSet<QString> PlayQueueModel::constFileExtensions =
    QSet<QString>() << QLatin1String("mp3") << QLatin1String("ogg")
                    << QLatin1String("flac") << QLatin1String("wma")
                    << QLatin1String("m4a") << QLatin1String("m4b")
                    << QLatin1String("mp4") << QLatin1String("m4p")
                    << QLatin1String("wav") << QLatin1String("wv")
                    << QLatin1String("wvp") << QLatin1String("aiff")
                    << QLatin1String("aif") << QLatin1String("aifc")
                    << QLatin1String("ape") << QLatin1String("spx")
                    << QLatin1String("tta") << QLatin1String("mpc")
                    << QLatin1String("mpp") << QLatin1String("mp+")
                    << QLatin1String("dff") << QLatin1String("dsf")
                    << QLatin1String("opus")
                    // And playlists...
                    << QLatin1String("m3u") << QLatin1String("m3u8")
                    << constPlsPlaylist << constXspfPlaylist;
...
static inline bool checkExtension(
    const QString& file,
    const QSet<QString>& exts = PlayQueueModel::constFileExtensions) {
...
static QString checkUrl(const QString& url, const QDir& dir, bool useServer,
                        bool useLocal, const QSet<QString>& handlers) {
    int pos = url.indexOf(QLatin1String("://"));
    QString handler = pos > 0 ? url.left(pos + 3).toLower() : QString();
    if (!handler.isEmpty() && (QLatin1String("http://") == handler ||
                               QLatin1String("https://") == handler)) {
        // Radio stream?
...
static QStringList expandM3uPlaylist(const QString& playlist, bool useServer,
                                     bool useLocal,
                                     const QSet<QString>& handlers) {
    QStringList files;
    QFile f(playlist);
    ...
    if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            if (!line.startsWith(QLatin1Char('#'))) {
                QString url =
                    checkUrl(line, dir, useServer, useLocal, handlers);
                if (!url.isEmpty()) {
                    files.append(url);
                }
            }
...
static QStringList expandPlsPlaylist(const QString& playlist, bool useServer,
                                     bool useLocal,
                                     const QSet<QString>& handlers) {
    QStringList files;
    QFile f(playlist);
        ...
        while (!in.atEnd()) {
            ...
            if (line.startsWith(QLatin1String("File"))) {
                ...
                if (2 == parts.length()) {
                    ...
                    QString url =
                        checkUrl(path, dir, useServer, useLocal, handlers);
                    if (!url.isEmpty()) {
                        urls.insert(num, url);
                    }
                }
                ...
                QStringList parts = line.split("=");
                if (2 == parts.length()) {
                    titles.insert(parts[0].left(5).toUInt(),
                                  parts[1].trimmed());
                }
            }
        }
    ...
    for (; it != end; ++it) {
        if (titles.contains(it.key()) &&
            (it.value().startsWith(QLatin1String("http://")) ||
...
static QStringList expandXspfPlaylist(const QString& playlist, bool useServer,
                                      bool useLocal,
                                      const QSet<QString>& handlers) {
    QStringList files;
    QFile f(playlist);
    ...
    if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!reader.atEnd()) {
            ...
            if (QXmlStreamReader::StartElement == reader.tokenType() &&
                QLatin1String("location") == reader.name()) {
                QString url = checkUrl(reader.readElementText().trimmed(), dir,
                                       useServer, useLocal, handlers);
                if (!url.isEmpty()) {
                    files.append(url);
                }
            }
...
static QStringList listFiles(const QDir& d, bool useServer, bool useLocal,
                             const QSet<QString>& handlers, int level = 5) {
    QStringList files;
    if (level) {
        for (const auto& f :
             d.entryInfoList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot |
                                 QDir::NoSymLinks,
                             QDir::LocaleAware | QDir::IgnoreCase)) {
            if (f.isDir()) {
                files += listFiles(QDir(f.absoluteFilePath()), useServer,
                                   useLocal, handlers, level - 1);
            } else if (checkExtension(f.fileName(), constM3uPlaylists)) {
                files += expandM3uPlaylist(f.absoluteFilePath(), useServer,
                                           useLocal, handlers);
            } else if (constPlsPlaylist == getExtension(f.fileName())) {
                files += expandPlsPlaylist(f.absoluteFilePath(), useServer,
                                           useLocal, handlers);
            } else if (constXspfPlaylist == getExtension(f.fileName())) {
                files += expandXspfPlaylist(f.absoluteFilePath(), useServer,
                                            useLocal, handlers);
            } else if (checkExtension(f.fileName())) {
                QString fUrl =
                    fileUrl(f.absoluteFilePath(), useServer, useLocal);
                if (!fUrl.isEmpty()) {
                    files.append(fUrl);
                }
            }
...
QStringList PlayQueueModel::parseUrls(const QStringList& urls) {
    ...
    for (const auto& path : urls) {
        QUrl u =
            path.indexOf("://") > 2 ? QUrl(path) : QUrl::fromLocalFile(path);
    ...
    (defined CDDB_FOUND || defined MUSICBRAINZ5_FOUND)
        ...
        if (!cdDevice.isEmpty()) {
            ...
            if (d.exists()) {
                ...
                useable +=
                    expandM3uPlaylist(u.path(), useServer, useLocal, handlers);
            } else if (constPlsPlaylist == getExtension(u.path())) {
                useable +=
                    expandPlsPlaylist(u.path(), useServer, useLocal, handlers);
            } else if (constXspfPlaylist == getExtension(u.path())) {
                useable +=
                    expandXspfPlaylist(u.path(), useServer, useLocal, handlers);
    ...
    if (haveLocalFiles && !useServer && !useLocal) {
        ...
        emit error(
            tr("Cannot add local files. Please enable in-built HTTP server, or "
               "configure MPD for local file playback."));
#else
        emit error(
            tr("Cannot add local files. Please configure MPD for local file "
               "playback."));
#endif
        return use;
    }
...
QStringList PlayQueueModel::decode(const QMimeData& mimeData,
                                   const QString& mime) {
...
QList<quint32> PlayQueueModel::decodeInts(const QMimeData& mimeData,
                                          const QString& mime) {
    QByteArray encodedData = mimeData.data(mime);
...
void PlayQueueModel::setSimpleSort(bool simple) { simpleSort = simple; }
...
PlayQueueModel::PlayQueueModel(QObject* parent)
    ...
    connect(fetcher, SIGNAL(result(const QStringList&, int, int, quint8, bool)),
            SLOT(addFiles(const QStringList&, int, int, quint8, bool)));
    connect(fetcher, SIGNAL(result(const QStringList&, int, int, quint8, bool)),
            SIGNAL(streamsFetched()));
    connect(fetcher, SIGNAL(status(QString)),
            SIGNAL(streamFetchStatus(QString)));
    connect(this,
            SIGNAL(filesAdded(const QStringList, quint32, quint32, int, quint8,
                              bool)),
            MPDConnection::self(),
            SLOT(add(const QStringList, quint32, quint32, int, quint8, bool)));
    connect(this, SIGNAL(populate(QStringList, QList<quint8>)),
            MPDConnection::self(), SLOT(populate(QStringList, QList<quint8>)));
    connect(this, SIGNAL(move(const QList<quint32>&, quint32, quint32)),
            MPDConnection::self(),
            SLOT(move(const QList<quint32>&, quint32, quint32)));
    connect(this, SIGNAL(setOrder(const QList<quint32>&)),
            MPDConnection::self(), SLOT(setOrder(const QList<quint32>&)));
    connect(MPDConnection::self(),
            SIGNAL(prioritySet(const QMap<qint32, quint8>&)),
            SLOT(prioritySet(const QMap<qint32, quint8>&)));
    connect(MPDConnection::self(), SIGNAL(stopAfterCurrentChanged(bool)),
            SLOT(stopAfterCurrentChanged(bool)));
    connect(this, SIGNAL(stop(bool)), MPDConnection::self(),
            SLOT(stopPlaying(bool)));
    connect(this, SIGNAL(clearStopAfter()), MPDConnection::self(),
            SLOT(clearStopAfter()));
    connect(this, SIGNAL(removeSongs(QList<qint32>)), MPDConnection::self(),
            SLOT(removeSongs(QList<qint32>)));
    connect(this, SIGNAL(clearEntries()), MPDConnection::self(), SLOT(clear()));
    connect(this, SIGNAL(addAndPlay(QString)), MPDConnection::self(),
            SLOT(addAndPlay(QString)));
    ...
    connect(this, SIGNAL(setRating(QStringList, quint8)), MPDConnection::self(),
            SLOT(setRating(QStringList, quint8)));
    connect(MPDConnection::self(), SIGNAL(rating(QString, quint8)),
            SLOT(ratingResult(QString, quint8)));
    connect(MPDConnection::self(), SIGNAL(stickerDbChanged()),
            SLOT(stickerDbChanged()));
#ifdef ENABLE_DEVICES_SUPPORT  // TODO: Problems here with devices support!!!
    connect(DevicesModel::self(), SIGNAL(updatedDetails(QList<Song>)),
            SLOT(updateDetails(QList<Song>)));
#endif
    ...
    undoAction = ActionCollection::get()->createAction(
        "playqueue-undo", tr("Undo"), MonoIcon::icon(FontAwesome::undo, col));
    undoAction->setShortcut(Qt::ControlModifier + Qt::Key_Z);
    redoAction = ActionCollection::get()->createAction(
        "playqueue-redo", tr("Redo"), MonoIcon::icon(FontAwesome::repeat, col));
    redoAction->setShortcut(Qt::ControlModifier + Qt::ShiftModifier +
                            Qt::Key_Z);
    connect(undoAction, SIGNAL(triggered()), this, SLOT(undo()));
    connect(redoAction, SIGNAL(triggered()), this, SLOT(redo()));
    connect(removeDuplicatesAction, SIGNAL(triggered()), this,
    ...
    connect(shuffleTracksAction, SIGNAL(triggered()), MPDConnection::self(),
            SLOT(shuffle()));
    connect(shuffleAlbumsAction, SIGNAL(triggered()), this,
            SLOT(shuffleAlbums()));
    ...
    alignments[COL_TITLE] = alignments[COL_ARTIST] = alignments[COL_ALBUM] =
        alignments[COL_GENRE] = alignments[COL_COMPOSER] =
            alignments[COL_PERFORMER] = alignments[COL_FILENAME] =
                alignments[COL_PATH] = alignments[COL_GROUPING] =
                    int(Qt::AlignVCenter | Qt::AlignLeft);
    alignments[COL_TRACK] = alignments[COL_LENGTH] = alignments[COL_DISC] =
        alignments[COL_YEAR] = alignments[COL_ORIGYEAR] = alignments[COL_PRIO] =
...
QModelIndex PlayQueueModel::index(int row, int column,
                                  const QModelIndex& parent) const {
    return hasIndex(row, column, parent)
               ? createIndex(row, column, (void*)&songs.at(row))
               : QModelIndex();
}
...
QVariant PlayQueueModel::headerData(int section, Qt::Orientation orientation,
                                    int role) const {
    if (Qt::Horizontal == orientation) {
        switch (role) {
            ...
            case Cantata::Role_InitiallyHidden:
                return COL_YEAR == section || COL_ORIGYEAR == section ||
                       COL_DISC == section || COL_GENRE == section ||
                       COL_PRIO == section || COL_COMPOSER == section ||
                       COL_PERFORMER == section || COL_RATING == section ||
                       COL_FILENAME == section || COL_PATH == section ||
                       COL_GROUPING == section;
            ...
            case Cantata::Role_ContextMenuText:
                return COL_TRACK == section ? tr("# (Track Number)")
                                            : headerText(section);
            default:
                break;
        }
...
bool PlayQueueModel::setHeaderData(int section, Qt::Orientation orientation,
                                   const QVariant& value, int role) {
    if (Qt::Horizontal == orientation && Qt::TextAlignmentRole == role &&
        section >= 0) {
...
QVariant PlayQueueModel::data(const QModelIndex& index, int role) const {
    ...
    switch (role) {
        ...
        case Cantata::Role_Song: {
            QVariant var;
            const Song& s = songs.at(index.row());
            if (Cantata::Role_SongWithRating == role &&
                Song::Standard == s.type && Song::Rating_Null == s.rating) {
                ...
                if (s.id == currentSongId) {
                    switch (mpdState) {
                        ...
                        case MPDState_Stopped:
                            return (int)GroupedView::State_Stopped;
                        case MPDState_Playing:
                            return (int)(stopAfterCurrent
                                             ? GroupedView::State_StopAfter
                                             : GroupedView::State_Playing);
                        case MPDState_Paused:
                            return (int)GroupedView::State_Paused;
                    }
            ...
            switch (index.column()) {
                case COL_TITLE:
                    return song.title.isEmpty()
                               ? Utils::getFile(basicPath(song))
                               : song.title;
                case COL_ARTIST:
                    return song.artist.isEmpty() ? Song::unknown()
                                                 : song.artist;
                case COL_ALBUM:
        ...
        case Cantata::Role_Decoration: {
            ...
            if (id == currentSongId) {
                switch (mpdState) {
                    case MPDState_Inactive:
                    case MPDState_Stopped:
                        return MonoIcon::icon(FontAwesome::stop,
                                              Utils::monoIconColor());
                    case MPDState_Playing:
                        return MonoIcon::icon(stopAfterCurrent
                                                  ? FontAwesome::playcircle
                                                  : FontAwesome::play,
                                              Utils::monoIconColor());
                    case MPDState_Paused:
                        return MonoIcon::icon(FontAwesome::pause,
                                              Utils::monoIconColor());
                }
            } else if (-1 != id && id == stopAfterTrackId) {
                return MonoIcon::icon(FontAwesome::stop,
...
bool PlayQueueModel::setData(const QModelIndex& index, const QVariant& value,
                             int role) {
...
QStringList PlayQueueModel::mimeTypes() const {
    return QStringList() << constMoveMimeType << constFileNameMimeType
                         << constUriMimeType;
}
 ...
 */
bool PlayQueueModel::dropMimeData(const QMimeData* data, Qt::DropAction action,
                                  int row, int /*column*/,
    ...
    if (data->hasFormat(constMoveMimeType)) {  // Act on internal moves
        if (row < 0) {
            emit move(decodeInts(*data, constMoveMimeType), songs.size(),
                      songs.size());
        } else {
...
void PlayQueueModel::load(const QStringList& urls, int action, quint8 priority,
                          bool decreasePriority) {
    if (-1 == action) {
        action = songs.isEmpty() ? MPDConnection::AppendAndPlay
                                 : MPDConnection::Append;
    }
...
void PlayQueueModel::addItems(const QStringList& items, int row, int action,
...
void PlayQueueModel::addFiles(const QStringList& filenames, int row, int action,
                              quint8 priority, bool decreasePriority) {
    if (MPDConnection::ReplaceAndplay == action) {
        emit filesAdded(filenames, 0, 0, MPDConnection::ReplaceAndplay,
...
void PlayQueueModel::updateCurrentSong(quint32 id) {
    ...
    if (-1 != oldIndex) {
        int row =
            -1 == currentSongRowNum ? getRowById(oldIndex) : currentSongRowNum;
    ...
    emit dataChanged(index(currentSongRowNum, 0),
                     index(currentSongRowNum, columnCount(QModelIndex()) - 1));
...
void PlayQueueModel::update(const QList<Song>& songList, bool isComplete) {
    ...
    // If we have too many changes UI can hang, so it is sometimes better just
        ...
        for (qint32 i = 0; i < songList.count(); ++i) {
            ...
            if (newSong || s.id != currentSongAtPos.id) {
                ...
                if (-1 == existingPos) {
                    ...
                    beginMoveRows(QModelIndex(), existingPos, existingPos,
                                  QModelIndex(), i > existingPos ? i + 1 : i);
                ...
                songs.replace(i, s);
                if (s.title != currentSongAtPos.title ||
                    s.artist != currentSongAtPos.artist ||
                    s.name() != currentSongAtPos.name()) {
...
void PlayQueueModel::setStopAfterTrack(qint32 track) {
    bool clear = track == stopAfterTrackId ||
                 (track == currentSongId && stopAfterCurrent);
...
void PlayQueueModel::removeAll() { emit clearEntries(); }
...
static bool otherSort(const Song* s1, const Song* s2, quint32 other) {
    const QString v1 =
        s1->hasExtraField(other) ? s1->extraField(other) : QString();
    const QString v2 =
        s2->hasExtraField(other) ? s2->extraField(other) : QString();
...
static bool albumArtistSort(const Song* s1, const Song* s2) {
    const QString v1 = s1->hasAlbumArtistSort() ? s1->albumArtistSort()
                                                : s1->albumArtistOrComposer();
    const QString v2 = s2->hasAlbumArtistSort() ? s2->albumArtistSort()
                                                : s2->albumArtistOrComposer();
    int c = Utils::compare(v1, v2);
    if (simpleSort) {
        return c < 0;
    }
...
void PlayQueueModel::removeDuplicates() {
    ...
    for (const Song& song : songs) {
        map[song.albumKey() + "-" + song.artistSong() + "-" + song.track]
            .append(song);
    }
...
void PlayQueueModel::stickerDbChanged() {
    ...
    for (const Song& song : songs) {
        if (Song::Standard == song.type && song.rating <= Song::Rating_Max &&
            !requests.contains(song.file)) {
            emit getRating(song.file);
...
void PlayQueueModel::cancelStreamFetch() { fetcher->cancel(); }

static bool songSort(const Song* s1, const Song* s2) {
    return s1->disc < s2->disc || (s1->disc == s2->disc &&
                                   (s1->track < s2->track ||
                                    (s1->track == s2->track && (*s1) < (*s2))));
}
...
void PlayQueueModel::shuffleAlbums() {
    ...
    while (!keys.isEmpty()) {
        quint32 key =
            keys.takeAt(QRandomGenerator::global()->bounded(keys.count()));
...
void PlayQueueModel::updateDetails(const QList<Song>& updated) {
    ...
    for (int i = 0; i < songs.count(); ++i) {
        ...
        if (songMap.contains(current.file)) {
            ...
            if (updatedSong.title != current.title ||
                updatedSong.artist != current.artist ||
                updatedSong.name() != current.name()) {
