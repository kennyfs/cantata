...
void UltimateLyricsProvider::enableDebug() { debugEnabled = true; }
...
static QString extract(const QString& source, const QString& begin,
                       const QString& end, bool isTag = false) {
    ...
    if (-1 == beginIdx && isTag) {
        beginIdx =
            source.indexOf(QString(begin).remove(">"), 0, Qt::CaseInsensitive);
        skipTagClose = true;
    }
    if (-1 == beginIdx) {
...
static QString exclude(const QString& source, const QString& begin,
    ...
    int endIdx =
        source.indexOf(end, beginIdx + begin.length(), Qt::CaseInsensitive);
    if (-1 == endIdx) {
        return source;
    }

    return source.left(beginIdx) +
           source.right(source.length() - endIdx - end.length());
}
...
static void applyExtractRule(const UltimateLyricsProvider::Rule& rule,
                             QString& content, const Song& song) {
    for (const UltimateLyricsProvider::RuleItem& item : rule) {
        if (item.second.isNull()) {
            content = extractXmlTag(content, doTagReplace(item.first, song));
        } else {
            content = extract(content, doTagReplace(item.first, song),
                              doTagReplace(item.second, song));
        }
    }
}

static void applyExcludeRule(const UltimateLyricsProvider::Rule& rule,
                             QString& content, const Song& song) {
    for (const UltimateLyricsProvider::RuleItem& item : rule) {
        if (item.second.isNull()) {
            content = excludeXmlTag(content, doTagReplace(item.first, song));
        } else {
            content = exclude(content, doTagReplace(item.first, song),
                              doTagReplace(item.second, song));
        }
    }
}
...
UltimateLyricsProvider::~UltimateLyricsProvider() { abort(); }
...
void UltimateLyricsProvider::fetchInfo(int id, Song metadata, bool removeThe) {
    const QTextCodec* codec =
        QTextCodec::codecForName(charset.toLatin1().constData());
    if (!codec) {
    ...
    if (QLatin1String("lyrics.wikia.com") == name) {
        ...
        connect(reply, SIGNAL(finished()), this,
                SLOT(wikiMediaSearchResponse()));
        return;
    }

    metadata.priority =
        removeThe ? 1
                  : 0;  // HACK Use this to indicate if searching without 'The '
    ...
    if (urlContainsDetails) {
        ...
        doUrlReplace(constArtistLowerNoSpaceArg, noSpace(artistFixed.toLower()),
                     urlText);
        ...
        doUrlReplace(constAlbumLowerNoSpaceArg,
                     noSpace(metadata.album.toLower()), urlText);
    ...
    // For some reason Qt messes up the ? -> %3F and & -> %26 conversions - by
    // placing 25 after the % So, try and revert this...
    QUrl url(urlText);
...
void UltimateLyricsProvider::wikiMediaSearchResponse() {
    ...
    if (url.isValid()) {
        ...
        QByteArray u = url.scheme().toLatin1() + "://" + url.host().toLatin1() +
                       "/api.php?action=query&prop=revisions&rvprop=content&"
                       "format=xml&titles=";
        QByteArray titles =
            QUrl::toPercentEncoding(
                ...
                .replace('+', "%2b");
        NetworkJob* reply =
            NetworkAccessManager::self()->get(QUrl::fromEncoded(u + titles));
        requests[reply] = id;
        connect(reply, SIGNAL(finished()), this,
                SLOT(wikiMediaLyricsFetched()));
    } else {
        emit lyricsReady(id, QString());
    }
}
...
void UltimateLyricsProvider::wikiMediaLyricsFetched() {
    ...
    const QTextCodec* codec =
        QTextCodec::codecForName(charset.toLatin1().constData());
    QString contents =
        codec->toUnicode(reply->readAll()).replace("<br />", "<br/>");
    DBUG << name << "response" << contents;
    emit lyricsReady(id, extract(contents, QLatin1String("&lt;lyrics&gt;"),
...
void UltimateLyricsProvider::doUrlReplace(const QString& tag,
                                          const QString& value,
                                          QString& u) const {
    if (!u.contains(tag)) {
        return;
