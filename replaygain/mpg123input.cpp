...
struct Mpg123Input::Handle {
    Handle() : mpg123(0), rate(0), channels(0), encoding(0), buffer(0) {}
    ~Handle() {
        if (buffer) {
            free(buffer);
        }
...
Mpg123Input::Mpg123Input(const QString& fileName) {
    ...
    if (handle->mpg123 &&
        ...
        MPG123_OK == mpg123_getformat(handle->mpg123, &handle->rate,
                                      &handle->channels, &handle->encoding) &&
        MPG123_OK == mpg123_format_none(handle->mpg123) &&
        MPG123_OK == mpg123_format(handle->mpg123, handle->rate,
                                   handle->channels, MPG123_ENC_FLOAT_32)) {
...
float* Mpg123Input::buffer() const { return handle ? handle->buffer : 0; }
...
size_t Mpg123Input::readFrames() {
    ...
    int result = mpg123_read(
        handle->mpg123, (unsigned char*)handle->buffer,
        (size_t)handle->rate * (size_t)handle->channels * sizeof(float),
        &numberRead);
