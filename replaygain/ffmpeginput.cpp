...
#define BUFFER_SIZE                                                 \
    ((((AVCODEC_MAX_AUDIO_FRAME_SIZE * 3) / 2) * sizeof(int16_t)) + \
     FFMPEG_INPUT_BUFFER_PADDING_SIZE)

#if LIBAVCODEC_VERSION_INT >= \
    AV_VERSION_INT(57, 89, 100)  // Not 100% of version here!
...
FfmpegInput::FfmpegInput(const QString& fileName) {
    ...
    if (avformat_open_input(&handle->formatContext,
                            QFile::encodeName(fileName).constData(), NULL,
                            NULL) != 0)
#else
    if (av_open_input_file(&handle->formatContext,
                           QFile::encodeName(fileName).constData(), NULL, 0,
                           NULL) != 0)
#endif
    {
        mutex.unlock();
...
#if LIBAVCODEC_VERSION_INT >= \
    AV_VERSION_INT(57, 89, 100)  // Not 100% of version here!
        handle->codec = avcodec_find_decoder(
            handle->formatContext->streams[handle->audioStream]
                ->codecpar->codec_id);
        handle->codecContext = avcodec_alloc_context3(handle->codec);
        if (avcodec_parameters_to_context(
                handle->codecContext,
                handle->formatContext->streams[handle->audioStream]->codecpar) <
            0) {
            ok = false;
        }
#else
        ...
        handle->codecContext =
            handle->formatContext->streams[handle->audioStream]->codec;
...
// request float output if supported
       ...
       LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(53, 4, 0))
        ...
        if (ok) {
            QString floatCodec =
                QLatin1String(handle->codec->name) + QLatin1String("float");
            const AVCodec* possibleFloatCodec =
...
size_t FfmpegInput::totalFrames() const {
    ...
    double tmp =
        (double)handle->formatContext->streams[handle->audioStream]->duration *
        (double)handle->formatContext->streams[handle->audioStream]
            ->time_base.num /
        (double)handle->formatContext->streams[handle->audioStream]
            ->time_base.den *
...
float* FfmpegInput::buffer() const { return handle ? handle->buffer : 0; }
...
size_t FfmpegInput::readFrames() {
    ...
    while (handle->currentBytes < BUFFER_SIZE) {
        ...
        handle->bufferList.append(
            QByteArray((const char*)(handle->buffer), bufferSize));
        handle->currentBytes += bufferSize;
    }

    while (handle->bufferList.count() &&
           handle->bufferList.first().size() + bufferPosition <= BUFFER_SIZE) {
...
#else
    int ret = avcodec_decode_audio4(handle->codecContext, handle->frame,
                                    &handle->gotFrame, handle->packet);
    if (ret < 0) {
        return ret;
    }
    ...
    switch (handle->codecContext->sample_fmt) {
        case AV_SAMPLE_FMT_S16: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                handle->buffer[i] = ((float)dataShort[i]) /
                                    qMax(-(float)SHRT_MIN, (float)SHRT_MAX);
            }
            break;
        }
        case AV_SAMPLE_FMT_S32: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                handle->buffer[i] =
                    ((float)dataInt[i]) / qMax(-(float)INT_MIN, (float)INT_MAX);
            }
            break;
        }
        ...
        case AV_SAMPLE_FMT_S16P: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                ...
                handle->buffer[currentSample * numChannels + currentChannel] =
                    ((float)((int16_t*)ed[currentChannel])[currentSample]) /
                    qMax(-(float)SHRT_MIN, (float)SHRT_MAX);
            }
            break;
        }
        case AV_SAMPLE_FMT_S32P: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                ...
                int currentSample = i % handle->frame->nb_samples;
                handle->buffer[currentSample * numChannels + currentChannel] =
                    ((float)((int32_t*)ed[currentChannel])[currentSample]) /
                    qMax(-(float)INT_MIN, (float)INT_MAX);
            }
            break;
        }
        case AV_SAMPLE_FMT_FLTP: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                ...
                int currentSample = i % handle->frame->nb_samples;
                handle->buffer[currentSample * numChannels + currentChannel] =
                    ((float*)ed[currentChannel])[currentSample];
            }
            break;
        }
        case AV_SAMPLE_FMT_DBLP: {
            ...
            for (int i = 0; i < handle->frame->nb_samples * numChannels; ++i) {
                ...
                int currentSample = i % handle->frame->nb_samples;
                handle->buffer[currentSample * numChannels + currentChannel] =
                    ((double*)ed[currentChannel])[currentSample];
            }
            break;
        }
...
static int decodeAudio(FfmpegInput::Handle* handle, int* frame_size_ptr) {
    ...
    ret = avcodec_decode_audio4(handle->codecContext, handle->frame, &got_frame,
                                handle->packet);
    ...
    if (ret >= 0 && got_frame) {
        ...
        int data_size = av_samples_get_buffer_size(
            &plane_size, handle->codecContext->channels,
            handle->frame->nb_samples, handle->codecContext->sample_fmt, 1);
        if (*frame_size_ptr < data_size) {
            return AVERROR(EINVAL);
        }

        memcpy(handle->audioBuffer, handle->frame->extended_data[0],
               plane_size);
...
next_frame:
    ...
    int len = avcodec_decode_audio3(handle->codecContext, handle->audioBuffer,
                                    &dataSize, handle->packet);
#else
    int len = avcodec_decode_audio2(handle->codecContext, handle->audioBuffer,
                                    &dataSize, handle->packet->data,
                                    handle->packet->size);
#endif
    if (len < 0) {
        goto out;
    ...
    switch (handle->codecContext->sample_fmt) {
        case SAMPLE_FMT_S16: {
            ...
            numberRead = (size_t)dataSize / sizeof(int16_t) /
                         (size_t)handle->codecContext->channels;
            for (unsigned int i = 0; i < (size_t)dataSize / sizeof(int16_t);
                 ++i) {
                handle->buffer[i] = ((float)dataShort[i]) /
                                    qMax(-(float)SHRT_MIN, (float)SHRT_MAX);
            }
        ...
        case SAMPLE_FMT_S32: {
            ...
            numberRead = (size_t)dataSize / sizeof(int32_t) /
                         (size_t)handle->codecContext->channels;
            for (unsigned int i = 0; i < (size_t)dataSize / sizeof(int32_t);
                 ++i) {
                handle->buffer[i] =
                    ((float)dataInt[i]) / qMax(-(float)INT_MIN, (float)INT_MAX);
            }
        ...
        case SAMPLE_FMT_FLT: {
            ...
            numberRead = (size_t)dataSize / sizeof(float) /
                         (size_t)handle->codecContext->channels;
            for (unsigned int i = 0; i < (size_t)dataSize / sizeof(float);
                 ++i) {
                handle->buffer[i] = dataFloat[i];
            }
            break;
        ...
        case SAMPLE_FMT_DBL: {
            ...
            numberRead = (size_t)dataSize / sizeof(double) /
                         (size_t)handle->codecContext->channels;
            for (unsigned int i = 0; i < (size_t)dataSize / sizeof(double);
                 ++i) {
                handle->buffer[i] = (float)dataDouble[i];
            }
            break;
...
bool FfmpegInput::isFloatCodec() const {
    return handle && handle->codec &&
           QString(QLatin1String(handle->codec->name))
               .endsWith(QLatin1String("float"));
}
