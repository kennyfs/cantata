...
class FancyTabWidget : public QWidget {
   ...
   public:
    ...
    struct Item {
        Item(const QIcon& icon, const QString& label, const QString& tt,
             bool en)
            : type(Type_Tab),
              tabLabel(label),
              tabTooltip(tt),
              ...
              index(-1) {}
        Item(int size)
            : type(Type_Spacer), spacerSize(size), enabled(true), index(-1) {}
    ...
    void addTab(QWidget* tab, const QIcon& icon, const QString& label,
                const QString& tt = QString(), bool enabled = true);
    int currentIndex() const;
    QWidget* currentWidget() const;
    bool isEnabled(int index) const {
        return index >= 0 && index < items.count() ? items[index].enabled
                                                   : false;
    }
   ...
   private:
    ...
    int IndexToTab(int index) const {
        return index >= 0 && index < items.count() ? items[index].index : 0;
    }
