...
static void drawIcon(const QIcon& icon, const QRect& r, QPainter* p,
                     const QSize& iconSize, bool selected) {
#ifdef Q_OS_WIN
    Q_UNUSED(selected);
    QPixmap px = icon.pixmap(iconSize, QIcon::Normal);
#else
    QPixmap px =
        icon.pixmap(iconSize, selected ? QIcon::Selected : QIcon::Normal);
#endif
    QSize layoutSize = px.size() / px.DEVICE_PIXEL_RATIO();
    p->drawPixmap(r.x() + (r.width() - layoutSize.width()) / 2.0,
                  r.y() + (r.height() - layoutSize.height()) / 2.0,
                  layoutSize.width(), layoutSize.height(), px);
}

void FancyTabProxyStyle::drawPrimitive(PrimitiveElement element,
...
void FancyTabProxyStyle::drawControl(ControlElement element,
    ...
    if (selected || underMouse) {
        ...
        QColor col =
            active ? option->palette.highlight().color() : QColor(96, 96, 96);
        col.setAlphaF(0.25);
#else
        ...
        if (selected) {
#ifndef Q_OS_MAC
            if (!active && option->palette.highlight().color() ==
...
bool FancyTabProxyStyle::eventFilter(QObject* o, QEvent* e) {
    ...
    if (bar && (e->type() == QEvent::MouseMove || e->type() == QEvent::Leave)) {
        ...
        const QString hoveredTab = e->type() == QEvent::Leave
                                       ? QString()
...
FancyTabBar::FancyTabBar(QWidget* parent, bool text, int iSize, Pos pos)
    ...
    if (Side != pos) {
        ...
        layout->addSpacerItem(new QSpacerItem(0, 0, QSizePolicy::Fixed,
...
QSize FancyTab::sizeHint() const {
    ...
    int padding =
        FancyTabBar::Side == tabbar->position() ? Utils::scaleForDpi(12) : 0;
    if (withText) {
        ...
        return QSize(iconSize + spacing + padding,
...
QSize FancyTabBar::tabSizeHint() const {
    ...
    if (withText) {
        ...
        return QSize(qMax(icnSize + spacing, maxTw),
                     icnSize + spacing + fm.height() + padding);
    } else {
...
void FancyTab::enterEvent(QEvent*) { underMouse = true; }

void FancyTab::leaveEvent(QEvent*) { underMouse = false; }

QSize FancyTabBar::sizeHint() const {
    QSize sh = tabSizeHint();
    return Side != pos ? QSize(sh.width() * tabs.count(), sh.height())
...
QRect FancyTabBar::tabRect(int index) const { return tabs[index]->geometry(); }
...
// mode itself
void FancyTabBar::emitCurrentIndex() { emit currentChanged(currentIdx); }
...
void FancyTabBar::addTab(const QIcon& icon, const QString& label,
    ...
    qobject_cast<QBoxLayout*>(layout())->insertWidget(
        layout()->count() - (Side == pos ? 1 : 0), tab);
}
...
void FancyTabBar::paintTab(QPainter* painter, int tabIndex) const {
    ...
    if (selected || underMouse) {
        ...
        QColor col = OSXStyle::self()->viewPalette().highlight().color();
#elif defined Q_OS_WIN
        QColor col = palette().currentColorGroup() == QPalette::Active
    ...
    selected =
        selected && (palette().currentColorGroup() == QPalette::Active ||
                     (palette().highlightedText().color() ==
                      palette()
                          .brush(QPalette::Active, QPalette::HighlightedText)
                          .color()));

    if (withText) {
        QString tabText(painter->fontMetrics().elidedText(
            this->tabText(tabIndex), elideMode(), width()));
...
#ifdef Q_OS_MAC
        painter->setPen(
            selected ? OSXStyle::self()->viewPalette().highlightedText().color()
                     : OSXStyle::self()->viewPalette().windowText().color());
#elif defined Q_OS_WIN
        ...
        drawIcon(tabIcon(tabIndex), tabIconRect, painter,
                 QSize(icnSize, icnSize), selected);
    } else {
...
void FancyTabWidget::addTab(QWidget* tab, const QIcon& icon,
                            const QString& label, const QString& tt,
                            bool enabled) {
...
int FancyTabWidget::currentIndex() const { return stack_->currentIndex(); }
...
int FancyTabWidget::count() const { return stack_->count(); }
...
void FancyTabWidget::setStyle(int s) {
    ...
    // Create new tab bar
    if (Tab == (s & Style_Mask) ||
        (Small == (s & Style_Mask) && !(s & IconOnly))) {
        switch (s & Position_Mask) {
            default:
            case Side:
                makeTabBar(QApplication::isRightToLeft() ? QTabBar::RoundedEast
                                                         : QTabBar::RoundedWest,
                           !(s & IconOnly), true, Small == (s & Style_Mask));
                break;
            case Top:
                makeTabBar(QTabBar::RoundedNorth, !(s & IconOnly), true,
                           Small == (s & Style_Mask));
                break;
            case Bot:
                makeTabBar(QTabBar::RoundedSouth, !(s & IconOnly), true,
                           Small == (s & Style_Mask));
                break;
        }
    } else {
        FancyTabBar* bar = new FancyTabBar(
            this, !(s & IconOnly),
            Small == (s & Style_Mask) ? smallIconSize : largeIconSize,
            Side == (s & Position_Mask)
                ? FancyTabBar::Side
                : (Top == (s & Position_Mask) ? FancyTabBar::Top
                                              : FancyTabBar::Bot));
        switch (s & Position_Mask) {
            default:
            case Side:
    ...
    if (index > -1 && index < items.count()) {
        ...
        if (tabBar && -1 != item.index) {
            if (qobject_cast<QTabBar*>(tabBar)) {
                static_cast<QTabBar*>(tabBar)->setTabToolTip(item.index,
                                                             item.tabTooltip);
            } else {
                static_cast<FancyTabBar*>(tabBar)->setTabToolTip(
                    item.index, item.tabTooltip);
            }
        }
    }
}
...
void FancyTabWidget::setHiddenPages(const QStringList& hidden) {
    ...
    for (int i = 0; i < count(); ++i) {
        QWidget* w = widget(i);
        if (w &&
            items[i].enabled == hidden.contains(w->metaObject()->className())) {
