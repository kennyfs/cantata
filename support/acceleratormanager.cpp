...
static inline QString esc(const QString& orig) { return orig.toHtmlEscaped(); }
...
// K_GLOBAL_STATIC_WITH_ARGS(QStringList, kaccmp_sns,
// (KStandardAction::internal_stdNames()))
...
AcceleratorManagerPrivate::Item::~Item() {
    if (m_children)
        while (!m_children->isEmpty()) delete m_children->takeFirst();
...
void AcceleratorManagerPrivate::calculateAccelerators(Item* item,
                                                      QString& used) {
    if (!item->m_children) return;
    ...
    for (Item* it : *item->m_children) {
        ...
        if (groupBox && !groupBox->isCheckable()) continue;
...
void AcceleratorManagerPrivate::traverseChildren(QWidget* widget, Item* item) {
    ...
    for (QWidget* w : childList) {
        ...
        if (!w->isVisibleTo(widget) ||
            (w->isTopLevel() && qobject_cast<QMenu*>(w) == nullptr))
            continue;

        if (AcceleratorManagerPrivate::ignored_widgets.contains(w)) continue;
...
void AcceleratorManagerPrivate::manageWidget(QWidget* w, Item* item) {
    ...
    if (qobject_cast<QComboBox*>(w) || qobject_cast<QLineEdit*>(w) ||
        w->inherits("Q3TextEdit") || qobject_cast<QTextEdit*>(w) ||
    ...
    if (w->focusPolicy() != Qt::NoFocus || label ||
        qobject_cast<QGroupBox*>(w) || qobject_cast<QRadioButton*>(w)) {
        QString content;
        QVariant variant;
        ...
        if (tprop == -1) {
            ...
            if (tprop != -1) {
                QMetaProperty p = w->metaObject()->property(tprop);
                if (p.isValid() && p.isWritable()) variant = p.read(w);
            }
        }

        if (variant.isValid()) content = variant.toString();
        ...
        if (!content.isEmpty()) {
            ...
            if (qobject_cast<QPushButton*>(w) || qobject_cast<QCheckBox*>(w) ||
                qobject_cast<QRadioButton*>(w) || qobject_cast<QLabel*>(w))
...
void AcceleratorManagerPrivate::manageTabBar(QTabBar* bar, Item* item) {
    ...
    // this looks like a conflict and triggers a new reset of the shortcuts ->
    // endless loop
    ...
    if (parentWidget) {
        ...
        if (mainWindow)  // && (mainWindow->layout()->indexOf(bar) != -1))
                         // QMainWindowLayout lacks proper support
            return;
    }
...
void AcceleratorManagerPrivate::manageDockWidget(QDockWidget* dock,
                                                 Item* item) {
    // As of Qt 4.4.3 setting a shortcut to a QDockWidget has no effect,
    // because a QDockWidget does not grab it, even while displaying an
    // underscore in the title for the given shortcut letter. Still it is useful
    // to set the shortcut, because if QDockWidgets are tabbed, the tab
    // automatically gets the same text as the QDockWidget title, including the
    // shortcut. And for the QTabBar the shortcut does work, it gets grabbed as
    // usual. Having the QDockWidget without a shortcut and resetting the tab
    // text with a title including the shortcut does not work, the tab text is
    // instantly reverted to the QDockWidget title (see also manageTabBar()).
...
void AcceleratorManagerPrivate::manageMenuBar(QMenuBar* mbar, Item* item) {
    ...
    for (int i = 0; i < mbar->actions().count(); ++i) {
        ...
        if (maction->isSeparator()) continue;
        ...
        if (maction->menu()) PopupAccelManager::manage(maction->menu());
    }
}
...
void AcceleratorManager::last_manage(QString& added, QString& changed,
                                     QString& removed) {
...
AccelString::AccelString(const QString& input, int initialWeight)
    ...
    if (m_orig_accel != -1) m_pureText.remove(m_orig_accel, 4);

    m_orig_accel = m_pureText.indexOf("(&&)");
    if (m_orig_accel != -1) m_pureText.replace(m_orig_accel, 4, "&");
    ...
    if (AcceleratorManagerPrivate::programmers_mode) {
        if (m_accel != m_orig_accel) {
            ...
            if (m_accel >= 0) {
                ...
                if (m_accel < m_orig_accel) oa += 4;
            }
            if (m_orig_accel >= 0) result.replace(oa, 1, "(&&)");
        }
    } else {
        if (m_accel >= 0 && m_orig_accel != m_accel) {
            if (m_orig_accel != -1) result.remove(m_orig_accel, 1);
            result.insert(m_accel, "&");
        }
    }
    return result;
...
QChar AccelString::accelerator() const {
    if ((m_accel < 0) || (m_accel > (int)m_pureText.length())) return QChar();
...
void AccelString::calculateWeights(int initialWeight) {
    ...
    while (pos < m_pureText.length()) {
        ...
        if (pos < 50) weight += (50 - pos);

        // try to preserve the wanted accelarators
        if ((int)pos == accel()) {
            weight += AccelManagerAlgorithm::WANTED_ACCEL_EXTRA_WEIGHT;
            // kDebug(240) << "wanted " << m_pureText << " " <<
            // AcceleratorManagerPrivate::standardName(m_origText);
...
int AccelString::stripAccelerator(QString& text) {
    ...
    while (p >= 0) {
        ...
        if (p <= 0 || p >= (int)text.length()) break;
...
int AccelString::maxWeight(int& index, const QString& used) const {
    ...
    for (int pos = 0; pos < m_pureText.length(); ++pos)
        if (used.indexOf(m_pureText[pos], 0, Qt::CaseInsensitive) == -1 &&
            m_pureText[pos].toLatin1() != 0)
            if (m_weight[pos] > max) {
                max = m_weight[pos];
                index = pos;
            }
 ...
 *********************************************************************/
void AccelManagerAlgorithm::findAccelerators(AccelStringList& result,
                                             QString& used) {
    AccelStringList accel_strings = result;

    // initially remove all accelerators
    for (AccelStringList::Iterator it = result.begin(); it != result.end();
         ++it) {
        (*it).setAccel(-1);
    }
...
QWidgetStackAccelManager::QWidgetStackAccelManager(QStackedWidget* stack)
    ...
    currentChanged(
        stack->currentIndex());  // do one check and then connect to show
...
bool QWidgetStackAccelManager::eventFilter(QObject* watched, QEvent* e) {
    ...
    if ((e->type() == QEvent::Show || e->type() == QEvent::Paint) &&
        qApp->activeWindow()) {
...
void QWidgetStackAccelManager::currentChanged(int child) {
    ...
    static_cast<QStackedWidget*>(parent())->widget(child)->installEventFilter(
        this);
}
