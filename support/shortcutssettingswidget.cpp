...
bool ShortcutsFilter::filterAcceptsRow(int source_row,
    ...
    QModelIndex index =
        source_parent.model()->index(source_row, 0, source_parent);
    Q_ASSERT(index.isValid());
    if (!qobject_cast<Action*>(
             index.data(ShortcutsModel::ActionRole).value<QObject*>())
             ->isShortcutConfigurable())
        return false;

    for (int col = 0; col < source_parent.model()->columnCount(source_parent);
         col++) {
        if (source_parent.model()
                ->index(source_row, col, source_parent)
                .data()
                .toString()
                .contains(_filterString, Qt::CaseInsensitive))
            return true;
    }
    return false;
}
...
ShortcutsSettingsWidget::ShortcutsSettingsWidget(
    const QHash<QString, ActionCollection*>& actionCollections, QWidget* parent)
    : QWidget(parent),
    ...
    connect(keySequenceWidget,
            SIGNAL(keySequenceChanged(QKeySequence, QModelIndex)),
            SLOT(keySequenceChanged(QKeySequence, QModelIndex)));

    connect(shortcutsView->selectionModel(),
            SIGNAL(currentChanged(QModelIndex, QModelIndex)),
            SLOT(setWidgetStates()));
    ...
    //   connect(_shortcutsModel, SIGNAL(hasChanged(bool)),
    //   SLOT(setChangedState(bool)));

    // fugly, but directly setting it from the ctor doesn't seem to work
    QTimer::singleShot(0, searchEdit, SLOT(setFocus()));
}

QTreeView* ShortcutsSettingsWidget::view() { return shortcutsView; }

void ShortcutsSettingsWidget::setWidgetStates() {
    if (shortcutsView->currentIndex().isValid() &&
        shortcutsView->currentIndex().parent().isValid()) {
        QKeySequence active = shortcutsView->currentIndex()
                                  .data(ShortcutsModel::ActiveShortcutRole)
                                  .value<QKeySequence>();
        QKeySequence def = shortcutsView->currentIndex()
                               .data(ShortcutsModel::DefaultShortcutRole)
                               .value<QKeySequence>();
        defaultShortcut->setText(def.isEmpty()
                                     ? tr("None")
...
void ShortcutsSettingsWidget::keySequenceChanged(
    const QKeySequence& seq, const QModelIndex& conflicting) {
    if (conflicting.isValid())
        _shortcutsModel->setData(conflicting, QKeySequence(),
                                 ShortcutsModel::ActiveShortcutRole);

    QModelIndex rowIdx =
        _shortcutsFilter->mapToSource(shortcutsView->currentIndex());
...
void ShortcutsSettingsWidget::toggledCustomOrDefault() {
    if (!shortcutsView->currentIndex().isValid()) return;
    ...
    if (useDefault->isChecked()) {
        _shortcutsModel->setData(
            index, index.data(ShortcutsModel::DefaultShortcutRole));
    } else {
        _shortcutsModel->setData(index, QKeySequence());
    }
    setWidgetStates();

    // If custom is selected, and the action has no short-cut, setWidgetStates()
    // re-checks the default radio. This is a bit counter-intuitive, so ensure
    // whichever radio caused the toggle, that it is checked.
...
void ShortcutsSettingsWidget::save() { _shortcutsModel->commit(); }
