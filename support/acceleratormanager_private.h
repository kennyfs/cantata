...
class AccelString {
   public:
    ...
    bool operator==(const AccelString& c) const {
        return m_pureText == c.m_pureText && m_accel == c.m_accel &&
               m_orig_accel == c.m_orig_accel;
    }
...
class AccelManagerAlgorithm {
   public:
    enum {
        ...
        // Additional weight for the dialog buttons (large, we basically never
        // want these reassigned)
