...
// This defines the unicode symbols for special keys (kCommandUnicode and
// friends)
#ifdef Q_OS_MAC
...
void KeySequenceButton::keyPressEvent(QKeyEvent* e) {
    ...
    if (keyQt == -1) {
        ...
        QMessageBox::information(
            this, tr("The key you just pressed is not supported by Qt."),
            tr("Unsupported Key"));
    ...
    if (!d->isRecording()) return QPushButton::keyPressEvent(e);
    ...
    switch (keyQt) {
        ...
        default:
            if (!(d->_modifierKeys & ~Qt::SHIFT)) {
                ...
                // allowed
                if (!d->isOkWhenModifierless(keyQt)) return;
            }

            // We now have a valid key press.
            if (keyQt) {
                if ((keyQt == Qt::Key_Backtab) &&
                    (d->_modifierKeys & Qt::SHIFT)) {
...
void KeySequenceButton::keyReleaseEvent(QKeyEvent* e) {
    ...
    if (!d->isRecording()) return QPushButton::keyReleaseEvent(e);
...
KeySequenceWidget::KeySequenceWidget(QWidget* parent)
    ...
    _keyButton->setToolTip(
        tr("Click on the button, then enter the shortcut like you would in the "
...
bool KeySequenceWidget::isShiftAsModifierAllowed(int keyQt) const {
    ...
    if (keyQt >= Qt::Key_F1 && keyQt <= Qt::Key_F35) return true;

    if (QChar(keyQt).isLetter()) return true;
...
void KeySequenceWidget::setKeySequence(const QKeySequence& seq) {
    ...
    // setKeySequence() is called while not recording then set oldKeySequence to
    // the existing sequence so that the keySequenceChanged() signal is emitted
    // if the new and previous key sequences are different
    if (!isRecording()) _oldKeySequence = _keySequence;
...
bool KeySequenceWidget::isKeySequenceAvailable(const QKeySequence& seq) {
    ...
    for (int cat = 0; cat < _shortcutsModel->rowCount(); cat++) {
        ...
        for (int r = 0; r < _shortcutsModel->rowCount(catIdx); r++) {
            ...
            if (actIdx.data(ShortcutsModel::ActiveShortcutRole)
                    .value<QKeySequence>() != seq)
                continue;

            if (!actIdx.data(ShortcutsModel::IsConfigurableRole).toBool()) {
                QMessageBox::warning(
                    this, tr("Shortcut Conflict"),
                    tr("The \"%1\" shortcut is already in use, and cannot be "
            ...
            QMessageBox box(QMessageBox::Warning, tr("Shortcut Conflict"),
                            (tr("The \"%1\" shortcut is ambiguous with the "
                                "shortcut for the following action:") +
                             "<br><ul><li>%2</li></ul><br>" +
                             tr("Do you want to reassign this shortcut to the "
                                "selected action?"))
                                .arg(seq.toString(QKeySequence::NativeText),
                                     actIdx.data().toString()),
