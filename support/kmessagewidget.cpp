...
void KMsgWidgetPrivate::init(KMsgWidget* q_ptr) {
    ...
    QObject::connect(timeLine, SIGNAL(valueChanged(qreal)), q,
                     SLOT(slotTimeLineChanged(qreal)));
    QObject::connect(timeLine, SIGNAL(finished()), q,
                     SLOT(slotTimeLineFinished()));
    ...
    closeAction->setIcon(MonoIcon::icon(FontAwesome::close, MonoIcon::constRed,
                                        MonoIcon::constRed));
...
void KMsgWidgetPrivate::updateSnapShot() {
    ...
    content->render(&contentSnapShot, QPoint(), QRegion(),
...
KMsgWidget::~KMsgWidget() { delete d; }

QString KMsgWidget::text() const { return d->textLabel->text(); }
...
void KMsgWidget::setMessageType(KMsgWidget::MessageType type) {
    ...
    switch (type) {
        case Positive:
            ...
            break;
        case Information:
            // There is no "information" background role in KColorScheme, use
            // the colors of highlighted items instead
    ...
    d->content->setStyleSheet(
        QString(".QFrame {"
            ...
            .arg(bg2.name())
            .arg(border.name())
            // DefaultFrameWidth returns the size of the external margin +
            // border width. We know our border is 1px, so we subtract this from
            // the frame normal QStyle FrameWidth to get our margin
            .arg(style()->pixelMetric(QStyle::PM_DefaultFrameWidth, nullptr,
                                      this) -
                 1)
            .arg(fg.name()));
}
...
bool KMsgWidget::wordWrap() const { return d->wordWrap; }
