...
ShortcutsModel::ShortcutsModel(
    const QHash<QString, ActionCollection*>& actionCollections, QObject* parent)
    : QAbstractItemModel(parent), _changedCount(0) {
...
QModelIndex ShortcutsModel::parent(const QModelIndex& child) const {
    ...
    if (!item->parentItem) return QModelIndex();
...
QModelIndex ShortcutsModel::index(int row, int column,
    ...
    if (parent.isValid())
        return createIndex(
            row, column,
            static_cast<Item*>(parent.internalPointer())->actionItems.at(row));
...
int ShortcutsModel::rowCount(const QModelIndex& parent) const {
    if (!parent.isValid()) return _categoryItems.count();

    Item* item = static_cast<Item*>(parent.internalPointer());
    Q_ASSERT(item);

    if (!item->parentItem) return item->actionItems.count();
...
QVariant ShortcutsModel::data(const QModelIndex& index, int role) const {
    ...
    switch (role) {
        ...
        case Qt::DecorationRole:
            if (index.column() == 0 && _showIcons) return action->icon();
            return QVariant();
...
bool ShortcutsModel::setData(const QModelIndex& index, const QVariant& value,
                             int role) {
    if (role != ActiveShortcutRole) return false;
    ...
    if (oldSeq == storedSeq && newSeq != storedSeq) {
        if (++_changedCount == 1) emit hasChanged(true);
    } else if (oldSeq != storedSeq && newSeq == storedSeq) {
        if (--_changedCount == 0) emit hasChanged(false);
    }
...
void ShortcutsModel::load() {
    for (Item* catItem : _categoryItems) {
        for (Item* actItem : catItem->actionItems) {
            actItem->shortcut =
                actItem->action->shortcut(Action::ActiveShortcut);
        }
    }
...
void ShortcutsModel::commit() {
    for (Item* catItem : _categoryItems) {
        for (Item* actItem : catItem->actionItems) {
            actItem->action->setShortcut(actItem->shortcut,
...
void ShortcutsModel::defaults() {
    for (int cat = 0; cat < rowCount(); cat++) {
        ...
        for (int act = 0; act < rowCount(catidx); act++) {
            ...
            setData(actidx, actidx.data(DefaultShortcutRole),
                    ActiveShortcutRole);
        }
    }
}
